...
class ErrorRecovery:
    ...
    def _register_default_patterns(self):
        ...
        patterns = [
            ...
            # CUDA OOMエラー
            ErrorPattern(r"CUDA out of memory", RuntimeError, "fix_cuda_oom"),
            # パス関連エラー
            ErrorPattern(r"hooks\.start.*import start", ImportError, "fix_phase0_import"),
            # タイムアウトエラー
            ErrorPattern(r"Command timed out", TimeoutError, "fix_timeout"),
            # 権限エラー
            ErrorPattern(r"Permission denied", PermissionError, "fix_permission"),
        ]
...
class AutoRecoveryExecutor:
    ...
    def execute_with_recovery(self, func: Callable, *args, **kwargs) -> Any:
        ...
        Returns:
            関数の実行結果
        """
        context = {"function": func.__name__, "retry_count": 0, "max_retries": 3}
