...
class DashboardStructureValidator:
    ...
    def _has_sidebar_structure(self, html_content: str) -> bool:
        ...
        return any(re.search(pattern, html_content, re.IGNORECASE) for pattern in sidebar_patterns)
    ...
    def _has_main_content_structure(self, html_content: str) -> bool:
        ...
        return any(re.search(pattern, html_content, re.IGNORECASE) for pattern in content_patterns)

    def _count_dashboard_links(self, html_content: str) -> int:
        """ダッシュボードリンク数カウント"""
        link_patterns = [r"/tracker/[A-Z]+-\d+", r'href="[^"]*tracker[^"]*"']
        count = 0
    ...
    def _has_tracker_content(self, html_content: str, tracker_id: str) -> bool:
        """トラッカーコンテンツ存在確認"""
        content_indicators = [tracker_id, "dashboard.html", "品質スコア", "統計分析"]
    ...
    def _count_statistical_sections(self, html_content: str) -> int:
        """統計分析セクション数カウント"""
        stats_patterns = [r"統計分析結果", r"p値", r"効果サイズ", r"改善率"]
        count = 0
