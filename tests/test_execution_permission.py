...
class TestExecutionPermissionManager(unittest.TestCase):
    ...
    def test_default_permission_level(self):
        """デフォルト権限レベルのテスト"""
        self.assertEqual(self.manager.get_current_level(), PermissionLevel.EXECUTE_FULL)

    def test_set_permission_level(self):
        """権限レベル設定のテスト"""
        self.manager.set_permission_level(PermissionLevel.READ_ONLY)
        self.assertEqual(self.manager.get_current_level(), PermissionLevel.READ_ONLY)
    ...
    def test_permission_check_read_only(self):
        ...
        # 読み取りは許可
        self.assertTrue(self.manager.check_permission(ActionType.READ))

        # 書き込みは拒否
        self.assertFalse(self.manager.check_permission(ActionType.WRITE))

        # 削除は拒否
        self.assertFalse(self.manager.check_permission(ActionType.DELETE))
    ...
    def test_permission_check_plan_only(self):
        ...
        # 実行は拒否
        self.assertFalse(self.manager.check_permission(ActionType.EXECUTE))
    ...
    def test_state_persistence(self):
        ...
        # 権限レベルが保持されている
        self.assertEqual(new_manager.get_current_level(), PermissionLevel.PLAN_ONLY)
