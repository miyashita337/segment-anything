...
class TestTEST001Extraction(unittest.TestCase):
    ...
    def test_file_format(self):
        ...
        for file in extracted_files:
            # JPEG形式確認
            self.assertTrue(file.suffix.lower() == ".jpg", f"{file.name}の拡張子が不正")

            # 画像として開けることを確認
            try:
                img = Image.open(file)
                self.assertEqual(img.format, "JPEG", f"{file.name}がJPEG形式ではありません")
                img.close()
    ...
    def test_file_dimensions(self):
        ...
        for file in extracted_files:
            ...
            # 最小サイズ確認（100x100以上）
            self.assertGreaterEqual(width, 100, f"{file.name}の幅{width}pxが最小値未満")
            self.assertGreaterEqual(height, 100, f"{file.name}の高さ{height}pxが最小値未満")

            # 最大サイズ確認（4000x4000以下）
            self.assertLessEqual(width, 4000, f"{file.name}の幅{width}pxが最大値超過")
            self.assertLessEqual(height, 4000, f"{file.name}の高さ{height}pxが最大値超過")
    ...
    def test_file_naming_convention(self):
        ...
        for file in extracted_files:
            ...
            stem = file.stem
            self.assertTrue(stem.startswith("extracted_"), f"{file.name}が命名規則に従っていません")
    ...
    def test_specific_success_files(self):
        ...
        for filename, expected_size in success_files:
            ...
            self.assertTrue(file_path.exists(), f"期待される成功ファイル{filename}が存在しません")

            # サイズ誤差10%以内
            actual_size = file_path.stat().st_size
            size_diff = abs(actual_size - expected_size) / expected_size
            self.assertLessEqual(
                size_diff, 0.1, f"{filename}のサイズ{actual_size}が期待値{expected_size}と10%以上異なります"
            )
    ...
    def test_failure_patterns(self):
        ...
        for filename in failed_files:
            file_path = self.extraction_dir / filename
            self.assertFalse(file_path.exists(), f"失敗すべき{filename}が存在しています")
