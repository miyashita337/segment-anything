...
class TestHookIntegration(unittest.TestCase):
    ...
    def test_analyze_tracker_context_hook(self):
        ...
        test_input = {
            "prompt": f"{self.tracker_id}の進捗を確認して次のステップを教えてください。現在Phase 1の計画フェーズです。"
        }
    ...
    def test_hook_sequence_workflow(self):
        ...
        pre_result = self._run_hook_script(
            "tools/hooks/validate_workflow_compliance.sh",
            pre_input
        )
        ...
        post_result = self._run_hook_script(
            "tools/hooks/update_workflow_progress.sh",
            post_input
        )
