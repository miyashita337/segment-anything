...
class TestQualityScore:
    ...
    def test_initialization(self):
        """初期化のテスト"""
        score = QualityScore(QualityMetric.ACCURACY, QualityLevel.PIXEL, 0.85, 0.9)
    ...
    def test_to_dict(self):
        """辞書変換のテスト"""
        score = QualityScore(
            QualityMetric.COMPLETENESS, QualityLevel.OBJECT, 0.75, 0.8, {"test_detail": "value"}
        )
...
class TestHierarchicalQualityEvaluator:
    ...
    def test_evaluate_batch(self, evaluator):
        ...
        for i in range(3):
            ...
            items.append({"id": f"item_{i}", "image": image, "mask": mask})
