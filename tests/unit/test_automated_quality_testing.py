...
class TestAutomatedQualityTesting(unittest.TestCase):
    ...
    def setUp(self):
        ...
        with patch("tools.core.automated_quality_testing.project_root", self.base_workspace_path):
            ...
            for dir_path in [
                self.system.baseline_dir,
                self.system.test_results_dir,
                self.system.workspace_dir,
            ]:
                try:
    ...
    def test_baseline_creation(self):
        """ベースライン作成テスト"""
        # モック設定
        with patch.object(self.system, "_run_extraction_pipeline", return_value=True), patch.object(
            self.system,
            "_evaluate_quality",
            return_value={
...
def main():
    ...
    # テストケース追加
    test_classes = [TestAutomatedQualityTesting, TestQualityBaseline, TestTestResult]
