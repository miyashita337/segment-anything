...
class TestYOLOThresholds:
    ...
    def test_images(self):
        """テスト用画像のパスを提供"""
        return ["test_small/img001.jpg", "test_small/img002.jpg", "test_small/img003.jpg"]
    ...
    def test_threshold_comparison(self, yolo_model, test_images):
        ...
        for img_path in test_images:
            if os.path.exists(img_path):
                ...
                detections_007 += (
                    len(results_007[0].boxes) if results_007[0].boxes is not None else 0
                )

                # 0.005での検出
                results_005 = yolo_model(img_path, conf=0.005, verbose=False)
                detections_005 += (
                    len(results_005[0].boxes) if results_005[0].boxes is not None else 0
                )
    ...
    @pytest.mark.parametrize(
        "threshold,min_detections", [(0.1, 1), (0.07, 4), (0.05, 10), (0.01, 30), (0.005, 40)]
    )
