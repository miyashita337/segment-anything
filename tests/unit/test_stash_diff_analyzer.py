...
3\t1\tother.py"""
        ...
        expected = {"test.py": (5, 2), "image.png": (0, 0), "other.py": (3, 1)}  # バイナリファイルは0,0として処理
    ...
    def test_classify_change_type_documentation(self):
        ...
        change_type = self.analyzer._classify_change_type(
            "docs/README.md", "Some documentation changes"
        )
...
"""
        ...
        change_type = self.analyzer._classify_change_type("features/test.py", diff_content)
...
+
 def test():
     pass"""
        ...
        change_type = self.analyzer._classify_change_type("test.py", diff_content)
...
"""
        ...
        change_type = self.analyzer._classify_change_type("processing.py", diff_content)
    ...
    def test_determine_action_critical_files(self):
        ...
        action = self.analyzer._determine_action(
            "features/extraction/commands/extract_character.py", ChangeType.FORMAT_ONLY, 2, 2
        )
        self.assertEqual(action, RecommendedAction.DISCARD)

        # 重要ファイルの機能変更 → REVIEW
        action = self.analyzer._determine_action(
            "features/extraction/commands/extract_character.py", ChangeType.FUNCTIONAL, 10, 5
        )
        self.assertEqual(action, RecommendedAction.REVIEW)

    def test_determine_action_tracker_id(self):
        """トラッカーID変更のアクション決定テスト"""
        action = self.analyzer._determine_action("some_file.py", ChangeType.TRACKER_ID, 3, 3)

        self.assertEqual(action, RecommendedAction.INTEGRATE)

    def test_determine_action_documentation(self):
        """ドキュメント変更のアクション決定テスト"""
        action = self.analyzer._determine_action("docs/README.md", ChangeType.DOCUMENTATION, 5, 2)
    ...
    def test_calculate_confidence(self):
        ...
        confidence = self.analyzer._calculate_confidence(
            ChangeType.FORMAT_ONLY, "normal_file.py", "some diff content"
        )
    ...
    def test_generate_reasons(self):
        ...
        reasons = self.analyzer._generate_reasons(
            ChangeType.FUNCTIONAL, "features/extraction/commands/extract_character.py", 100, 50
        )
...
+new line 2"""
    ...
    def test_analyze_file_changes(self):
        ...
        with patch.object(self.analyzer, "_get_file_diff") as mock_get_diff:
            ...
            self.assertEqual(
                analysis.file_path, "features/extraction/commands/extract_character.py"
            )
