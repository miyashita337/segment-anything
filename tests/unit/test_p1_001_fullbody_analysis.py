...
from utils.learned_quality_assessment import ImageCharacteristics, LearnedQualityAssessment
...
class TestCurrentFullBodyDetection(unittest.TestCase):
    ...
    def test_aspect_ratio_fullbody_scoring(self):
        ...
        for height, width, expected_good in test_cases:
            with self.subTest(height=height, width=width):
                ...
                print(
                    f"Aspect ratio {aspect_ratio:.2f}: score={fullbody_score:.3f}, "
    ...
    def test_fullbody_priority_weights(self):
        ...
        self.assertAlmostEqual(
            total_weight, 1.0, places=2, msg="Fullbody priority weights should sum to 1.0"
        )

        # Fullbody weight should be highest
        max_weight = max(expected_weights.values())
        self.assertEqual(
            expected_weights["fullbody"],
            max_weight,
        ...
        print(
            f"Fullbody weight dominance: {expected_weights['fullbody']:.2f} "
