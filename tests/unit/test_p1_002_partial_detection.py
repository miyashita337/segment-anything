...
class TestPartialExtractionDetector(unittest.TestCase):
    ...
    def test_face_only_mask_analysis(self):
        ...
        print(
            f"Face-only analysis: completeness={analysis.completeness_score:.3f}, "
    ...
    def test_full_body_mask_analysis(self):
        ...
        print(
            f"Full-body analysis: completeness={analysis.completeness_score:.3f}, "
            f"quality={analysis.quality_assessment}, issues={len(analysis.issues)}"
        )
        ...
        self.assertEqual(
            len(face_only_issues), 0, "Full body mask should not have face_only issues"
        )
    ...
    def test_limb_presence_analysis(self):
        ...
        self.assertFalse(
            has_limbs_central, "Central region only should not be detected as having limbs"
        )
    ...
    def test_completeness_score_calculation(self):
        ...
        for has_face, has_torso, has_limbs, issues, expected_min in test_cases:
            score = self.detector._calculate_completeness_score(
                has_face, has_torso, has_limbs, issues
            )
