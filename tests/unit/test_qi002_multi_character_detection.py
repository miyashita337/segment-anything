...
class TestCharacterSeparator:
    ...
    def test_create_individual_character_images(self, separator):
        ...
        # 赤キャラクターは赤い色素が多い
        red_char = char1_img if np.mean(char1_img[:,:,0]) > np.mean(char1_img[:,:,1]) else char2_img
...
class TestQI002MultiCharacterIntegration:
    ...
    def test_qi002_full_pipeline_integration(self):
        ...
        if any(cls is None for cls in [MultiCharacterDetector, CharacterSeparator, CharacterQualityAssessor]):
