...
class TestWorkspaceConfig(unittest.TestCase):
    ...
    def setUp(self):
        ...
        self.expected_workspace = Path(
            f"/mnt/c/AItools/lora/train/yado/tracker-workspace/{self.test_tracker_id}"
        )
    ...
    def test_tracker_id_validation(self):
        """トラッカーID形式検証テスト"""
        valid_tracker_ids = ["INTG-087", "QUAL-001", "KIRO-002", "INCI-004", "OPTM-007"]
    ...
    def test_invalid_tracker_id_handling(self):
        """不正なトラッカーID処理テスト"""
        invalid_tracker_ids = ["", "invalid", "TOOLONG-001", "123-ABC", None]
    ...
    def test_multiple_tracker_isolation(self):
        ...
        self.assertEqual(len(unique_paths), len(tracker_ids), "All workspaces should be unique")
    ...
    def test_workspace_config_error_handling(self):
        ...
        error_cases = [("", "空文字列"), ("   ", "空白文字"), ("invalid_format", "不正フォーマット")]
