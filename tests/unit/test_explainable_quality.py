...
from features.evaluation.explainable_quality import (
    ExplainableQualityEvaluator, VisualExplainer, TextualExplainer,
    QualityFactor, ExplanationResult, ExplanationType,
...
class TestVisualExplainer:
    ...
    def test_create_comparison_chart(self, explainer):
        ...
        for i in range(3):
            ...
            result.factors = [
                QualityFactor(f"要因{j}", np.random.rand(), 0.5, f"説明{j}")
                for j in range(3)
            ]
...
class TestExplainableQualityEvaluator:
    ...
    def test_evaluate_with_explanation(self, evaluator, sample_data):
        ...
        result = evaluator.evaluate_with_explanation(
            "test_item", image, mask, quality_scores
        )
    ...
    def test_save_explanations(self, evaluator, sample_data, tmp_path):
        ...
        # 複数の評価を実行
        for i in range(3):
            evaluator.evaluate_with_explanation(
                f"item_{i}", image, mask, quality_scores
            )
    ...
    def test_full_evaluation_pipeline(self, tmp_path):
        ...
        for i in range(3):
            ...
            if i == 0:  # 高品質
                ...
                quality_scores = {'coverage': 0.9, 'edge_accuracy': 0.8, 
                                'clarity': 0.85, 'size_relevance': 0.9, 
                                'position_relevance': 0.95}
            elif i == 1:  # 中品質
                cv2.ellipse(mask, (50, 50), (25, 15), 0, 0, 360, 1, -1)
                quality_scores = {'coverage': 0.6, 'edge_accuracy': 0.5,
                                'clarity': 0.7, 'size_relevance': 0.6,
                                'position_relevance': 0.7}
            else:  # 低品質
                mask[80:90, 80:90] = 1
                quality_scores = {'coverage': 0.2, 'edge_accuracy': 0.3,
                                'clarity': 0.4, 'size_relevance': 0.1,
                                'position_relevance': 0.2}
            
            result = evaluator.evaluate_with_explanation(
                f"sample_{i}", image, mask, quality_scores
            )
