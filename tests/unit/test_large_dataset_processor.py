...
class TestLargeDatasetProcessor:
    ...
    def test_process_single_chunk_failure(self, mock_subprocess):
        ...
        with tempfile.TemporaryDirectory() as temp_dir:
            ...
            # 処理実行
            success = processor._process_single_chunk(
                chunk=chunk_files, output_dir=output_dir, processing_params={}
            )
...
class TestLargeDatasetIntegration:
    ...
    def test_sequential_processing_simulation(self, sample_images_large, mock_sam_yolo_success):
        ...
        config = LargeDatasetConfig(chunk_size=5, enable_progressive_processing=True)
        processor = LargeDatasetProcessor("P1-015-TEST", config)
        ...
        success = processor.process_large_dataset(
            input_dir=temp_input_dir, processing_params={"score_threshold": 0.05}
        )
    ...
    @patch(
        "features.processing.large_dataset_processor.LargeDatasetProcessor._process_chunks_parallel"
    )
    def test_parallel_processing_selection(self, mock_parallel, sample_images_large):
        """並列処理選択テスト"""
        image_files, temp_input_dir = sample_images_large

        config = LargeDatasetConfig(enable_progressive_processing=False)  # 並列処理を選択
        processor = LargeDatasetProcessor("P1-015-TEST", config)
        ...
        # 処理実行 - 正しいシグネチャを使用
        success = processor.process_large_dataset(input_dir=temp_input_dir, processing_params={})
    ...
    def test_nonexistent_directory_handling(self):
        ...
        success = processor.process_large_dataset(
            input_dir="/nonexistent/directory", processing_params={}
        )
    ...
    def test_empty_directory_handling(self):
        ...
        with tempfile.TemporaryDirectory() as empty_dir:
            success = processor.process_large_dataset(input_dir=empty_dir, processing_params={})
