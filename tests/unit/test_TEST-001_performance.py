...
class TestTEST001Performance(unittest.TestCase):
    ...
    def test_success_rate(self):
        ...
        self.assertGreaterEqual(
            success_rate, 0.90,
            f"成功率{success_rate:.1%}が基準90%未満"
        )
        
        # 実際は92.9%
        self.assertAlmostEqual(
            success_rate, 0.929, places=2,
            msg="成功率計算が不正確"
        )
    ...
    def test_processing_speed(self):
        ...
        self.assertLessEqual(
            avg_time, 600,
            f"平均処理時間{avg_time:.1f}秒が基準600秒超過"
        )
        
        # 実際は約350秒/画像
        self.assertAlmostEqual(
            avg_time, 350.6, places=0,
            msg="平均処理時間が予期しない値"
        )
    ...
    def test_memory_usage(self):
        ...
        # RAM使用量が4GB以下
        self.assertLessEqual(
            self.metrics["ram_usage"], 4096,
            f"RAM使用量{self.metrics['ram_usage']:.1f}MBが制限4GB超過"
        )
        
        # GPU使用量が8GB以下
        self.assertLessEqual(
            self.metrics["gpu_usage"], 8192,
            f"GPU使用量{self.metrics['gpu_usage']:.1f}MBが制限8GB超過"
        )
    ...
    def test_output_quality_distribution(self):
        ...
        if total > 0:
            ...
            for category, count in size_categories.items():
                ...
                self.assertGreaterEqual(
                    ratio, 0.10,
                    f"{category}カテゴリが{ratio:.1%}で10%未満"
                )
