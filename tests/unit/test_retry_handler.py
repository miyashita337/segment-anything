...
class TestRetryHandler:
    ...
    def test_retry_with_specific_exceptions(self):
        """特定の例外のみリトライ"""
        config = RetryConfig(max_retries=3, initial_delay=0.1, retry_on_exceptions=(ValueError,))
    ...
    def test_exponential_backoff(self):
        """指数バックオフの動作確認"""
        config = RetryConfig(
            max_retries=3, initial_delay=0.1, exponential_backoff=True, backoff_factor=2.0
        )
        handler = RetryHandler(config)
    ...
    def test_retry_with_fallback(self):
        ...
        decorated = handler.retry_with_fallback(lambda: mock_main(), lambda: mock_fallback())
