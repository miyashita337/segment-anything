...
class TestUniversalStatisticalAnalyzer:
    ...
    def test_cohens_d_calculation_accuracy(self, analyzer, sample_data):
        ...
        result = analyzer.calculate_enhanced_cohens_d(
            sample_data["current"], sample_data["baseline"], "TEST-CURRENT", "TEST-BASELINE"
        )
    ...
    def test_effect_magnitude_categories(self, analyzer):
        ...
        for cohens_d, expected_category in test_cases:
            ...
            assert (
                expected_category in category
            ), f"Expected '{expected_category}' in '{category}' for d={cohens_d}"
    ...
    def test_integrated_analysis_success(self, mock_sqa, analyzer):
        ...
        mock_analyzer_instance.load_extraction_results.side_effect = [
        ...
        with patch.object(analyzer, "update_google_sheets_statistics", return_value=True):
            result = analyzer.run_integrated_analysis("TEST-CURRENT", "TEST-BASELINE", verbose=True)
    ...
    def test_integrated_analysis_data_load_failure(self, analyzer):
        ...
        with patch.object(
            analyzer.statistical_analyzer, "load_extraction_results", return_value=None
        ):
            result = analyzer.run_integrated_analysis("INVALID-001", "INVALID-002", verbose=False)
    ...
    def test_division_by_zero_handling(self, analyzer):
        ...
        result = analyzer.calculate_enhanced_cohens_d(identical_data, identical_data)
    ...
    def test_empty_data_handling(self, analyzer):
        ...
        with pytest.raises(Exception):
            analyzer.calculate_enhanced_cohens_d(np.array([]), np.array([]))
    ...
    def test_mismatched_data_types(self, analyzer):
        ...
        result = analyzer.calculate_enhanced_cohens_d(np.array(current), baseline)
...
class TestIntegrationScenarios:
    ...
    def test_full_workflow_with_mocked_data(self):
        ...
        with patch("tools.progress_tracker.universal_statistical_analyzer.GoogleSheetsClient"):
            with patch("tools.progress_tracker.universal_statistical_analyzer.get_default_config"):
                with patch(
                    "tools.progress_tracker.universal_statistical_analyzer.StatisticalQualityAnalyzer"
                ):
                    analyzer = UniversalStatisticalAnalyzer()

                    # データモック
                    mock_metrics = Mock(quality_scores=list(np.random.normal(0.7, 0.1, 30)))
                    analyzer.statistical_analyzer.load_extraction_results = Mock(
                        return_value=mock_metrics
                    )

                    # Google Sheetsモック
                    analyzer.sheets_client.get_sheet_values = Mock(
                        return_value=[["トラッカーID"], ["INCI-004"]]
                    )
                    analyzer.sheets_client.update_sheet_values = Mock(return_value=True)

                    # 実行
                    result = analyzer.run_integrated_analysis("INCI-004", "QUAL-001", verbose=True)
