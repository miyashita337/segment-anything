...
from features.analysis.sample_size_validator import (
    SampleSizeValidator, TestType, SampleSizeRequirement, StatisticalValidation
)
...
class TestSampleSizeValidator(unittest.TestCase):
    ...
    def test_statistical_warnings_generation(self):
        ...
        clt_warning_found = any(
            "30サンプル以上" in warning 
            for warning in validation.improvement_suggestions
        )
...
class TestEdgeCases(unittest.TestCase):
    ...
    def test_zero_effect_size(self):
        ...
        with self.assertRaises(ZeroDivisionError):
            # 効果サイズ0では計算不可
            self.validator.calculate_required_sample_size(
                TestType.ONE_SAMPLE_T, effect_size=0.0
            )
