...
class TestPlanCommandHandler(unittest.TestCase):
    ...
    def test_validate_inputs_success(self):
        """入力検証成功テスト"""
        # 正常な入力
        is_valid, error = self.handler.validate_inputs("TRACKER-001", "テスト概要", "テスト詳細説明")
    ...
    def test_validate_inputs_empty_tracker_id(self):
        """トラッカーID空文字テスト"""
        is_valid, error = self.handler.validate_inputs("", "テスト概要", "テスト詳細説明")
    ...
    def test_validate_inputs_empty_summary(self):
        """概要空文字テスト"""
        is_valid, error = self.handler.validate_inputs("TRACKER-001", "", "テスト詳細説明")
    ...
    def test_validate_inputs_empty_details(self):
        """詳細空文字テスト"""
        is_valid, error = self.handler.validate_inputs("TRACKER-001", "テスト概要", "")
    ...
    def test_validate_inputs_details_too_long(self):
        ...
        is_valid, error = self.handler.validate_inputs("TRACKER-001", "テスト概要", long_details)
    ...
    def test_validate_inputs_details_max_length(self):
        ...
        is_valid, error = self.handler.validate_inputs("TRACKER-001", "テスト概要", max_details)
    ...
    def test_execute_plan_command_success(self):
        ...
        success, message = self.handler.execute_plan_command("TRACKER-001", "テスト概要", "テスト詳細説明")
    ...
    def test_execute_plan_command_validation_error(self):
        """planコマンド実行（入力検証エラー）"""
        success, message = self.handler.execute_plan_command("", "テスト概要", "テスト詳細説明")
    ...
    def test_execute_plan_command_priority_mapping(self):
        ...
        success, message = self.handler.execute_plan_command(
            "TRACKER-001", "テスト概要", "テスト詳細説明", "high"
        )
    ...
    def test_japanese_characters(self):
        """日本語文字テスト"""
        is_valid, error = self.handler.validate_inputs(
            "TRACKER-001", "日本語の概要テスト", "これは日本語の詳細説明です。\n改行も含まれています。\n特殊文字：！？（）「」"
        )
    ...
    def test_unicode_characters(self):
        """Unicode文字テスト"""
        is_valid, error = self.handler.validate_inputs(
            "TRACKER-001", "Unicode test: 🚀 ✅ ❌", "Emoji and symbols: 📋 🔧 ⚠️ 🎯"
        )
    ...
    def test_boundary_values(self):
        ...
        # 詳細が制限ちょうど
        max_details = "a" * PlanCommandHandler.MAX_DETAILS_LENGTH
        is_valid, error = self.handler.validate_inputs("TRACKER-001", "境界値テスト", max_details)
        ...
        is_valid, error = self.handler.validate_inputs("TRACKER-001", "境界値テスト", over_details)
...
class TestPlanCommandHandlerIntegration(unittest.TestCase):
    ...
    def test_initialization_error_handling(self, mock_progress_manager_class):
        ...
        with patch(
            "tools.progress_tracker.config.get_default_config",
