...
class IntegratedTestRunner:
    ...
    def run_tests(
        ...
        if level == TestLevel.ALL:
            levels_to_run = [TestLevel.LEVEL_1, TestLevel.LEVEL_2, TestLevel.LEVEL_3, TestLevel.LEVEL_4]
        else:
            levels_to_run = [level]
    ...
    def _generate_summary(self, overall_success: bool, verbose: bool) -> Dict[str, Any]:
        ...
        summary = {
            ...
            "level_stats": {
                ...
                "level_success_rate": (levels_passed / len(self.test_results) * 100) if self.test_results else 0.0
            },
    ...
    def _print_summary(self, summary: Dict[str, Any]):
        ...
        print(f"📊 レベル成功率: {level_stats['level_success_rate']:.1f}% ({level_stats['levels_passed']}/{exec_info['levels_executed']})")
        
        print("\n📋 レベル別詳細:")
        for result in summary["detailed_results"]:
            level_name = self.test_levels[TestLevel(result['level'])]['name']
            result_emoji = "✅" if result['result'] == 'passed' else ("❌" if result['result'] == 'failed' else "⚠️")
