...
class QCCompatibleExtractor:
    ...
    def extract_with_sam_predictor(
        self, image_rgb: np.ndarray, box: list
    ...
    def process_single_image(
        self, image_path: str, output_path: str, conf_threshold: float = 0.07
    ) -> bool:
        """
        単一画像処理（QC成功版完全互換）
    ...
    def process_batch(
        self, input_dir: str, output_dir: str, conf_threshold: float = 0.07, max_files: int = None
    ) -> dict:
        """
        バッチ処理
        ...
        # 処理実行
        stats = {"total": len(image_files), "success": 0, "failed": 0, "sizes": []}
        ...
        for i, img_file in enumerate(image_files, 1):
            ...
            # 処理実行
            success = self.process_single_image(str(img_file), str(output_file), conf_threshold)
        ...
        print("📊 処理完了統計")
        print(
            f"   成功: {stats['success']}/{stats['total']} ({stats['success']/stats['total']*100:.1f}%)"
        )
        ...
        if stats["sizes"]:
            ...
            print(f"   サイズ範囲:")
            print(
                f"     幅: {min(s[0] for s in stats['sizes'])} - {max(s[0] for s in stats['sizes'])}"
            )
            print(
                f"     高さ: {min(s[1] for s in stats['sizes'])} - {max(s[1] for s in stats['sizes'])}"
            )
...
def main():
    ...
    extractor = QCCompatibleExtractor(
        sam_checkpoint=args.sam_checkpoint, yolo_model_path=args.yolo_model
    )
    ...
    if input_path.is_file():
        ...
        success = extractor.process_single_image(str(input_path), str(output_file), args.conf)
        sys.exit(0 if success else 1)
    else:
        # バッチ処理
        stats = extractor.process_batch(str(input_path), args.output, args.conf, args.max_files)
