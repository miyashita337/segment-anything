...
try:
    ...
    from tools.utils.tracker_id_validator import TrackerIdValidationError, TrackerIdValidator
    from tools.utils.workflow_validator import WorkflowValidationError, WorkflowValidator
...
class TestBasicWorkflow:
    ...
    def test_integrated_workflow_validation_invalid_tracker(self):
        ...
        with tempfile.TemporaryDirectory() as temp_dir:
            ...
            with pytest.raises(WorkflowValidationError):
                WorkflowValidator.validate_workflow_inputs("INVALID-ID", input_dir)

    def test_integrated_workflow_validation_invalid_input(self):
        """統合ワークフロー検証（無効入力パス）テスト"""
        with pytest.raises(WorkflowValidationError):
            WorkflowValidator.validate_workflow_inputs("TEST-001", "/nonexistent/input")
