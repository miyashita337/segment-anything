...
class TestApprovalWorkflow:
    ...
    def test_pushover_basic_notification(self):
        ...
        result = self.pushover_client.send_notification(
            message="テスト通知メッセージ", title="テストタイトル", priority=1
        )
    ...
    def test_pushover_quality_alert_notification(self):
        ...
        result = self.pushover_client.send_quality_alert(
            tracker_id="TEST-003", quality_score=0.45, threshold=0.60, failed_images=failed_images
        )
    ...
    def test_pushover_failure_mode(self):
        ...
        result = self.pushover_client.send_notification(message="失敗テストメッセージ", title="失敗テスト")
    ...
    def test_approval_request_approval(self):
        ...
        success = self.approval_system.approve_request(
            request.request_id, approver="test_user@example.com", comment="品質基準を満たしています。承認します。"
        )
    ...
    def test_approval_stage_requirement_check(self):
        ...
        # 各ステージの承認要否確認
        assert (
            self.approval_system.check_stage_approval_required(
                "TEST-010", ApprovalStage.EXTRACTION_START
            )
            is False
        )
        ...
        assert (
            self.approval_system.check_stage_approval_required(
                "TEST-010", ApprovalStage.QUALITY_CHECK
            )
            is True
        )
        ...
        assert (
            self.approval_system.check_stage_approval_required(
                "TEST-010", ApprovalStage.STATISTICAL_ANALYSIS
            )
            is True
        )
        ...
        assert (
            self.approval_system.check_stage_approval_required(
                "TEST-010", ApprovalStage.FINAL_APPROVAL
            )
            is True
        )

        assert (
            self.approval_system.check_stage_approval_required("TEST-010", ApprovalStage.DEPLOYMENT)
            is True
        )
    ...
    def test_progress_task_lifecycle(self):
        ...
        # タスク作成
        task = self.progress_manager.create_task(
            tracker_id="TEST-015", title="品質評価タスク", description="抽出結果の品質評価"
        )
    ...
    def test_progress_task_dependencies(self):
        ...
        base_task = self.progress_manager.create_task(
            tracker_id="TEST-016", title="基礎タスク", description="依存元タスク"
        )
    ...
    def test_progress_task_failure_and_cancellation(self):
        ...
        task1 = self.progress_manager.create_task(
            tracker_id="TEST-017", title="失敗テストタスク", description="失敗をテストするタスク"
        )
        ...
        task2 = self.progress_manager.create_task(
            tracker_id="TEST-018", title="キャンセルテストタスク", description="キャンセルをテストするタスク"
        )
    ...
    def test_integrated_quality_alert_workflow(self):
        ...
        # 1. 品質問題のあるタスク作成
        task = self.progress_manager.create_task(
            tracker_id=tracker_id, title="低品質抽出処理", description="品質問題のある抽出処理"
        )
        ...
        self.pushover_client.send_quality_alert(
            tracker_id=tracker_id, quality_score=0.35, threshold=0.60, failed_images=failed_images
        )
        ...
        # 4. タスク失敗処理
        self.progress_manager.fail_task(task.task_id, f"品質基準未達成: スコア 0.35 < 閾値 0.60")
