...
class TestSubAgentCommandHandler(WorkflowTestBase):
    ...
    def setUp(self):
        ...
        # モックの設定
        self.add_mock(
            "tools.workflow.subagent_command_handler.get_state_manager",
            return_value=self.mock_state_manager,
        )
        self.add_mock(
            "tools.workflow.subagent_command_handler.get_subagent_monitor",
            return_value=self.mock_subagent_monitor,
        )
        self.add_mock(
            "tools.workflow.subagent_command_handler.get_lock_manager",
            return_value=self.mock_lock_manager,
        )
        self.add_mock(
            "tools.workflow.subagent_command_handler.get_workspace_config",
    ...
    def test_handle_subagent_extraction_success(self):
        ...
        # ワークスペース設定が存在
        self.mock_workspace_config.get_workspace_config.return_value = (
            MockData.MOCK_WORKSPACE_CONFIG[SAMPLE_TRACKER_ID]
        )
        ...
        # メソッドが呼ばれたことを確認
        self.mock_lock_manager.is_duplicate_execution_risk.assert_called_once_with(
            SAMPLE_TRACKER_ID, "extraction"
        )
    ...
    def test_handle_subagent_terminate_success(self):
        ...
        output = CLITestHelper.capture_output(
            handler.handle_subagent_terminate, SAMPLE_TRACKER_ID, False
        )
        ...
        self.mock_subagent_monitor.terminate_subagent.assert_called_once_with(
            SAMPLE_TRACKER_ID, force=False
        )
    ...
    def test_handle_subagent_terminate_force(self):
        ...
        output = CLITestHelper.capture_output(
            handler.handle_subagent_terminate, SAMPLE_TRACKER_ID, True
        )
    ...
    def test_handle_subagent_locks_status_empty(self):
        ...
        # ロックなし
        lock_info = {"active_locks": [], "total_count": 0}
    ...
    def test_handle_subagent_auto_retry_check(self):
        ...
        output = CLITestHelper.capture_output(
            handler.handle_subagent_auto_retry_check, SAMPLE_TRACKER_ID
        )
    ...
    def test_handle_subagent_auto_retry(self):
        ...
        # 自動再実行成功
        retry_result = {"success": True, "message": "自動再実行を開始しました", "new_process_id": "SUB-001"}
