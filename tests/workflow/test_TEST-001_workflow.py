...
class TestTEST001Workflow(unittest.TestCase):
    ...
    def test_extraction_results(self):
        ...
        # 26枚の成功を確認（ログから）
        self.assertEqual(
            len(extracted_files), 26,
            f"期待される抽出数26枚に対し、実際は{len(extracted_files)}枚"
        )
    ...
    def test_extraction_quality(self):
        ...
        self.assertGreaterEqual(
            quality_ratio, 0.8,
            f"高品質ファイル比率 {quality_ratio:.1%} が基準80%未満"
        )
    ...
    def test_missing_files(self):
        ...
        self.assertEqual(
            missing, {2, 27},  # ログから2と27が失敗
            f"予期しない欠損ファイル: {missing}"
        )
    ...
    def test_checkpoint_system(self):
        ...
        if checkpoint_file.exists():
            with open(checkpoint_file, 'r') as f:
                ...
                # 処理済み26件確認
                self.assertEqual(
                    len(checkpoint_data.get("processed", [])), 26,
                    "チェックポイントの処理済み数が不正"
                )
