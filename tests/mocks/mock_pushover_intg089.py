...
class MockPushoverClient:
    ...
    def _record_message(self, title: str, message: str, priority: int, 
    ...
    def get_statistics(self) -> Dict[str, Any]:
        ...
        return {
            ...
            "first_message_time": self.sent_messages[0]["timestamp"] if self.sent_messages else None,
            "last_message_time": self.sent_messages[-1]["timestamp"] if self.sent_messages else None
        }
...
class MockNotificationBridge:
    ...
    def send_enhanced_notification(self, title: str, message: str, 
        ...
        # 優先度設定取得
        priority_config = self.notification_priorities.get(priority_level, 
                                                          self.notification_priorities['normal'])
    ...
    def get_notification_statistics(self) -> Dict[str, Any]:
        ...
        total_notifications = sum(history['count'] for history in self.notification_history.values())
        
        return {
            "pushover_stats": pushover_stats,
            "unique_notifications": unique_notifications,
            "total_notifications": total_notifications,
            "duplicate_prevention_rate": (total_notifications - pushover_stats['total_calls']) / total_notifications if total_notifications > 0 else 0.0
        }
...
def get_notification_count_by_priority(mock_client: MockPushoverClient, priority: int) -> int:
    """指定優先度の通知数を取得"""
    return sum(1 for msg in mock_client.get_sent_messages() 
