...
class MockApprovalSystem:
    ...
    def get_requests_by_tracker(self, tracker_id: str) -> List[ApprovalRequest]:
        """トラッカーID別承認依頼取得"""
        return [req for req in self.approval_requests.values() 
                if req.tracker_id == tracker_id]
    ...
    def get_approval_statistics(self) -> Dict[str, Any]:
        ...
        for status in ApprovalStatus:
            count = sum(1 for req in self.approval_requests.values() 
                       if req.status == status)
        ...
        # 処理時間統計
        processed_requests = [req for req in self.approval_requests.values() 
                             if req.approved_at]
    ...
    def _auto_process_approval(self, request_id: str, approve: bool):
        ...
        if approve:
            self.approve_request(
                ...
                comment="Automatically approved by mock system"
            )
        else:
            self.reject_request(
                request_id,
                rejector="auto_system", 
                reason="Automatically rejected by mock system"
            )
