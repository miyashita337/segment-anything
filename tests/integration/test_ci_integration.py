...
class TestModelWrapperIntegration:
    ...
    @pytest.mark.skipif(not Path("features/extraction/models").exists(), 
                       reason="Model wrapper modules not found")
...
class TestCIStatisticalValidation:
    ...
    def test_wilson_confidence_interval_validation(self):
        ...
        # テストケース2: 425/424矛盾検出
        with pytest.raises(ValueError, match="Success count \\(425\\) cannot exceed total count \\(424\\)"):
