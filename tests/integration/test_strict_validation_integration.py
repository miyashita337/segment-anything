...
class TestStrictValidationConfigIntegration:
    ...
    def test_pipeline_config_defaults_disabled(self):
        ...
        if config_path.exists():
            ...
            assert (
                "# input_alternatives:" in config_content
                or "input_alternatives:" not in config_content
            )
...
class TestStrictValidationEndToEndIntegration:
    ...
    def test_strict_validation_author_structure_detection(self):
        ...
        validator = StrictPathValidator(
            strict_mode=True, require_author_structure=True, interactive_mode=False
        )
    ...
    def test_strict_validation_security_constraints(self):
        ...
        security_errors = [issue for issue in result.issues if "システムディレクトリ" in issue.message]
...
class TestStrictValidationRegressionTests:
    ...
    def test_backward_compatibility_non_strict_mode(self):
        """非厳密モードでの後方互換性テスト"""
        validator = StrictPathValidator(
            strict_mode=False, require_author_structure=False, interactive_mode=False
        )
    ...
    def test_existing_validation_system_integration(self):
        """既存検証システムとの統合確認"""
        from features.common.input_validation import InputValidationError, validate_input_directory
