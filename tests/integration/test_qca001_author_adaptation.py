...
class TestQCA001AuthorAdaptationIntegration:
    ...
    def test_sam_optimization_config_integration(self):
        ...
        for author, expected_sam_profile in test_cases:
            ...
            assert (
                sam_config["points_per_side"] == expected_profile.points_per_side
            ), f"{author}: points_per_side不一致"
            assert (
                sam_config["pred_iou_thresh"] == expected_profile.pred_iou_thresh
            ), f"{author}: pred_iou_thresh不一致"
            assert (
                sam_config["stability_score_thresh"] == expected_profile.stability_score_thresh
    ...
    def test_extract_character_integration(self, mock_adapter_class):
        ...
        try:
            ...
            with tempfile.NamedTemporaryFile(
                suffix=".jpg"
            ) as input_file, tempfile.TemporaryDirectory() as output_dir:
                # ダミー画像ファイル作成
                ...
                result = runner.invoke(
                    extract_character,
                    [input_file.name, "-o", output_dir, "--enable-author-adaptation", "--verbose"],
                )
    ...
    def test_author_parameter_yolo_confidence_application(self):
        ...
        for author, expected_confidence in test_cases:
            ...
            # スコア閾値も同じ値であることを確認
            assert params["score_threshold"] == expected_confidence, f"{author}作者のスコア閾値不一致"
    ...
    def test_sam_profile_consistency(self):
        ...
        for author in ["yado", "aichi", "zundamon"]:
            ...
            assert (
                expected_sam_profile in optimizer.OPTIMIZATION_PROFILES
            ...
            assert (
                0 < profile.stability_score_thresh < 1
            ), f"{expected_sam_profile}: stability_score_thresh無効"
    ...
    def test_performance_characteristics_mapping(self):
        ...
        for author, (expected_char, expected_speedup) in expected_characteristics.items():
            ...
            # 特性確認
            assert (
                params["characteristics"] == expected_char
            ...
            assert (
                abs(profile.expected_speedup - expected_speedup) < 0.1
...
def test_qca001_end_to_end_simulation():
    ...
    print(
        f"📊 信頼度範囲: {stats['confidence_range']['min']:.3f} - {stats['confidence_range']['max']:.3f}"
    )
