...
class TestScreentoneDetectorIntegration(unittest.TestCase):
    ...
    def test_enhanced_vs_legacy_comparison(self):
        ...
        self.assertTrue(
            any(r["detection_improvement"] > 0 for r in comparison_results),
    ...
    def test_real_world_scenarios(self):
        ...
        scenarios = [
            ...
            {"name": "fine_lines", "creator": lambda: self._create_fine_line_pattern(128, 128)},
            {
    ...
    def test_performance_comparison(self):
        ...
        self.assertLess(
            enhanced_avg / legacy_avg, 10.0, "Enhanced detector should not be more than 10x slower"
        )
    ...
    def test_edge_cases(self):
        ...
        for name, test_image in edge_cases:
            try:
                ...
                print(
                    f"{name}: detected={result.has_screentone}, "
    ...
    def test_mask_quality_validation(self):
        ...
        self.assertTrue(np.all((mask == 0) | (mask == 255)), "Mask should be binary (0 or 255)")
        ...
        if len(contours) > 0:
            ...
            if total_mask_area > 0:
                ...
                self.assertGreater(
                    largest_ratio, 0.1, "Largest connected component should be significant"
                )

                print(
                    f"Mask quality: largest component ratio={largest_ratio:.3f}, "
    ...
    def test_robustness_to_noise(self):
        ...
        for noise_level in noise_levels:
            if noise_level == 0:
                ...
                noisy_image = np.clip(clean_image.astype(np.float32) + noise, 0, 255).astype(
                    np.uint8
                )
        ...
        for result in results:
            print(
                f"Noise level {result['noise_level']:2d}: "
                f"detected={result['detected']}, "
                f"conf={result['confidence']:.3f}, "
                f"quality={result['quality']:.3f}"
            )
        ...
        self.assertLess(
            confidence_drop, 0.5, "Confidence should not drop drastically with moderate noise"
        )
