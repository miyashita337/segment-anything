...
class TestQI004SystemIntegration(unittest.TestCase):
    ...
    def test_complete_qi004_optimization_workflow(self):
        ...
        result = system.run_complete_optimization(
            self.tracker_id, self.extraction_dir, os.path.join(self.temp_workspace, self.tracker_id)
        )
    ...
    def test_dashboard_generation_with_real_images(self):
        ...
        # ダッシュボード生成
        dashboard_path = generator.generate_standard_dashboard(dashboard_data, self.dashboard_dir)
    ...
    def test_qi004_entry_point_integration(self):
        ...
        success = create_qi004_optimized_dashboard(
            f"{self.tracker_id}_ENTRY", self.extraction_dir, output_dir
        )
    ...
    def test_quality_score_distribution_accuracy(self):
        ...
        # （テスト画像の品質設定に基づく）
        total_categorized = (
            len(high_quality_scores)
            + len(medium_quality_scores)
            + len(low_quality_scores)
            + len(poor_quality_scores)
        )
    ...
    def test_error_handling_integration(self):
        ...
        # 存在しない抽出ディレクトリでの実行
        result = system.run_complete_optimization(
            "QI-004-ERROR", "/nonexistent/directory", self.temp_workspace
        )
    ...
    def test_large_dataset_handling(self):
        ...
        result = system.run_complete_optimization(
            "QI-004-LARGE", large_dataset_dir, os.path.join(self.temp_workspace, "QI-004-LARGE")
        )
