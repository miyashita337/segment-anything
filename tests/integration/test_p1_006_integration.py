...
class TestP1006Integration(unittest.TestCase):
    ...
    def test_evaluation_function_integration(self):
        ...
        with tempfile.TemporaryDirectory() as temp_dir:
            ...
            # 評価関数実行
            results = evaluate_solid_fill_processing(str(temp_image_path), save_results=True)
    ...
    def test_solid_fill_types_coverage(self):
        ...
        for result in detection_results:
            print(
                f"{result['pattern']}: 検出={result['detected']}, "
    ...
    def test_performance_with_different_sizes(self):
        ...
        for width, height in test_sizes:
            ...
            self.assertLess(
                processing_time, max_time, f"処理時間が期待値を超過: {processing_time:.3f}s > {max_time:.3f}s"
            )
        ...
        for result in performance_results:
            print(
                f"サイズ: {result['size']}, 時間: {result['time']:.3f}s, "
                f"領域数: {result['regions_found']}, "
                f"処理速度: {result['pixels_per_second']:.0f}ピクセル/秒"
            )

    def _create_solid_background(self, width: int, height: int, color) -> np.ndarray:
        """単色背景画像作成"""
    ...
    def _create_black_hair_pattern(self, width: int, height: int) -> np.ndarray:
        ...
        # 黒い縦長の楕円（髪風）
        cv2.ellipse(
            image, (width // 2, height // 3), (width // 6, height // 2), 0, 0, 360, (20, 20, 20), -1
        )
    ...
    def _create_clothing_pattern(self, width: int, height: int) -> np.ndarray:
        ...
        # 青い服風の矩形
        cv2.rectangle(
            image, (width // 4, height // 2), (3 * width // 4, height - 10), (100, 50, 200), -1
        )
    ...
    def _create_mixed_solid_pattern(self, width: int, height: int) -> np.ndarray:
        ...
        cv2.rectangle(
            image, (width // 4, 2 * height // 3), (3 * width // 4, height - 10), (0, 0, 0), -1
        )  # 黒
