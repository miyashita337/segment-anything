...
from features.common.dashboard_generator import DashboardGenerator, create_standard_dashboard
...
class TestQccFix001Integration(unittest.TestCase):
    ...
    def test_unified_success_rate_calculation(self):
        ...
        self.assertLessEqual(
            stats.successful_extractions, stats.total_input_images, "成功数 ≤ 入力数の数学的整合性"
        )
        ...
        self.assertAlmostEqual(stats.success_rate_percent, expected_rate, places=2, msg="成功率計算正確性")
    ...
    def test_mathematical_consistency_violation_detection(self):
        ...
        # 数学的整合性が自動修正されることを確認
        self.assertLessEqual(stats.successful_extractions, stats.total_input_images, "数学的矛盾の自動修正")
        # 実際の処理では、追加ファイルは同一ディレクトリに配置されるため
        # 実際にカウントされるのは元の13枚 + 追加20枚 = 33枚だが
        # 入力数15枚の制限により15枚に制限される
        # ただし、数学的整合性チェックが正しく動作していることが重要
        self.assertEqual(stats.successful_extractions, min(33, 15), "成功数上限を入力数で制限（数学的整合性の自動修正確認）")
    ...
    def test_qcc021_specific_correction(self):
        ...
        self.assertAlmostEqual(stats.success_rate_percent, 100.0, places=1, msg="QCC-021修正後100%成功率")
