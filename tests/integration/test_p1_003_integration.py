...
class TestEnhancedFullBodyIntegration(unittest.TestCase):
        ...
        for criteria, weights in weight_configs.items():
            with self.subTest(criteria=criteria):
                ...
                self.assertAlmostEqual(
                    total_weight, 1.0, places=2, msg=f"Weights for {criteria} should sum to 1.0"
                )

                if "fullbody_priority" in criteria:
                    self.assertGreater(
                        weights["fullbody"], 0.35, f"Fullbody weight in {criteria} should be > 0.35"
                    )

                print(
                    f"✅ {criteria}: weights sum to {total_weight:.2f}, "
    ...
    def test_error_handling_integration(self):
        """エラーハンドリング統合テスト"""
        # 不正なマスクデータでのテスト
        invalid_mask_data = {"bbox": [0, 0, 0, 0], "area": 0, "yolo_confidence": 0.0}  # 無効なbbox
