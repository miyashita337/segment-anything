...
"""
        ...
        with patch('subprocess.run') as mock_run:
            ...
            # JSON内容の検証
            with open(self.output_path / "stash_analysis_detailed.json", 'r', encoding='utf-8') as f:
                detailed_data = json.load(f)
                
                # 重要ファイルの詳細確認
                extract_char_detail = detailed_data['features/extraction/commands/extract_character.py']
    ...
    def test_large_scale_analysis_performance(self):
        ...
        # 100ファイルの大規模変更を模擬
        large_numstat = "\n".join([
            f"{i%20+1}\t{i%15+1}\tfile_{i:03d}.py" for i in range(100)
        ])
