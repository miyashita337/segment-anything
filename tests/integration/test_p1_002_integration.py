...
class TestPartialDetectionIntegration(unittest.TestCase):
    ...
    def test_extraction_with_partial_analysis(self):
        ...
        try:
            ...
            result = extract_character(
                input_path=str(self.test_image_path), output_path=None, verbose=True, batch=False
            )
            ...
            if "extraction_analysis" in result:
                ...
                print(
                    f"   Structure: face={analysis['has_face']}, "
                ...
                if "issues" in analysis and analysis["issues"]:
                    ...
                    for issue in analysis["issues"][:3]:  # 最大3件表示
                        print(
                            f"     - {issue['type']} ({issue['severity']}): {issue['description'][:50]}..."
                        )
    ...
    def test_error_handling_integration(self):
        ...
        try:
            result = extract_character(
                input_path="/nonexistent/image.jpg", output_path=None, verbose=False
            )
    ...
    def test_performance_monitoring_integration(self):
        ...
        try:
            result = extract_character(
                input_path=str(self.test_image_path), output_path=None, verbose=True
            )
