...
class TestQCC022Integration(unittest.TestCase):
    ...
    def setUp(self):
        ...
        self.analyzer = StatisticalQualityAnalyzer(workspace_base=str(self.workspace_base))
    ...
    def test_end_to_end_comparison(self):
        ...
        result = self.analyzer.compare_trackers(
            "TEST-IMPROVED", "TEST-BASELINE", metric="quality_score"
        )
    ...
    def test_improvement_analysis(self):
        """改善効果分析のテスト"""
        analysis = self.analyzer.analyze_improvement("TEST-BASELINE", "TEST-IMPROVED")
    ...
    def test_report_generation(self):
        ...
        result = self.validator.welch_t_test(
            baseline_metrics.quality_scores, improved_metrics.quality_scores
        )

        # JSONレポート生成
        json_report = self.reporter.generate_json_report(
            result, test_name="Integration Test", metadata={"test": True}
        )
        ...
        # JSONファイル保存
        json_path = self.reporter.save_json_report(result, filename="test_report.json")

        self.assertTrue(json_path.exists())

        # HTMLレポート生成
        html_content = self.reporter.generate_html_report(
            result, test_name="統合テスト", group_a_name="ベースライン", group_b_name="改善版"
        )

        self.assertIn("<!DOCTYPE html>", html_content)
        self.assertIn("統合テスト", html_content)
        # p値は非常に小さい場合 "< 0.0001" と表示される
        self.assertTrue(str(result.p_value)[:6] in html_content or "< 0.0001" in html_content)

        # HTMLファイル保存
        html_path = self.reporter.save_html_report(result, filename="test_report.html")
    ...
    def test_batch_comparison(self):
        ...
        summary = self.analyzer.batch_compare(
            ["TEST-BASELINE", "TEST-IMPROVED", "TEST-MIDDLE"], baseline="TEST-BASELINE"
        )
    ...
    def test_statistical_dashboard_generation(self):
        ...
        dashboard_path = self.analyzer.generate_statistical_dashboard(
            "TEST-IMPROVED", comparison_trackers=["TEST-BASELINE"], output_dir=self.temp_dir
        )
    ...
    def test_multiple_comparison_correction(self):
        ...
        for comp_name, comp_data in report["comparisons"].items():
            ...
            # 補正後のp値は元の値以上
            self.assertGreaterEqual(comp_data["corrected_p_value"], comp_data["original_p_value"])
    ...
    def test_real_world_scenario(self):
        ...
        for i in range(len(experiments) - 1):
            result = self.validator.welch_t_test(experiments[i], experiments[i + 1])
        ...
        # 3. 最終レポート生成
        final_result = self.validator.welch_t_test(experiments[0], experiments[-1])  # 初期状態  # 最終状態
