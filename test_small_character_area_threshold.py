...
def test_adaptive_area_filtering():
    ...
    for test_case in test_masks:
        ...
        adaptive_filtered = remove_small_components_adaptive(
            mask, min_area_ratio=0.001, absolute_min_area=50  # 0.1%
        )
        ...
        print(
            f"  従来フィルタ後: {traditional_area:,} ピクセル " f"(保持率: {traditional_area/original_area:.1%})"
        )
        print(f"  適応フィルタ後: {adaptive_area:,} ピクセル " f"(保持率: {adaptive_area/original_area:.1%})")
...
def generate_test_report():
    ...
    for test_case in test_masks:
        ...
        test_result = {
            ...
            "default_pass": bool(
                quality_metrics["coverage_ratio"] >= checker.thresholds["coverage_ratio"]
            ),
            "adaptive_pass": bool(quality_metrics["coverage_ratio"] >= adaptive_threshold),
            "improvement": bool(
                not (quality_metrics["coverage_ratio"] >= checker.thresholds["coverage_ratio"])
