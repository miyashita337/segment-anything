...
def debug_pattern_classification():
    ...
    for name, test_image in [("Line Pattern", line_image), ("Dot Pattern", dot_image)]:
        print(f"\n{name} Analysis:")
        print(
            f"Image stats: min={np.min(test_image)}, max={np.max(test_image)}, mean={np.mean(test_image):.1f}"
        )
        ...
        if name == "Line Pattern":
            ...
            print(
                f"  wavelet_directionality: {combined_features.get('wavelet_directionality', 0):.4f}"
            )
            print(
                f"  orientation_uniformity: {combined_features.get('orientation_uniformity', 0):.4f}"
            )
