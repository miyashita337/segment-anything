...
class RegionMatcher:
    ...
    def create_cost_matrix(
        self, iou_matrix: np.ndarray, similarity_matrix: np.ndarray, alpha: Optional[float] = None
    ) -> np.ndarray:
        """コスト行列の作成"""
        if alpha is None:
    ...
    def match(
        ...
        scores = (
            alpha * iou_matrix[row_indices, col_indices]
    ...
    def filter_matches(
        ...
        """閾値によるマッチングフィルタリング"""
        valid_mask = (iou_matrix[row_indices, col_indices] >= self.config.iou_threshold) & (
...
class MultiCharacterMatcher(RegionMatcher):
    ...
    def calculate_primary_score(
        self, bbox: Tuple[int, int, int, int], confidence: float, image_shape: Tuple[int, int]
    ) -> float:
        """主キャラクタースコアの計算"""
        x, y, w, h = bbox
