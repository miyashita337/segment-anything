...
class EvaluationOrchestrator:
    ...
    def extract_crops_from_image(
        self, image_path: Union[str, Path], bboxes: List[Tuple[int, int, int, int]]
    ) -> List[np.ndarray]:
        """画像から境界ボックス領域を切り出し"""
    ...
    def run_single_image(
        ...
        try:
            ...
            integrated_score = (
                ...
                + (1 - self.config.alpha) * np.mean(similarity_matrix)
    ...
    def run_batch(
        self, predictions_batch: List[Dict[str, Any]], ground_truths_batch: List[Dict[str, Any]]
    ) -> Dict[str, Any]:
        """バッチ評価の実行"""
        start_time = time.time()
        ...
        logger.info(
            f"Batch evaluation completed: {success_rate:.1%} success rate, "
