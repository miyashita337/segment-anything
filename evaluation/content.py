...
class ContentEvaluator(EvaluatorBase):
    ...
    def batch_encode(self, image_tensors: list) -> torch.Tensor:
        ...
        if len(image_tensors) == 0:
            return torch.empty(
                0, 512 if not self.config.use_pca_dimension else self.config.use_pca_dimension
            )

        batch_tensor = torch.cat(image_tensors, dim=0)
        return self.encode(batch_tensor)

    def evaluate_crop_similarity(self, pred_crop: np.ndarray, gt_crop: np.ndarray) -> float:
        """クロップ画像間の類似度評価"""
        try:
            # 前処理
    ...
    def evaluate(
        self, prediction: Union[np.ndarray, Tuple], ground_truth: Union[np.ndarray, Tuple]
    ) -> EvaluationResult:
        """内容評価の実行"""
