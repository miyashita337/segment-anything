...
def main():
    ...
    print("🔍 QI-002 vs QCA-001 統計分析開始")
    
    # データ読み込み
    with open('/mnt/c/AItools/lora/train/yado/tracker-workspace/QI-002/extraction_result.json', 'r') as f:
        qi002_data = json.load(f)
    
    with open('/mnt/c/AItools/lora/train/yado/tracker-workspace/QCA-001/extraction_result.json', 'r') as f:
        qca001_data = json.load(f)
    
    print(f"📈 Current(QI-002): {qi002_data['total_images']}枚、平均品質スコア: {qi002_data['mean_quality_score']:.6f}")
    print(f"📈 Baseline(QCA-001): {qca001_data['total_images']}枚、平均品質スコア: {qca001_data['mean_quality_score']:.6f}")
    
    # 統計分析実行
    stats_result = perform_statistical_analysis(qi002_data, qca001_data)
    ...
    analysis_result = {
        ...
        'current_tracker': 'QI-002',
        'baseline_tracker': 'QCA-001',
        'current_metrics': {
    ...
    # JSON保存
    output_file = '/mnt/c/AItools/lora/train/yado/tracker-workspace/QI-002/statistical_analysis_result.json'
    with open(output_file, 'w', encoding='utf-8') as f:
        json.dump(analysis_result, f, indent=2, ensure_ascii=False, default=str)
    
    print(f"📄 分析結果保存: {output_file}")
    print("✅ QI-002 vs QCA-001 統計分析完了")
