...
class AutoProgressSystem:
            ...
            for result in panel_results:
                if result.success and result.largest_panel:
                    ...
                    claude_score = (
                        ...
                        + min(panel.area / 50000, 1.0) * 0.3  # 信頼度40%
                        + (1.0 / max(result.processing_time, 0.1)) * 0.3  # サイズ正規化30%  # 速度30%
                    )
    ...
    def send_test_batch_notification(self, phase: str, summary):
        ...
        try:
            ...
            subprocess.run(["windows-notify", "-t", "Claude Code", "-m", message], check=False)
    ...
    def run_phase1_data_expansion(self) -> bool:
        ...
        try:
            ...
            report_file = (
                expansion_system.output_dir
                / f"expansion_report_{time.strftime('%Y%m%d_%H%M%S')}.md"
            )
    ...
    def run_phase1_panel_detection(self) -> bool:
        ...
        try:
            ...
            if success_count > 0:
                self.log_execution(
                    "phase1", "コマ検出ネット実行", "成功", f"{success_count}/{len(results)}件成功処理"
                )
