...
class ParallelTrackerRunner:
    ...
    def _run_command(self, command: List[str], task_id: str, timeout: int = 300) -> Tuple[bool, str, str]:
        """コマンドを実行"""
    ...
    def run_extraction_pipeline(self) -> bool:
        ...
        # バッチ分割抽出システムを使用
        command = [
            "python3", "tools/automation/batched_extraction_runner.py",
            self.tracker_id,
    ...
    def generate_dashboard(self) -> bool:
        ...
        dashboard_command = [
            ...
            "--results", str(self.workspace_dir / "extraction"),
            "--output", str(self.workspace_dir / "dashboard" / "dashboard.html")
        ]
        
        dashboard_success, _, _ = self._run_command(dashboard_command, f"{task_id}_html", timeout=60)
    ...
    def run_parallel_workflow(self, skip_extraction: bool = False) -> bool:
        ...
        try:
            ...
            if not skip_extraction:
                ...
                print(f"   {'✅' if extraction_result else '❌'} 抽出パイプライン: {'成功' if extraction_result else '失敗'}")
            else:
            ...
            print(f"   {'✅' if dashboard_result else '❌'} ダッシュボード: {'成功' if dashboard_result else '失敗'}")
            print(f"   {'✅' if report_result else '❌'} 最終レポート: {'成功' if report_result else '失敗'}")
            
            # 結果判定
            critical_success = light_results.get('workspace', False)
            overall_success = critical_success and (light_results.get('tests', False) or light_results.get('quality', False))
