...
from tools.progress_tracker.data_models import ProgressTrackerConfig, TaskStatus
...
class SheetsPollingMonitor:
    ...
    async def fetch_current_status(self) -> Tuple[Optional[Dict[str, Any]], bool]:
        ...
        try:
            ...
            change_detected = (
                ...
                or self.last_details != current_details
            )
    ...
    def display_monitoring_summary(self):
        ...
        print(
            f"   平均間隔: {elapsed.total_seconds() / self.poll_count:.1f}秒"
    ...
    async def monitor_extraction_progress(self) -> bool:
        ...
        monitoring_end_time = self.monitoring_start_time + timedelta(
            hours=self.max_monitoring_hours
        )
            ...
            try:
                ...
                if changed:
                    ...
                    if status == TaskStatus.RELEASE.value:
                        ...
                        and task_info.get("extraction_pipeline") == "completed"
                    ):
