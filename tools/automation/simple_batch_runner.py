...
from tools.progress_tracker.data_models import ComponentStatus, TaskStatus
...
class SimpleBatchRunner:
    ...
    def __init__(
        ...
        use_pushover: bool = True,
        pushover_interval_minutes: int = 30,
    ):
    ...
    def calculate_estimated_completion(self) -> Dict[str, Any]:
        ...
        remaining_images = (
            ...
            - self.stats["processed_images"]
            - self.stats["failed_images"]
        )
        remaining_batches = (
            remaining_images + self.max_images_per_batch - 1
    ...
    def send_progress_notification(self, force: bool = False, is_milestone: bool = False):
        ...
        # 進捗率計算
        progress_rate = (
            (self.stats["processed_images"] / self.stats["total_images"] * 100)
        ...
        if self.stats["processed_images"] > 0:
            success_rate = (
                (self.stats["processed_images"] - self.stats["failed_images"])
                / self.stats["processed_images"]
                * 100
            )
            message_lines.append(
                f"📈 成功率: {success_rate:.1f}% ({self.stats['processed_images'] - self.stats['failed_images']}/{self.stats['processed_images']})"
            )
    ...
    def run(self, input_dir: Path) -> bool:
        ...
        for i in range(0, len(image_paths), self.max_images_per_batch):
            ...
            total_batches = (
                len(image_paths) + self.max_images_per_batch - 1
            ) // self.max_images_per_batch
            ...
            logger.info(
                f"進捗: {progress:.1f}% ({processed_total + failed_total}/{len(image_paths)})"
            )
    ...
    def generate_report(self, output_dir: Path) -> Dict[str, Any]:
        ...
        report = {
            ...
            "fps": (self.stats["total_images"] / duration * 3600) if duration > 0 else 0,  # 枚/時間
        }
    ...
    def _update_sheets_failure(self, error_message: str):
        ...
        try:
            ...
            self.progress_manager.update_details(
                self.tracker_id, json.dumps(details, ensure_ascii=False)
            )
    ...
    def _update_sheets_completion(self, report_data: Dict[str, Any], is_success: bool):
        ...
        try:
            ...
            self.progress_manager.update_status(
                self.tracker_id, final_status, component_status, "extraction_pipeline"
            )
    ...
    def _update_sheets_progress(self, current_processed: int, current_failed: int):
        ...
        try:
            progress_rate = (
                ((current_processed + current_failed) / self.stats["total_images"] * 100)
                if self.stats["total_images"] > 0
                else 0
            )
            current_success_rate = (
                (current_processed / (current_processed + current_failed) * 100)
            ...
            self.progress_manager.update_details(
                self.tracker_id, json.dumps(progress_details, ensure_ascii=False)
            )
            logger.info(
                f"Google Sheets進捗更新: {progress_rate:.1f}%完了 ({current_processed}成功/{current_failed}失敗)"
            )
