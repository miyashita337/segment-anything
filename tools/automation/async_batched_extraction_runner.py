    ...
    async def check_gpu_available() -> Tuple[bool, Dict[str, Any]]:
        ...
        try:
            ...
            with ProcessPoolExecutor(max_workers=1) as executor:
                result = await loop.run_in_executor(executor, AsyncGPUMonitor._check_gpu_sync)
            return result
...
class AsyncBatchedExtractionRunner:
    ...
    async def _execute_extraction_command(
        self, batch: AsyncImageBatch, batch_input_dir: Path, batch_output_dir: Path
    ) -> Tuple[int, str, str]:
        """抽出コマンドを非同期実行"""
        command = [
            "python3",
    ...
    async def _process_single_batch(
        self, batch: AsyncImageBatch, input_dir: Path, output_dir: Path
    ) -> bool:
        """単一バッチを非同期処理"""
    ...
    async def _cleanup_existing_temp_dirs(self, output_dir: Path, current_batch_id: int):
        ...
        try:
            ...
            if existing_temp.exists():
                ...
                await self._safe_cleanup_temp_dir(
                    existing_temp, f"existing_batch_{current_batch_id}"
                )
    ...
    async def _safe_cleanup_temp_dir(self, temp_dir: Path, context: str):
        ...
        for attempt in range(max_retries):
            try:
                ...
                logger.info(
                    f"一時ディレクトリクリーンアップ [{context}] (試行 {attempt + 1}/{max_retries}): {temp_dir}"
                )
                ...
                # ディレクトリ削除
                await asyncio.get_event_loop().run_in_executor(None, shutil.rmtree, temp_dir, True)
    ...
    async def _process_individual_images(
        self, failed_batch: AsyncImageBatch, input_dir: Path, output_dir: Path
    ) -> int:
        """失敗したバッチの個別画像を非同期処理"""
            ...
            async with semaphore:
                try:
                    ...
                    stdout, stderr = await asyncio.wait_for(
                        process.communicate(), timeout=self.max_image_timeout
                    )
        ...
        # 全画像を並列処理
        tasks = [
            process_single_image(i, img_path) for i, img_path in enumerate(failed_batch.image_paths)
        ]
    ...
    async def run_async_batched_extraction(self, input_dir: Path, output_dir: Path) -> bool:
        ...
        success_rate = (
            (final_output_count / self.stats["total_images"]) * 100
...
async def main():
    ...
    parser.add_argument(
        "--log_level", default="INFO", choices=["DEBUG", "INFO", "WARNING", "ERROR"], help="ログレベル"
    )
