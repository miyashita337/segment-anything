...
class BatchedExtractionRunner:
    ...
    def _process_single_batch(self, batch: ImageBatch, input_dir: Path, output_dir: Path) -> bool:
        ...
        try:
            ...
            command = [
                ...
                "--output_dir", str(batch_temp_dir / "output"),
            ...
            if result.returncode == 0:
                ...
                # 処理済み画像数カウント
                processed_count = len(list(batch_output_dir.glob("*.jpg"))) + len(list(batch_output_dir.glob("*.png")))
    ...
    def _process_individual_images(self, failed_batch: ImageBatch, input_dir: Path, output_dir: Path) -> int:
        """失敗したバッチの個別画像処理"""
    ...
    def run_batched_extraction(self, input_dir: Path, output_dir: Path, max_workers: int = 2) -> bool:
        """バッチ分割抽出実行"""
        ...
        # 最終結果確認
        final_output_count = len(list(output_dir.glob("*.jpg"))) + len(list(output_dir.glob("*.png")))
