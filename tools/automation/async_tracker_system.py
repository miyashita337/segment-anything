...
logging.basicConfig(
    ...
    handlers=[
        logging.FileHandler('async_tracker_system.log'),
...
class TrackerWorkflowManager:
    ...
    async def _generate_execution_report(self, tracker_id: str, results: Dict[str, TaskResult]):
        ...
        report_data = {
            ...
            'tasks': {
                task_id: asdict(result) for task_id, result in results.items()
            },
            'summary': {
                ...
                'total_duration': str(sum(
                    (r.duration for r in results.values() if r.duration), 
                    timedelta()
                ))
            }
        }
