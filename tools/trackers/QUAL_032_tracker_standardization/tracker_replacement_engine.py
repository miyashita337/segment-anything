...
class TrackerReplacementEngine:
    ...
    def __init__(self, mapping_file: str = "tools/trackers/QUAL_032_tracker_standardization/tracker_id_mapping.json"):
        ...
        self.target_extensions = {
            '.py', '.md', '.json', '.html', '.txt', 
            '.yaml', '.yml', '.toml', '.sh', '.bat'
        }
        
        # 除外パターン
        self.exclude_patterns = {
            '.git/', '__pycache__/', '.pytest_cache/', 
            'node_modules/', '.venv/', 'sam-env/',
            '.idea/', '.vscode/', 'deprecated/untracked_files/experimental_current/'
        }
    ...
    def get_git_commit(self) -> str:
        ...
        try:
            result = subprocess.run(['git', 'rev-parse', 'HEAD'], 
                                  capture_output=True, text=True, cwd='/mnt/c/AItools/segment-anything')
        ...
        try:
            ...
            for old_id, new_id in self.mapping.items():
                if old_id in content:
                    ...
                    replacements[old_id] = replacements.get(old_id, 0) + content.count(new_id) - original_content.count(old_id)
    ...
    def replace_directory_names(self, directories: List[str], dry_run: bool = True) -> Dict[str, str]:
        """ディレクトリ名置換"""
        renamed = {}
                ...
                for dirname in dirs:
                    for old_id in self.mapping.keys():
                        if old_id in dirname:
                            ...
                            verification["warnings"].append(f"ディレクトリ名に残存: {old_id} in {os.path.join(root, dirname)}")
