...
class ReplacementVerificationChecker:
    ...
    def load_mapping(self):
        """マッピング読み込み"""
        with open(
            "tools/trackers/QUAL_032_tracker_standardization/tracker_id_mapping.json",
            "r",
            encoding="utf-8",
        ) as f:
            data = json.load(f)
                        ...
                        try:
                            ...
                            for old_id in self.old_ids:
                                if old_id in content:
                                    dir_remaining.append(
                                        f"{old_id} in {os.path.relpath(filepath, self.base_dir)}"
                                    )
            ...
            if os.path.exists(filepath):
                try:
                    ...
                    results[filename] = {
                        ...
                        else f"⚠️  旧ID残存: {len(remaining_old)}個",
                    }
    ...
    def test_basic_functionality(self) -> Dict[str, any]:
        ...
        try:
            result = subprocess.run(
                ["git", "status", "--porcelain"], cwd=self.base_dir, capture_output=True, text=True
            )
