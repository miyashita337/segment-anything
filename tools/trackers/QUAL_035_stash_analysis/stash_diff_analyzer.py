...
class StashDiffAnalyzer:
    ...
    def _determine_action(
        self, file_path: str, change_type: ChangeType, added: int, removed: int
    ) -> RecommendedAction:
        """推奨アクションの決定"""
    ...
    def _calculate_confidence(
        self, change_type: ChangeType, file_path: str, diff_content: str
    ) -> float:
        """判定信頼度の計算"""
    ...
    def _generate_reasons(
        self, change_type: ChangeType, file_path: str, added: int, removed: int
    ) -> List[str]:
        """判定理由の生成"""
        reasons = []
    ...
    def generate_summary_report(self, analyses: Dict[str, FileAnalysis]) -> Dict:
        ...
        summary = {
            ...
            "change_types": {ct.value: count for ct, count in change_type_counts.items()},
            "confidence_stats": {
                "high_confidence_count": len(high_confidence),
                "high_confidence_ratio": len(high_confidence) / len(analyses) if analyses else 0,
                "average_confidence": sum(a.confidence for a in analyses.values()) / len(analyses)
                if analyses
                else 0,
            },
    ...
    def save_analysis_results(
        self, analyses: Dict[str, FileAnalysis], output_dir: str = "tools/analysis"
    ):
        """分析結果を保存"""
    ...
    def _generate_text_report(
        self, analyses: Dict[str, FileAnalysis], summary: Dict, output_file: Path
    ):
        """テキストレポート生成"""
        ...
        with open(output_file, "w", encoding="utf-8") as f:
            ...
            f.write(
                f"- **変更行数**: +{summary['total_changes']['added']} / -{summary['total_changes']['removed']}\n"
            )
