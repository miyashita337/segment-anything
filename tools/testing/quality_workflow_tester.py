...
class QualityWorkflowTester:
    ...
    def run_batch_test(self, input_dir: str, output_dir: str, max_files: Optional[int] = None) -> Dict[str, Any]:
        """
        バッチ処理テスト実行
    ...
    def run_quality_method_comparison(self, input_dir: str, output_base_dir: str) -> Dict[str, Any]:
        ...
        # 比較分析
        best_method = max(comparison_results.keys(), key=lambda m: comparison_results[m]["success_rate"])
        worst_method = min(comparison_results.keys(), key=lambda m: comparison_results[m]["success_rate"])
        ...
        comparison_summary = {
            ...
            "analysis": {
                ...
                "average_success_rate": sum(r["success_rate"] for r in comparison_results.values()) / len(methods)
            }
        }
...
def main():
    """CLI実行用メイン関数"""
    if len(sys.argv) < 4:
        print("Usage: python quality_workflow_tester.py <test_type> <input_dir> <output_dir> [options]")
