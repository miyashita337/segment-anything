...
class ActiveTaskAnalyzer:
    ...
    def get_high_priority_tasks(self) -> List[Dict]:
        ...
        try:
            ...
            for task in all_tasks:
                # 優先度最高または高のタスクを抽出
                if (hasattr(task, 'priority') and 
                    task.priority in [PriorityLevel.HIGHEST, PriorityLevel.HIGH]):
                    task_dict = {
                        'tracker_id': task.tracker_id,
                        'priority': task.priority.value if hasattr(task, 'priority') else 'unknown',
                        'status': task.status.value,
                        'description': task.description or '',
                        'created_date': task.created_date.strftime('%Y-%m-%d') if task.created_date else '',
                        'updated_date': task.updated_date.strftime('%Y-%m-%d') if task.updated_date else '',
    ...
    def generate_implementation_roadmap(self, high_priority_tasks: List[Dict]) -> List[Dict]:
        ...
        for task in high_priority_tasks:
            tracker_id = task['tracker_id']
            priority_info = implementation_priority.get(tracker_id, {
                'order': 999, 
                'reason': '標準実装順序'
            })
    ...
    def generate_analysis_report(self, high_priority_tasks: List[Dict], 
                               roadmap: List[Dict], output_path: Optional[str] = None) -> str:
        """分析レポート生成"""
    ...
    def print_console_summary(self, high_priority_tasks: List[Dict], roadmap: List[Dict]):
        ...
        if highest_tasks:
            ...
            for task in highest_tasks:
                status_emoji = "🔴" if task['status'] == '着手前' else "🟡" if task['status'] == '実装中' else "🟢"
        ...
        for i, item in enumerate(roadmap[:3], 1):  # 上位3件表示
            print(f"  {i}. {item['tracker_id']} ({item['estimated_hours']}h) - {item['priority_reason']}")
...
def main():
    ...
    parser.add_argument('--priority', choices=['highest', 'high', 'all'], default='all',
                       help='分析対象の優先度')
