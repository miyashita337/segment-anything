...
class TrackerCompletionValidator:
    ...
    REQUIRED_DIRECTORIES = ["extraction", "quality", "dashboard", "tests"]

    REQUIRED_FILES = ["dashboard/dashboard.html", "quality/unified_quality_report.json"]
    ...
    def validate_quality_reports(self) -> Dict[str, any]:
        ...
        return {"status": "exists", "reports": reports_info, "report_count": len(json_files)}
    ...
    def _calculate_overall_status(self, results: Dict) -> Dict[str, any]:
        ...
        checks = {
            ...
            "has_extractions": results["extraction_results"]["has_results"]
            if results["extraction_results"]["status"] == "exists"
            else False,
            "has_quality_reports": results["quality_reports"]["report_count"] > 0
            if results["quality_reports"]["status"] == "exists"
            else False,
            "dashboard_accessible": results["dashboard"]["accessible"]
            if results["dashboard"]["status"] == "complete"
            else False,
        }
    ...
    def print_validation_report(self):
        ...
        # 総合ステータス
        status_emoji = {"COMPLETE": "✅", "MOSTLY_COMPLETE": "🟡", "PARTIAL": "🟠", "INCOMPLETE": "❌"}

        print(f"\n📊 総合ステータス: {status_emoji.get(overall['status'], '❓')} {overall['status']}")
        print(
            f"📈 完了率: {overall['completion_rate']:.1%} ({overall['passed_checks']}/{overall['total_checks']})"
        )
        ...
        if overall["status"] != "COMPLETE":
            ...
            if "dashboard_accessible" in overall["failed_checks"]:
                print(
                    f"   2. 品質ワークフロー実行: ./tools/scripts/run_quality_workflow.sh {self.tracker_id}"
                )
