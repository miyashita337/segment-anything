...
class PathComplianceAuditor:
    ...
    def _generate_fix_suggestion(
        self, issue_type: str, line_content: str, matched_text: str
    ) -> str:
        """修正提案生成"""
        suggestions = {
            "relative_path": f"""
...
""",
    ...
    def scan_project(
        ...
        exclude_dirs: List[str] = [".git", "__pycache__", "venv", "node_modules"],
    ) -> List[PathIssue]:
        """プロジェクト全体のスキャン"""
        all_issues = []
    ...
    def _generate_recommendations(self) -> List[str]:
        ...
        if high_issues:
            recommendations.append(f"🚨 高優先度問題 {len(high_issues)} 件の即座対応が必要")

        # 相対パス問題
        relative_path_issues = [
            issue for issue in self.issues if issue.issue_type == "relative_path"
        ]
        if relative_path_issues:
            recommendations.append(f"📁 相対パス {len(relative_path_issues)} 件をOutputPathManagerで置換推奨")

        # ハードコード問題
        hardcoded_issues = [
            issue for issue in self.issues if issue.issue_type == "hardcoded_output"
        ]
        if hardcoded_issues:
            recommendations.append(f"🔧 ハードコード出力パス {len(hardcoded_issues)} 件の仕様準拠化が必要")
    ...
    if issues:
        ...
        try:
            ...
            report_path = manager.get_output_path(
                OutputCategory.TEST_RESULT, filename="path_compliance_audit_report.md"
            )
