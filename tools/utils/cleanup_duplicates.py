    ...
    for duplicate_file in duplicate_files:
        ...
        if not dry_run:
            try:
                ...
                if clean_path.exists():
                    ...
                    if duplicate_size > clean_size:
                        ...
                        print(f"     🗑️ Removing duplicate (clean version exists and is larger/equal)")
            ...
            if clean_path.exists():
                ...
                clean_size = clean_path.stat().st_size
                if duplicate_size > clean_size:
                    print(f"     → Would replace smaller clean file ({clean_size} bytes) with larger duplicate ({duplicate_size} bytes)")
                else:
                    print(f"     → Would remove duplicate ({duplicate_size} bytes), clean exists ({clean_size} bytes)")
            else:
...
def main():
    ...
    parser = argparse.ArgumentParser(description='Cleanup duplicate suffix files in QCC-011 workspace')
