...
class QualityDashboardGenerator:
    ...
    def generate_distribution_chart(self, distribution: Dict):
        ...
        ax1.axvline(
            ...
            color="red",
            linestyle="--",
            label=f'平均: {distribution["mean"]:.3f}',
        )
        ax1.axvline(
            distribution["median"],
            color="green",
            linestyle="--",
            label=f'中央値: {distribution["median"]:.3f}',
        )
    ...
    def generate_metrics_analysis_chart(self, metrics_analysis: Dict):
        ...
        for i, (metric_name, title) in enumerate(zip(metrics_names, titles)):
            ...
            if values:
                ...
                ax.axvline(
                    stats["mean"], color="red", linestyle="--", label=f'平均: {stats["mean"]:.3f}'
                )
                ax.set_xlabel(title)
                ax.set_ylabel("頻度")
        ...
        if distribution and "levels" in distribution:
            ...
            level_info = [
                (
                    "critical",
                    "Critical (<5%)",
                    f"{levels['critical']}件 ({percentages['critical']:.1f}%)",
                ),
                (
                    "monitoring",
                    "Monitoring (5-10%)",
                    f"{levels['monitoring']}件 ({percentages['monitoring']:.1f}%)",
                ),
                (
                    "improvement",
                    "Improvement (10-15%)",
                    f"{levels['improvement']}件 ({percentages['improvement']:.1f}%)",
                ),
                (
                    "acceptable",
                    "Acceptable (15-30%)",
                    f"{levels['acceptable']}件 ({percentages['acceptable']:.1f}%)",
                ),
                ("good", "Good (30-50%)", f"{levels['good']}件 ({percentages['good']:.1f}%)"),
                (
                    "excellent",
                    "Excellent (≥50%)",
                    f"{levels['excellent']}件 ({percentages['excellent']:.1f}%)",
                ),
            ]
...
"""
        ...
        if distribution:
            ...
            if "percentages" in distribution:
                ...
                if critical_pct > 10:
                    html_content += (
                        f"<li>🚨 Critical品質（<5%）の割合が{critical_pct:.1f}%と高めです。要因分析が必要です。</li>"
                    )
...
def main():
    ...
    parser.add_argument("--stats-file", type=str, help="quality_statistics.jsonlファイルパス")
    parser.add_argument("--output-dir", type=str, default="quality_dashboard", help="出力ディレクトリ")
