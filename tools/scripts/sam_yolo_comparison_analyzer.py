        ...
        if extracted_path.exists():
            ...
            if estimated_bounds:
                ...
                if result["correct_rectangle"]:
                    ...
                    correct_area = (
                        result["correct_rectangle"]["width"] * result["correct_rectangle"]["height"]
                    )
    ...
    def analyze_all_images(self) -> Dict:
        ...
        for i, filename in enumerate(image_files, 1):
            ...
            if result["has_correct_annotation"] and result["has_extracted_result"]:
                ...
                logger.info(
                    f"  IoU: {result['iou_score']:.3f}, Coverage: {result['coverage_ratio']:.3f}, Grade: {result['quality_grade']}"
                )
            else:
                logger.warning(
                    f"  不完全なデータ: 正解={result['has_correct_annotation']}, 抽出={result['has_extracted_result']}"
                )
        ...
        results["summary_statistics"] = {
            ...
            "success_rate_ab": (grade_counts["A"] + grade_counts["B"]) / valid_results
            if valid_results > 0
            else 0.0,
    ...
    def generate_visual_comparison(self, analysis_results: Dict):
        ...
        for filename, result in analysis_results["individual_results"].items():
            ...
            try:
                ...
                cv2.putText(
                    img, info_text, (10, 30), cv2.FONT_HERSHEY_SIMPLEX, 0.8, (255, 255, 255), 2
                )
...
def main():
    ...
    logger.info(
        f"🎖️ 品質分布: A:{stats['grade_distribution']['A']} B:{stats['grade_distribution']['B']} C:{stats['grade_distribution']['C']} D:{stats['grade_distribution']['D']} F:{stats['grade_distribution']['F']}"
    )
