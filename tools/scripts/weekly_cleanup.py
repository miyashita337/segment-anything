...
class WeeklyCleanupManager:
    ...
    def _cleanup_log_files(self):
        ...
        logger.info(f"✅ ログファイル整理完了: {task_info['files_processed']}ファイル, "
    ...
    def _cleanup_temp_files(self):
        ...
        self.cleanup_report['tasks'].append(task_info)
        logger.info(f"✅ 一時ファイル除去完了: {task_info['files_processed']}項目, "
                   f"{self._format_bytes(task_info['bytes_freed'])} 解放")
                ...
                if artifact_path.is_file():
                    ...
                    if not self.dry_run:
                        try:
                            ...
                            task_info['actions'].append(f"削除: {artifact_path.relative_to(self.project_root)}")
                        except OSError as e:
                    ...
                    if not self.dry_run:
                        try:
                            ...
                            task_info['actions'].append(f"ディレクトリ削除: {artifact_path.relative_to(self.project_root)}")
                        except OSError as e:
                            error_msg = f"ビルド成果物ディレクトリ削除エラー: {artifact_path} - {e}"
                            self.cleanup_report['errors'].append(error_msg)
        
        self.cleanup_report['tasks'].append(task_info)
        logger.info(f"✅ ビルド成果物除去完了: {task_info['files_processed']}項目, "
                   f"{self._format_bytes(task_info['bytes_freed'])} 解放")
    ...
    def _cleanup_workspace_directories(self):
        ...
        self.cleanup_report['tasks'].append(task_info)
        logger.info(f"✅ ワークスペース整理完了: {task_info['files_processed']}ディレクトリ, "
                   f"{self._format_bytes(task_info['bytes_freed'])} 解放")
    ...
    def _generate_cleanup_report(self):
        ...
        for task in self.cleanup_report['tasks']:
            ...
            logger.info(f"  📋 {task_name}: {task['files_processed']}項目, "
...
def main():
    ...
    parser = argparse.ArgumentParser(
        description="P1-021: ソースコード肥大化解決システム - 週次実行スクリプト"
    )
    parser.add_argument(
        '--dry-run', 
        action='store_true',
        help='実際の削除は行わず、削除予定のファイルを表示のみ'
    )
    parser.add_argument(
        '--verbose', '-v',
        action='store_true',
    ...
    # クリーンアップ実行
    cleanup_manager = WeeklyCleanupManager(
