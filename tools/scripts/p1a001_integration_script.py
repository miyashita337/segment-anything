...
class P1A001IntegratedSystem:
    ...
    def _execute_enhanced_sam_test(self):
        ...
        try:
            ...
            quality_results = {}
            for method in ['balanced', 'confidence_priority', 'size_priority', 'fullbody_priority', 'central_priority']:
                result = evaluator.evaluate_extraction_quality(test_image, test_mask, test_bbox, method)
    ...
    def _generate_recovery_visualization(self):
        ...
        try:
            ...
            if component_names:
                ...
                # 復旧機能数棒グラフ
                bars = ax1.bar(component_names, recovered_counts, color=['#3498db', '#e74c3c', '#2ecc71'])
                ...
                # 保存
                viz_file = self.workspace / f"recovery_visualization_{datetime.now():%Y%m%d_%H%M%S}.png"
...
def main():
    ...
    subprocess.run([
        "windows-notify", "-t", "P1-A001復旧プロジェクト完了",
        "-m", f"復旧機能: {summary['total_recovered_features']}件\\n成功率: {summary['success_rate']:.1f}%"
    ], check=False)
