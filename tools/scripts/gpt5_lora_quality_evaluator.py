    ...
    def evaluate_all_images(self, image_dir: Path) -> Dict[str, Any]:
        ...
        final_result = {
            'evaluation_summary': {
                ...
                'successful_evaluations': len([r for r in evaluation_results if r.get('status') == 'success']),
...
def main():
    ...
    # 画像ファイル確認
    image_files = [f for f in extraction_dir.glob("kana08_*.jpg") 
                   if "_multi_char_detection" not in f.name]
