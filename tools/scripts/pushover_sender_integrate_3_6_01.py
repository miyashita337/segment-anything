#!/usr/bin/env python3
"""
INTEGRATE-3-6-01抽出結果をPushoverに送信
品質劣化診断Phase 1の結果確認用
"""
...
def main():
    ...
    try:
        ...
        # 抽出結果ディレクトリ
        extraction_dir = "/mnt/c/AItools/lora/train/yado/tracker-workspace/INTEGRATE-3-6-01/extraction"
        
        # 画像ファイル取得
        image_files = glob.glob(os.path.join(extraction_dir, "extracted_*.png"))
        image_files.sort()
        ...
        logger.info(f"📤 INTEGRATE-3-6-01 抽出結果をPushoverに送信開始")
        ...
        for i, image_path in enumerate(selected_images, 1):
            ...
            title = f"INTEGRATE-3-6-01 Phase1品質診断 ({i}/10)"
            message = f"シンプルパラメータ品質テスト\nファイル: {filename}"
