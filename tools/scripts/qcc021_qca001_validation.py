...
class QCA001ValidationIntegrator:
    ...
    def analyze_qca001_sample_adequacy(self) -> Dict[str, Any]:
        ...
        validation = self.validator.validate_sample_adequacy(
            current_sample_size=current_sample_size, test_scenarios=qca001_scenarios
        )
        ...
        # QCA-001特化の改善提案生成
        qca001_recommendations = self._generate_qca001_recommendations(
            validation, current_sample_size
        )
    ...
    def _analyze_validation_results(self, validation, current_sample_size: int) -> Dict[str, Any]:
        ...
        precision_analysis = {
            ...
            "sample_efficiency": current_sample_size / validation.recommended_n
            if validation.recommended_n > 0
            else 1.0,
        }
    ...
    def _generate_qca001_recommendations(self, validation, current_sample_size: int) -> List[str]:
        ...
        if not validation.overall_adequacy:
            shortage = validation.recommended_n - current_sample_size
            recommendations.append(f"QCA-001の統計的信頼性向上には追加{shortage}サンプル推奨")
            recommendations.append(f"他作者（kiri, zundamon）からの画像追加でサンプル数拡張を検討")

        if validation.current_power < 0.8:
            recommendations.append("検出力向上のため、効果サイズの大きい作者ペアでの検証を推奨")

        if validation.precision_assessment == "低精度":
            recommendations.append("推定精度向上のため、品質スコア分散を考慮したサンプリング戦略を推奨")

        # QCA-001固有の提案
        if current_sample_size < 30:
            recommendations.append("中心極限定理の適用には30サンプル以上を強く推奨（現在の機械学習評価における業界標準）")
    ...
    def _generate_text_summary(self, report: Dict[str, Any]) -> str:
        ...
        lines.extend(
            ["", f"🔬 総合評価: {report['analysis_results']['overall_assessment']}", "", "=" * 80]
        )
...
def main():
    ...
    try:
        ...
        recommendations = validation_report["warnings_and_suggestions"][
