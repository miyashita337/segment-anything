...
class P1A002ExtractionPipeline:
    ...
    def run_extraction_background(self) -> Dict[str, Any]:
        ...
        try:
            ...
            return {"success": False, "error": str(e), "log_file": str(self.log_file)}
    ...
    def analyze_extraction_results(self) -> Optional[Dict[str, Any]]:
        ...
        if not result_files:
            ...
            # 簡易解析（画像ファイル数カウント）
            extracted_images = list(self.output_dir.glob("*.png")) + list(
    ...
    def save_extraction_summary(
        self, extraction_result: Dict[str, Any], quality_evaluation: Dict[str, Any]
    ) -> Path:
        """抽出処理サマリー保存"""
        summary = {
        ...
        summary_file = (
            self.extraction_dir / f"P1A002_extraction_summary_{datetime.now():%Y%m%d_%H%M%S}.json"
        )
