...
class PH2006MonitoringTester:
    ...
    def test_processing_metrics_integration(self) -> Dict:
        ...
        # 処理メトリクス確認
        processing_metrics_count = len(
            self.monitoring_system.metrics_collector.processing_metrics_history
        )
        ...
        try:
            ...
            test_result = {
                ...
                "report_summary": {
                    ...
                    "system_performance": report.get("monitoring_health", {}).get(
                        "system_performance"
                    ),
                },
                "success": report_has_system_stats
                and report_has_monitoring_health
                and report_file_exists,
            }
            ...
            return {"test_type": "report_generation", "success": False, "error": str(e)}
        ...
        try:
            ...
            for endpoint, name in endpoints:
                try:
                    ...
                    api_results[name] = {"endpoint": endpoint, "success": False, "error": str(e)}
            ...
            # 結果集計
            successful_apis = sum(
                1 for result in api_results.values() if result.get("success", False)
            )
            ...
            return {"test_type": "web_dashboard_api", "success": False, "error": str(e)}
    ...
    def test_stress_monitoring(self) -> Dict:
        ...
        try:
            ...
            # 結果分析
            cpu_increase = (
                (after_stress.cpu_percent - before_stress.cpu_percent)
                if before_stress and after_stress
                else 0
            )
            memory_increase = (
                (after_stress.memory_percent - before_stress.memory_percent)
                if before_stress and after_stress
                else 0
            )
            ...
            return {"test_type": "stress_monitoring", "success": False, "error": str(e)}
    ...
    def log_test_summary(self):
        ...
        success_rate = (
            summary["completed_tests"] / summary["total_tests"] if summary["total_tests"] > 0 else 0
        )
