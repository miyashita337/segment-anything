...
def monitor_mode(args):
    ...
    monitor = ResourceMonitor(monitoring_interval=args.interval, history_size=args.history_size)
    ...
    try:
        ...
        while time.time() - start_time < args.duration:
            ...
            if stats:
                print(
                    f"📈 監視統計: 最適化{stats.get('optimization_count', 0)}回, "
    ...
    if status.gpu_count > 0:
        ...
        for i in range(status.gpu_count):
            if i < len(status.gpu_memory_used):
                ...
                print(
                    f"  GPU{i}: {memory_percent:.1f}% ({memory_used:.1f}/{memory_total:.1f}GB), "
                    f"{temp}°C, {util}%使用率"
                )
    else:
        print("\n🎮 GPU: 検出されませんでした")
...
def main():
    ...
    monitor_parser.add_argument("--duration", type=int, default=300, help="監視時間（秒、デフォルト: 300）")
    monitor_parser.add_argument("--interval", type=float, default=5.0, help="監視間隔（秒、デフォルト: 5.0）")
    monitor_parser.add_argument("--history-size", type=int, default=100, help="履歴保持数（デフォルト: 100）")
