...
class PH2005ScalabilityTester:
    ...
    def test_thread_pool_engine(self) -> dict:
        ...
        results, metrics = self.engine.process_with_thread_pool(
    ...
    def test_process_pool_engine(self) -> dict:
        ...
        start_time = time.time()
        results, metrics = self.engine.process_with_process_pool(
            cpu_intensive_task, test_data
        )
        end_time = time.time()
    ...
    async def test_async_io_engine(self) -> dict:
        ...
        start_time = time.time()
        results, metrics = await self.engine.process_with_async_io(
            async_io_task, test_data
        )
        end_time = time.time()
    ...
    def test_gpu_parallel_engine(self) -> dict:
        ...
        # テンソルデータ作成
        test_tensors = [
            torch.randn(32, 32) for _ in range(10)
        ]
        
        start_time = time.time()
        results, metrics = self.engine.process_with_gpu_parallel(
            gpu_tensor_task, test_tensors
        )
        end_time = time.time()
    ...
    def test_auto_engine_selection(self) -> dict:
        ...
        selection_tests.append({
            ...
            "parameters": {"task_count": 5, "cpu_intensive": False, "io_intensive": False, "gpu_compatible": False},
        ...
        # テストケース3: I/O集約的タスク
        engine_type = self.engine.choose_optimal_engine(
            task_count=100, io_intensive=True
        )
        selection_tests.append({
        ...
        # テストケース4: GPU対応タスク
        engine_type = self.engine.choose_optimal_engine(
            task_count=20, gpu_compatible=True
        )
        selection_tests.append({
    ...
    async def run_all_tests(self):
        ...
        try:
            ...
            # 5. 自動エンジン選択テスト
            self.test_results["integration_tests"]["auto_selection"] = self.test_auto_engine_selection()
            self.test_results["test_summary"]["completed_tests"] += 1
            ...
            # 成功率計算
            success_rate = (self.test_results["test_summary"]["completed_tests"] - 
                          self.test_results["test_summary"]["failed_tests"]) / self.test_results["test_summary"]["total_tests"]
            
            self.test_results["test_summary"]["success_rate"] = success_rate
            
            logger.info(f"✅ 全テスト完了: {self.test_results['test_summary']['completed_tests']}/{self.test_results['test_summary']['total_tests']}")
    ...
    def log_test_summary(self):
        ...
        for engine_name, result in self.test_results.get("engine_tests", {}).items():
            if isinstance(result, dict) and "success_rate" in result:
                logger.info(f"  {engine_name}: 成功率 {result['success_rate']:.1%}, "
