...
def fix_qcc021_extended_statistics():
    ...
    try:
        ...
        success_rate = (
            actual_successes / estimated_input_total if estimated_input_total > 0 else 0.0
        )
...
def generate_fixed_dashboard(unified_stats):
    ...
    try:
        # ダッシュボード生成器初期化
        generator = DashboardGenerator(tracker_id=tracker_id, input_directories=[])  # 制限のため空に設定
...
def main():
    ...
    print(
        f"📊 修正後: {unified_stats['success_count']}/{unified_stats['total_input_images']}（数学的整合性確保）"
    )
