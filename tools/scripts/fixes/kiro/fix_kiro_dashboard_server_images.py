#!/usr/bin/env python3
"""
KIRO-001-002 ダッシュボード画像パス修正スクリプト
サーバー経由アクセス用の正しい画像パスに修正
"""
...
def generate_dashboard_with_server_compatible_images():
    """
    KIRO-001-002ダッシュボードにサーバー互換の画像パスを設定
    """
        ...
        card_html = f"""                <div class="bg-white rounded-lg shadow-md overflow-hidden">
                    <div class="relative">
                        <img src="{image_url}" alt="{filename}" 
                             ...
                             loading="lazy"
                             onerror="console.log('Image load error: {filename}'); this.style.display='none'; this.nextElementSibling.style.display='flex';">
    ...
    <div class="container mx-auto px-4 py-8">
        ...
        <div class="bg-white rounded-lg shadow-md p-6">
            ...
            <div class="text-sm text-gray-500 mb-4">
                画像パス形式: /workspace/{tracker_id}/extraction/[filename]
            </div>
...
{chr(10).join(image_cards_html)}
            ...
            <div class="mt-6 p-4 bg-blue-50 rounded-lg">
                <h3 class="font-semibold text-blue-800 mb-2">🔍 画像表示トラブルシューティング</h3>
                <div class="text-sm text-blue-700">
                    <p>• 画像が表示されない場合は、統合ダッシュボードサーバーの画像配信設定を確認してください</p>
                    <p>• 画像パス: <code class="bg-white px-1 rounded">/workspace/{tracker_id}/extraction/[filename]</code></p>
                    <p>• 実ファイル数: {len(extracted_files)}枚</p>
                </div>
            </div>
        </div>
    </div>
    ...
    <script>
        ...
        window.addEventListener('load', function() {{
            const images = document.querySelectorAll('img[src*="/workspace/"]');
            console.log('Dashboard loaded with', images.length, 'images');
