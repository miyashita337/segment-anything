...
class SafePathReplacer:
    ...
    def process_batch(self, target_files: List[Path], dry_run: bool = True) -> dict:
        ...
        if not processable_files:
            ...
            return {"processed": 0, "skipped": len(self.skipped_files), "errors": len(self.error_files)}
...
def main():
    ...
    parser.add_argument("--directories", nargs="+", default=["tests", "tools", "features"],
                      help="処理対象ディレクトリ")
    parser.add_argument("--batch-size", type=int, default=10,
                      help="バッチサイズ（ファイル数）")
    parser.add_argument("--dry-run", action="store_true",
                      help="ドライラン実行（実際の変更なし）")
    parser.add_argument("--stage", choices=["critical", "all"], default="critical",
