...
class UniversalDashboardMerger:
    ...
    def _load_config(self, config_path: Optional[str] = None) -> Dict[str, Any]:
        ...
        return {
            ...
            "quality_thresholds": {"high_size_kb": 100, "medium_size_kb": 50},
            "dashboard_server": {"host": "100.123.241.106", "port": 8088, "auth_required": True},
    ...
    def merge_tracker_dashboard(self, tracker_id: str) -> str:
        ...
        if not primary_workspace:
            raise ValueError(
                f"Primary author '{self.config['integration']['primary_author']}' workspace not found for {tracker_id}"
            )

        # 画像統合処理
        all_images = self._collect_and_merge_images(
            tracker_id, author_workspaces, primary_workspace
        )
    ...
    def _collect_and_merge_images(
        self, tracker_id: str, author_workspaces: Dict[str, Path], primary_workspace: Path
    ) -> List[Dict[str, Any]]:
        """画像収集・統合処理"""
        all_images = []
    ...
    def _generate_dashboard(
        self, tracker_id: str, all_images: List[Dict[str, Any]], primary_workspace: Path
    ) -> Path:
        """ダッシュボードHTML生成"""
        ...
        quality_score = (
            (quality_counts["high"] + quality_counts["medium"] * 0.5) / len(all_images) * 100
            if all_images
            else 0
        )
...
<body>
    ...
    def _get_relative_path(self, image_path: str, tracker_id: str) -> str:
        ...
        primary_workspace_path = (
            ...
            / self.config["integration"]["primary_author"]
            / "tracker-workspace"
        )
