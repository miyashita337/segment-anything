...
class CorrectAnnotationExtractor:
    ...
    def extract_all_annotations(self) -> Dict:
        ...
        # 画像ファイル取得
        image_files = list(self.annotation_dir.glob("*.jpg")) + list(self.annotation_dir.glob("*.png"))
        image_files.sort()
        ...
        for filename, annotation in results['annotations'].items():
            ...
            try:
                ...
                for i, rect in enumerate(annotation['rectangles']):
                    ...
                    cv2.putText(img, info_text, (x, y-10), cv2.FONT_HERSHEY_SIMPLEX, 0.5, color, 1)
...
def main():
    """メイン実行"""
    # パス設定
    annotation_dir = Path("C:/AItools/lora/train/yado/tracker-workspace/P1-B004/correct_annotations")
