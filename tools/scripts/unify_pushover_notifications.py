...
def migrate_file(file_path: Path) -> bool:
    ...
    try:
        ...
        replacements = [
            ...
            (r"notifier = (?:PushoverNotifier|ExtractionNotifier)\(\)", "# 統一通知システムを使用（インスタンス化不要）"),
            # send_pushover_notification呼び出しの置換
            (r"send_pushover_notification\((.*?)\)", r"notify_success(\1)"),
        ]
        ...
        method_replacements = [
            ...
            (r"notifier\.send_notification\((.*?)\)", r"notify_success(\1)"),
            (r"notifier\.send_error\((.*?)\)", r"notify_error(\1)"),
        ]
