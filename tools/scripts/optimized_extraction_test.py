...
def run_optimized_extraction_test():
    ...
    test_output_dir = Path("C:/AItools/lora/train/yado/tracker-workspace/P1-B004/optimization/test_extraction")
    ...
    if results['summary']['extraction_times']:
        avg_time = sum(results['summary']['extraction_times']) / len(results['summary']['extraction_times'])
    ...
    logger.info(f"📊 成功率: {success_rate:.1%} ({results['summary']['successful_extractions']}/{results['summary']['total_tested']})")
