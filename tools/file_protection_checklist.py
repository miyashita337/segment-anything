...
class FileProtectionChecker:
    ...
    def check_dependencies(self, file_path: str) -> List[str]:
        ...
        try:
            ...
            ], capture_output=True, text=True, cwd=Path(file_path).parent.parent)
    ...
    def analyze_file_content(self, file_path: str) -> Dict[str, any]:
        ...
        try:
            with open(file_path, 'r', encoding='utf-8') as f:
                ...
                analysis['has_imports'] = any(line.strip().startswith('import') or 
                                            line.strip().startswith('from') 
                                            for line in lines)
                
                # 重要キーワード検出
                important_keywords = [
                    'quality', 'evaluation', 'stable', 'improved',
                    'sam', 'yolo', 'extraction', 'segment'
                ]
...
def main():
    ...
    parser.add_argument('--output', default='file_protection_report.json', 
                       help='レポート出力パス')
