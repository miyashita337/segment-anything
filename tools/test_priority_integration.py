...
from progress_tracker.data_models import TaskRecord, TaskStatus, ComponentStatus, PriorityLevel, MetricsRecord
...
def verify_sheet_structure():
    ...
    try:
        ...
        print("   現在のヘッダー構造:")
        expected_headers = [
            "トラッカーID", "優先度", "ステータス", "登録日付", "更新日付", "説明",
            "動作確認", "テストUNIT", "品質評価", "統合スクリプト", "ダッシュボード", "抽出パイプライン",
            "LCA", "A/B評価率", "FPS", "C以上評価率", "平均カバレッジ率", "平均コンパクトネス",
            "平均フィル率", "SCI", "PLA", "PLE"
        ]
        ...
        for i, (expected, actual) in enumerate(zip(expected_headers, headers)):
            ...
            print(f"   {col_letter}列: {match} {actual} {'(期待: ' + expected + ')' if expected != actual else ''}")
