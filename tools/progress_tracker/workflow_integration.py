...
class WorkflowIntegrator:
        ...
        try:
            ...
            if success:
                ...
                self.manager.update_component_status(
                    tracker_id, "quality_evaluation", ComponentStatus.COMPLETED
                )

                # 10指標データ自動更新
                if "quality_results" in results:
                    self.manager.update_from_quality_checker_results(
                        tracker_id, results["quality_results"]
                    )
                ...
                # 失敗マーク
                self.manager.update_component_status(
                    tracker_id, "quality_evaluation", ComponentStatus.FAILED
                )
    ...
    def _run_unified_quality_checker(self, **kwargs) -> Dict[str, Any]:
        ...
        try:
            ...
            return {"success": True, "quality_results": results, "message": "品質評価完了"}
    ...
    def run_extraction_pipeline_with_tracking(self, tracker_id: str, **kwargs) -> Dict[str, Any]:
        ...
        try:
            # 抽出パイプライン開始
            self.manager.update_component_status(
                tracker_id, "extraction_pipeline", ComponentStatus.IN_PROGRESS
            )
            ...
            if success:
                self.manager.update_component_status(
                    tracker_id, "extraction_pipeline", ComponentStatus.COMPLETED
                )
                self.manager.update_task_status(tracker_id, TaskStatus.EXTRACTION_PIPELINE)
                logger.info(f"抽出パイプライン成功: {tracker_id}")
            else:
                self.manager.update_component_status(
                    tracker_id, "extraction_pipeline", ComponentStatus.FAILED
                )
...
def main():
    ...
    parser.add_argument(
        "--mode", choices=["quality", "extraction", "complete"], default="quality", help="実行モード"
    )
    parser.add_argument("--quality-method", default="balanced", help="品質評価手法")
    ...
    try:
        ...
        if args.mode == "quality":
            results = integrator.run_quality_workflow_with_tracking(
                args.tracker_id, quality_method=args.quality_method
            )
