...
class StatisticalIntegrationSystem:
    ...
    def update_statistical_columns_enhanced(self, tracker_id: str) -> bool:
        ...
        try:
            ...
            result = self.statistical_analyzer.compare_trackers(
                baseline, tracker_id, "quality_score"
            )
    ...
    def batch_update_statistical_columns(self, tracker_ids: List[str]) -> Dict[str, bool]:
        ...
        print(f"\n📊 バッチ更新結果:")
        print(
            f"   - 成功: {success_count}/{len(tracker_ids)} ({success_count/len(tracker_ids)*100:.1f}%)"
        )
