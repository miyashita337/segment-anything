...
⚠️ 注意: INCI-004により、統計分析時のBASELINE_ID指定が必須化されました"""
    ...
    def calculate_enhanced_cohens_d(
        ...
        # 7. 改善率計算
        improvement_rate = ((mean_current - mean_baseline) / mean_baseline * 100) if mean_baseline > 0 else 0.0
    ...
    def _display_results(
        ...
        print(f"   95%信頼区間: [{result.confidence_interval[0]:.4f}, {result.confidence_interval[1]:.4f}]")
        ...
        if verbose:
            ...
            print(f"   {result.current_tracker}: 平均={current_stats['mean']:.4f}, SD={current_stats['std']:.4f}, N={current_stats['n']}")
            print(f"   {result.baseline_tracker}: 平均={baseline_stats['mean']:.4f}, SD={baseline_stats['std']:.4f}, N={baseline_stats['n']}")
...
注意:
  ...
  --baseline オプションを省略するとエラーで終了します。
    ...
    parser.add_argument('--baseline', required=True,
                       help='ベースライントラッカーID（必須）- INCI-004により必須化')
