...
class ExecutionPermissionManager:
    ...
    def set_permission_level(self, level: PermissionLevel) -> None:
        ...
        # 監査ログ記録
        self._add_audit_log("permission_change", {
    ...
    def check_permission(self, action: ActionType, target: Optional[str] = None) -> bool:
        ...
        permission_matrix = {
            ...
            PermissionLevel.EXECUTE_FULL: [
                ...
                ActionType.EXECUTE, ActionType.GIT_OPERATION,
                ActionType.SYSTEM_COMMAND, ActionType.CONFIG_CHANGE
            ]
        }
        ...
        # 監査ログ記録
        self._add_audit_log("permission_check", {
            "action": action.value,
            "target": target,
    ...
    def require_confirmation(self, action: str, details: Optional[str] = None) -> bool:
        ...
        try:
            ...
            # 監査ログ記録
            self._add_audit_log("user_confirmation", {
                "action": action,
                "details": details,
                "approved": approved
            })
    ...
    def enforce_permission(self, action: ActionType, target: Optional[str] = None) -> None:
        ...
        if not self.check_permission(action, target):
            current_level = self.get_current_level()
            error_msg = (
                f"権限違反: {action.value} は現在の権限レベル "
                f"{current_level.name} では実行できません。"
            )
            if target:
