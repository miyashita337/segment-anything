...
from tools.progress_tracker.data_models import ComponentStatus, TaskRecord, TaskStatus
                ...
                if task_data["completion_date"]:
                    try:
                        completion_date = datetime.strptime(
                            task_data["completion_date"], "%Y-%m-%d"
                        )
...
def main():
    ...
    parser.add_argument(
        "--markdown-path",
        default="docs/workflows/PROGRESS_TRACKER.md",
        help="PROGRESS_TRACKER.mdのパス",
    )
    parser.add_argument("--execute", action="store_true", help="実際の移行を実行（デフォルトはドライラン）")
    parser.add_argument("--create-initial", action="store_true", help="初期タスクセットを作成")
