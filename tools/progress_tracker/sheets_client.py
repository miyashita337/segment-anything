...
class GoogleSheetsClient:
    ...
    def _authenticate(self) -> None:
        ...
        try:
            ...
            credentials = Credentials.from_service_account_file(str(auth_path), scopes=self.SCOPES)
    ...
    def _detect_actual_sheet_name(self) -> None:
        ...
        try:
            ...
            logger.warning(
                f"設定シート名 '{self.config.sheet_name}' が見つかりません。" f"最初のシート '{first_sheet_name}' を使用します"
            )
    ...
    def ensure_proper_sheet_setup(self) -> bool:
        ...
        try:
            ...
            if current_name != self.config.sheet_name:
                logger.warning(f"シート名が設定と異なります: '{current_name}' != '{self.config.sheet_name}'")
    ...
    def _try_rename_sheet(self, sheet_id: int, current_name: str) -> bool:
        ...
        try:
                ...
                "requests": [
                    {
                        "updateSheetProperties": {
                            "properties": {"sheetId": sheet_id, "title": self.config.sheet_name},
                            "fields": "title",
                        }
                    }
                ]
            ...
            request = self.service.spreadsheets().batchUpdate(
                spreadsheetId=self.config.spreadsheet_id, body=request_body
            )
    ...
    def get_sheet_values(self, range_name: str) -> List[List[str]]:
        ...
        try:
            ...
            request = (
                ...
                .values()
                .get(spreadsheetId=self.config.spreadsheet_id, range=safe_range)
            )
    ...
    def setup_sheet_validation(self) -> None:
        ...
        try:
                ...
                "setDataValidation": {
                    ...
                    "rule": {
                        "condition": {
                            "type": "ONE_OF_LIST",
                            "values": [
                                {"userEnteredValue": status.value} for status in ComponentStatus
                            ],
                        },
                        "showCustomUi": True,
                        "strict": True,
            ...
            # バッチ更新実行
            requests = [status_validation, component_validation, conditional_formatting]
