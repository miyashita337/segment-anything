    ...
    try:
        ...
        try:
            ...
            if task_row:
                ...
                print("⚠️ 基本タスクは作成されましたが、詳細情報の更新に失敗しました")
...
def cmd_create_task_enhanced(args):
    ...
    try:
        ...
        # 基本タスク作成
        manager.create_task(args.tracker_id, args.description or "")
...
def main():
    ...
    create_parser.add_argument(
        "--priority", "-p", choices=["高", "中", "低"], default="高", help="優先度 (デフォルト: 高)"
    )
    ...
    create_enhanced_parser.add_argument(
        "--priority", "-p", choices=["高", "中", "低"], default="高", help="優先度 (デフォルト: 高)"
    )
    create_enhanced_parser.add_argument("--summary", "-s", help="概要（省略時は説明を使用）")
    create_enhanced_parser.add_argument(
        "--registration-date", "--reg-date", help="登録日付 (yyyy-mm-dd hh:mm:ss形式、省略時は現在時刻)"
    )
    ...
    update_details_parser.add_argument(
        "--registration-date", "--reg-date", help="登録日付 (yyyy-mm-dd hh:mm:ss形式)"
    )
