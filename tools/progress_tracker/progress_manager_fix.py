...
class ProgressManager:
    ...
    def create_task(self, tracker_id: str, description: str = "") -> TaskRecord:
        ...
        try:
            ...
            # 新規タスク作成
            task = TaskRecord(
                tracker_id=tracker_id, description=description, status=TaskStatus.NOT_STARTED
            )
    ...
    def update_component_status(
        self, tracker_id: str, component: str, status: ComponentStatus
    ) -> TaskRecord:
        """コンポーネントステータス更新"""
        try:
            task = self.get_task(tracker_id)
    ...
    def progress_workflow_step(
        self, tracker_id: str, step: str, success: bool = True
    ) -> TaskRecord:
        """ワークフローステップ進行"""
        try:
            task = self.get_task(tracker_id)
    ...
    def bulk_update_from_workflow(
        self, tracker_id: str, workflow_results: Dict[str, bool]
    ) -> TaskRecord:
        """ワークフロー結果の一括更新"""
        try:
            task = self.get_task(tracker_id)
            ...
            for workflow_key, component_field in component_map.items():
                if workflow_key in workflow_results:
                    ...
                    component_status = (
                        ComponentStatus.COMPLETED if success else ComponentStatus.FAILED
                    )
    ...
    def generate_status_report(self) -> Dict[str, Any]:
        ...
        try:
            ...
            # 最近の活動
            recent_tasks = sorted(
                all_tasks, key=lambda t: t.updated_date or datetime.min, reverse=True
            )[:5]
    ...
    def update_from_quality_checker_results(
        self, tracker_id: str, quality_results: Dict[str, Any]
    ) -> TaskRecord:
        """統合品質チェッカー結果から自動更新"""
        try:
            # 品質チェッカーの結果形式を解析
