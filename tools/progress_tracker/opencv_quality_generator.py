...
class OpenCVQualityGenerator:
    ...
    def calculate_image_quality_opencv(self, image_path: Path) -> Dict:
        ...
        try:
            ...
            # 重み付け総合スコア
            quality_score = (
                0.25 * normalized_edge +      # エッジ鮮明度
                0.20 * normalized_contrast +  # コントラスト
                0.15 * normalized_brightness + # 適正明度
                0.15 * normalized_edge_ratio + # エッジ検出率
                0.10 * normalized_noise +      # ノイズの少なさ
                0.10 * color_richness +        # 色彩豊かさ
                0.05 * normalized_structural    # 構造品質
            )
    ...
    def generate_extraction_result_json(self, tracker_id: str, force_regenerate: bool = False) -> Dict:
        """
        ...
        if json_path.exists() and not force_regenerate:
            try:
                ...
                if ('results' in existing_data and 
                    existing_data['results'] and 
                    'quality_score' in existing_data['results'][0]):
                    
                    result.update({
                        'success': True,
                        'images_analyzed': len(existing_data['results']),
                        'mean_quality_score': existing_data.get('mean_quality_score', 0.0),
                        'successful_extractions': existing_data.get('successful_extractions', 0),
        ...
        try:
            ...
            print(f"✅ {tracker_id}: 品質分析完了（平均品質: {mean_quality:.3f}, 成功: {successful_count}/{len(image_files)}）")
    ...
    def _calculate_quality_statistics(self, analysis_results: List[Dict]) -> Dict:
        """品質統計計算"""
        quality_scores = [
            r['quality_score'] for r in analysis_results 
    ...
    def batch_generate_quality_scores(self, tracker_ids: List[str], force_regenerate: bool = False) -> Dict:
        """
        複数トラッカーの一括品質スコア生成
        ...
        for i, tracker_id in enumerate(tracker_ids, 1):
            ...
            if generation_result['success']:
                if 'skipped_reason' in generation_result:
                    ...
                    print(f"   ✅ 完了: {generation_result['images_analyzed']}画像, 平均品質={generation_result['mean_quality_score']:.3f}")
            else:
        ...
        print(f"   - 成功率: {(results['successful_generations']+results['skipped_generations'])/results['total_trackers']*100:.1f}%")
...
def main():
    ...
    test_trackers = [
        'QCC-022', 'QCC-021', 'QI-003', 'QI-002', 'QCC-FIX-001'
    ]
