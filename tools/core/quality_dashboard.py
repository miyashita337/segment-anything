...
class QualityDashboard:
    ...
    def __init__(self):
        ...
        self.category_colors = {"evaluation": "#3498db", "mask": "#9b59b6", "objective": "#e67e22"}
    ...
    def _create_radar_chart(self, report_data: Dict[str, Any], output_dir: Path) -> str:
        ...
        try:
            ...
            if not implemented_metrics:
                # データなしの場合のプレースホルダー
                ax.text(
                    0.5, 0.5, "データなし", ha="center", va="center", transform=ax.transAxes, fontsize=20
                )
    ...
    def _create_category_bar_chart(self, report_data: Dict[str, Any], output_dir: Path) -> str:
        ...
        try:
            ...
            bars = ax.bar(
                ...
                pass_rates,
                color=[
                    self.category_colors["evaluation"],
        ...
        try:
            ...
            for status, color in self.colors.items():
                if status in statuses:
                    label = {"passed": "合格", "failed": "不合格", "insufficient_data": "データ不足"}.get(
                        status, status
                    )
                    legend_elements.append(
                        mpatches.Rectangle((0, 0), 1, 1, facecolor=color, label=label)
                    )
    ...
    def _create_html_dashboard(
        self, report_data: Dict[str, Any], graphs: Dict[str, str], output_dir: Path
    ) -> str:
        """HTMLダッシュボード作成"""
        try:
            html_content = f"""
