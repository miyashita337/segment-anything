...
class UnifiedQualityStandardSystem:
    ...
    def _load_or_create_standard(self) -> QualityStandard:
        ...
        standard = QualityStandard(
            name="unified_quality_standard_v1",
            version="1.0.0"
        )
    ...
    def evaluate_dataset_quality(self, dataset_name: str, 
                                results_data: Dict[str, Any]) -> UnifiedQualityResult:
        """データセット品質評価（統一基準）"""
        ...
        unified_score = self._calculate_unified_score(
            ab_evaluation_rate, sci_score, pla_score, ple_score, 
            success_rate, avg_fill_ratio, avg_compactness, avg_coverage,
            dataset_name
        )
    ...
    def _calculate_unified_score(self, ab_rate: float, sci: float, pla: float, ple: float,
                               success_rate: float, fill_ratio: float, compactness: float, 
                               coverage: float, dataset_name: str) -> float:
        """統一スコア計算"""
        # データセット重み取得
    ...
    def _generate_recommendations(self, results: List[UnifiedQualityResult]) -> List[str]:
        ...
        if poor_datasets:
            recommendations.append(
                f"品質改善が必要なデータセット: {[r.dataset_name for r in poor_datasets]}"
            )
        
        low_ab_datasets = [r for r in results if r.ab_evaluation_rate < 0.6]
        if low_ab_datasets:
            recommendations.append(
                f"A/B評価率改善が必要: {[r.dataset_name for r in low_ab_datasets]}"
            )
        
        best_dataset = max(results, key=lambda x: x.unified_score)
        recommendations.append(f"最高品質データセット: {best_dataset.dataset_name} ({best_dataset.unified_score:.3f})")
        
        return recommendations
    
    def save_evaluation_result(self, result: UnifiedQualityResult) -> Path:
        """評価結果保存"""
        output_file = self.workspace_dir / "quality" / f"unified_quality_{result.dataset_name}_{datetime.now():%Y%m%d_%H%M%S}.json"
    ...
    def save_comparison_result(self, comparison: Dict[str, Any]) -> Path:
        """比較結果保存"""
        output_file = self.workspace_dir / "quality" / f"dataset_comparison_{datetime.now():%Y%m%d_%H%M%S}.json"
    ...
    def generate_unified_report(self, results: List[UnifiedQualityResult]) -> Dict[str, Any]:
        ...
        report = {
            ...
            "summary": {
                ...
                "average_unified_score": np.mean([r.unified_score for r in results]) if results else 0.0
            },
        ...
        # レポート保存
        report_file = self.workspace_dir / "quality" / f"unified_quality_report_{datetime.now():%Y%m%d_%H%M%S}.json"
...
def main():
    ...
    if args.dataset and args.results:
        ...
        # サンプルデータで比較デモ
        sample_results = [
            system.evaluate_dataset_quality("kana08", {
                "total_processed": 16, "successful_extractions": 15,
                "ab_evaluation_rate": 0.8, "sci_score": 0.75, "pla_score": 0.8, "ple_score": 0.85,
                "avg_fill_ratio": 0.85, "avg_compactness": 0.7, "avg_coverage": 0.8
            }),
            system.evaluate_dataset_quality("kana05", {
                "total_processed": 36, "successful_extractions": 34,
                "ab_evaluation_rate": 0.85, "sci_score": 0.8, "pla_score": 0.75, "ple_score": 0.9,
                "avg_fill_ratio": 0.9, "avg_compactness": 0.75, "avg_coverage": 0.85
            })
        ]
