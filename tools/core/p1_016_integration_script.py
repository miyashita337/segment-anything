...
from features.processing.feedback_loop_system import FeedbackLoopSystem, create_feedback_loop_system
...
class P1016IntegrationRunner:
    ...
    def run_integration_test(self, input_dir: str) -> Dict[str, Any]:
        ...
        try:
            ...
            for i, image_file in enumerate(image_files, 1):
                ...
                try:
                    ...
                    session_result = self._process_single_image_with_feedback(
                        str(image_file), str(extraction_dir)
                    )
                    ...
                    # 進捗報告
                    logger.info(
                        f"✅ 処理完了 {i}/{len(image_files)} (成功: {successful_sessions}, 失敗: {failed_sessions})"
                    )
            ...
            return {"tracker_id": self.tracker_id, "success": False, "error": str(e)}
    ...
    def _process_single_image_with_feedback(
        ...
        try:
            ...
            logger.info(
                f"📊 品質予測: {session.quality_prediction.predicted_quality:.3f} ({session.quality_prediction.recommended_method})"
            )
            ...
            self.feedback_system.record_processing_stage(
                session_id, "yolo_inference", yolo_duration
            )
            ...
            self.feedback_system.record_processing_stage(
                session_id, "postprocessing", post_duration
            )
    ...
    def _run_extraction_subprocess(
        self, image_path: str, output_dir: str, yolo_threshold: float, method: str
    ) -> Dict[str, Any]:
        """抽出処理をサブプロセスで実行"""
        try:
            ...
            if result.returncode == 0:
                # 成功時の結果構築
                output_files = list(Path(output_dir).glob("*.png")) + list(
...
def run_p1016_integration_test():
    ...
    if results.get("success", True):  # successキーがない場合は正常処理と判定
        ...
        if summary:
            logger.info(
                f"📊 処理結果: {summary['successful_images']}/{summary['total_images']}枚成功 ({summary['success_rate_percent']}%)"
            )
