...
class CharacterExtractionQueue(ProcessingQueue):
    ...
    def _evaluate_extraction_quality(self, output_dir: Path) -> float:
        ...
        try:
            ...
            logger.debug(
                f"品質評価: files={len(extracted_files)}, "
    ...
    def _move_results_to_main_output(self, task_output_dir: Path, image_stem: str):
        """結果をメイン出力ディレクトリに移動"""
        try:
            extracted_files = list(task_output_dir.glob("*.png")) + list(
                task_output_dir.glob("*.jpg")
            )
...
class P1011ExtractionRunner:
    ...
    def __init__(self, tracker_id: str = "P1-011"):
        ...
        self.extraction_queue = CharacterExtractionQueue(
            config=self.queue_config, output_dir=self.extraction_dir
        )
    ...
    def run_extraction(self, input_dir: Path) -> Dict[str, Any]:
        ...
        try:
            ...
            # 結果ファイル数カウント
            extracted_files = list(self.extraction_dir.glob("*.png")) + list(
                self.extraction_dir.glob("*.jpg")
            )
            ...
            logger.info(
                f"P1-011抽出完了: {result['success_rate']:.1f}%成功率, "
