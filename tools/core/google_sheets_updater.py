...
class GoogleSheetsUpdater:
    ...
    def _authenticate(self):
        ...
        try:
            ...
            credentials = Credentials.from_service_account_file(str(auth_file_path), scopes=scopes)
    ...
    def update_status(
        ...
        try:
            ...
            # 更新データ準備
            values = [[tracker_id, status, dataset_name, total_images, timestamp]]
    ...
    def update_quality_metrics(self, tracker_id: str, quality_data: Dict[str, Any]) -> bool:
        ...
        try:
            timestamp = quality_data.get("timestamp", datetime.now().strftime("%Y-%m-%d %H:%M:%S"))
    ...
    def update_processing_log(
        self, tracker_id: str, log_type: str, message: str, details: Optional[Dict] = None
    ) -> bool:
        """
        処理ログシート更新
        ...
        try:
            ...
            values = [[tracker_id, timestamp, log_type, message, details_str]]
    ...
    def update_improvement_suggestions(self, tracker_id: str, improvements: List[str]) -> bool:
        """
        改善提案シート更新
        ...
        try:
            ...
            for improvement in improvements:
                values.append([tracker_id, timestamp, improvement])
...
def update_progress_tracker_record(
    tracker_id: str, quality_data: Dict[str, Any], priority: Optional[PriorityLevel] = None
) -> bool:
...
def update_from_quality_report(report_path: str, tracker_id: str = "PH2-002"):
    ...
    if success:
        ...
        logger.info(
            f"Spreadsheet URL: https://docs.google.com/spreadsheets/d/10B7JIXPR7AoVHBrLbIG6bvn4wfKha_SradJODwzUHFA/edit"
        )
...
def display_sheet_data(
    data: List[List[str]], title: str = "Google Sheetsデータ", limit: int = None
) -> None:
    """シートデータをテーブル形式で表示"""
    if not data:
        print("📝 表示するデータがありません")
  ...
  python google_sheets_updater.py --export-json tasks.json       # JSON出力
    ...
    if any([args.read_all, args.priority, args.status, args.get_task, args.stats]):
        try:
            ...
            print(
                f"📊 スプレッドシートURL: https://docs.google.com/spreadsheets/d/{updater.spreadsheet_id}/edit"
            )
            # 詳細: docs/google_sheets_reference.md を参照
            ...
            if args.get_task:
                ...
                if task_data:
                    ...
                    if args.export_json:
                        task_dict = {
                            headers[i]: (task_data[i] if i < len(task_data) else "")
