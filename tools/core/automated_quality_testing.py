...
class AutomatedQualityTesting:
    ...
    def _load_config(self) -> Dict[str, Any]:
        ...
        default_config = {
            ...
            "test_modes": ["quick", "standard", "full"],  # 5枚での簡易テスト  # 20枚での標準テスト  # 全データでの完全テスト
            "alert_settings": {
    ...
    def _run_extraction_pipeline(
        self, input_path: Path, output_path: Path, mode: str = "standard"
    ) -> bool:
        """抽出パイプライン実行"""
        try:
            # 入力データ確認
    ...
    def _evaluate_quality(self, output_path: Path) -> Dict[str, Any]:
        ...
        try:
            ...
            quality_result = self.quality_checker.check_batch_quality(
                str(output_path), enable_sci=True, enable_pla=True, enable_ple=True
            )
    ...
    def _detect_degradation(
        self, baseline: QualityBaseline, current: QualityBaseline, thresholds: Dict[str, float]
    ) -> Tuple[bool, List[str]]:
        """品質劣化検出"""
        degradation_details = []
        ...
        baseline_success_rate = (
            (baseline.success_count / baseline.total_processed * 100)
            if baseline.total_processed > 0
            else 0
        )
        current_success_rate = (
            (current.success_count / current.total_processed * 100)
            if current.total_processed > 0
            else 0
        )
    ...
    def _generate_recommendation(
        self, degradation_details: List[str], current: QualityBaseline
    ) -> str:
        """推奨事項生成"""
        ...
        # 一般的な推奨事項
        recommendations.extend(["- 最新のベースライン品質と比較確認", "- 入力データの品質確認", "- システムリソース使用量の確認"])
    ...
    def _send_notification(self, test_result: TestResult):
        ...
        try:
            ...
            # 通知コマンド実行
            subprocess.run(["windows-notify", "-t", title, "-m", message], check=False)
...
def main():
    ...
    parser.add_argument(
        "--action", choices=["baseline", "test", "monitor"], default="test", help="実行アクション"
    )
    parser.add_argument("--dataset", default="kana08", help="データセット名")
    parser.add_argument(
        "--mode", choices=["quick", "standard", "full"], default="standard", help="テストモード"
    )
    parser.add_argument("--force-baseline", action="store_true", help="ベースライン強制更新")
    parser.add_argument("--monitor-interval", type=int, default=24, help="監視間隔（時間）")
