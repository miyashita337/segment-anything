...
# 統一ダッシュボード生成システム v2.0
from features.common.dashboard_generator import DashboardGenerator

# PHS-013: 復旧機能システム統合
try:
    from features.common.recovery_system import AutoRecoverySystem, RecoveryManager, RecoveryState
...
class ValidationEngine:
    ...
    def validate_dependencies(self) -> ValidationResult:
        ...
        # Python依存関係チェック
        required_modules = ["torch", "cv2", "ultralytics", "segment_anything"]
        ...
        # モデルファイルチェック
        model_files = ["sam_vit_h_4b8939.pth", "yolov8x.pt"]
...
# DashboardGeneratorクラスは features.common.dashboard_generator に移動しました
# 統一ダッシュボードシステム v2.0 への移行により、重複実装を解消
...
class IntegratedQualityPipeline:
    ...
    def __init__(self, config_path: str, tracker_id: str):
        ...
        self.dashboard_generator = DashboardGenerator(
            self.config, self.workspace_dir, self.logger, tracker_id
        )
    ...
    def execute_pipeline_with_recovery(
        self, resume: bool = False, max_retries: int = 3
    ) -> PipelineResult:
    ...
    def _execute_phase3(self) -> PhaseResult:
        ...
        try:
            ...
            self.state_manager.save_checkpoint(
                "phase3", {"validation_passed": True, "input_paths": input_paths}
            )
    ...
    def _execute_phase4(self) -> PhaseResult:
        ...
        try:
            ...
            # 新アーキテクチャの抽出コマンド使用
            extraction_script = (
                project_root / "features" / "extraction" / "commands" / "extract_character.py"
            )
    ...
    def _execute_phase6(self) -> PhaseResult:
        ...
        try:
            ...
            if extraction_dir.exists() and PUSHOVER_AVAILABLE:
                ...
                pushover_success = self.dashboard_generator.send_extraction_results_to_pushover(
                    extraction_dir, max_images=10
                )
    ...
    def _create_pipeline_result(
        self, start_time: datetime, success: bool, phase_results: List[PhaseResult]
    ) -> PipelineResult:
        """パイプライン結果作成"""
        total_duration = (datetime.now() - start_time).total_seconds()
        dashboard_url = None
