...
class TextDetector:
    ...
    def create_text_mask(self, image: np.ndarray, text_regions: List[Tuple[int, int, int, int]]) -> np.ndarray:
        """テキスト除去マスク作成"""
        h, w = image.shape[:2]
...
class QualityEvaluator:
    ...
    def evaluate_extraction_quality(self, image: np.ndarray, mask: np.ndarray, 
                                  bbox: Tuple[int, int, int, int], 
                                  method: str = 'balanced') -> Dict:
        """抽出品質評価"""
    ...
    def _evaluate_balanced(self, image: np.ndarray, mask: np.ndarray, 
    ...
    def _evaluate_confidence_priority(self, image: np.ndarray, mask: np.ndarray, 
                                    bbox: Tuple[int, int, int, int]) -> Dict:
        """信頼度優先評価"""
        # エッジ連続性チェック
    ...
    def _evaluate_size_priority(self, image: np.ndarray, mask: np.ndarray, 
                              bbox: Tuple[int, int, int, int]) -> Dict:
        """サイズ優先評価"""
        area_ratio = self._calculate_area_ratio(mask)
    ...
    def _evaluate_fullbody_priority(self, image: np.ndarray, mask: np.ndarray, 
                                  bbox: Tuple[int, int, int, int]) -> Dict:
        """全身検出優先評価"""
    ...
    def _evaluate_central_priority(self, image: np.ndarray, mask: np.ndarray, 
                                 bbox: Tuple[int, int, int, int]) -> Dict:
        """中心位置優先評価"""
        centrality = self._calculate_centrality(mask, image.shape[:2])
    ...
    def _calculate_bbox_ratio(self, bbox: Tuple[int, int, int, int], 
    ...
    def _calculate_vertical_coverage(self, bbox: Tuple[int, int, int, int], image_height: int) -> float:
        """垂直カバレッジ計算"""
        _, y1, _, y2 = bbox
        coverage = (y2 - y1) / image_height
        return min(coverage, 1.0)
    
    def _calculate_fullbody_likelihood(self, aspect_ratio: float, vertical_coverage: float) -> float:
        """全身らしさ計算"""
    ...
    def _calculate_center_distance(self, bbox: Tuple[int, int, int, int], 
                                 image_shape: Tuple[int, int]) -> float:
        """中心距離計算"""
        h, w = image_shape
...
class EnhancedSAMPipeline:
    ...
    def process_image(self, image_path: str, output_dir: str, 
                     quality_method: str = 'balanced') -> Dict:
        """画像処理実行"""
        ...
        if best_result:
            ...
            output_path = self._save_result(image_rgb, best_result, output_dir, Path(image_path).stem)
            self.monitor.end_stage()
    ...
    def _overlaps_with_text(self, mask: np.ndarray, text_mask: np.ndarray, 
    ...
    def _save_result(self, image: np.ndarray, result: Dict, output_dir: str, 
                    filename: str) -> str:
        """結果保存"""
...
def main():
    ...
    parser.add_argument("--quality_method", default="balanced", 
