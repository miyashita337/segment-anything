...
class UnifiedQualityChecker:
        ...
        try:
            ...
            metrics.append(
                QualityMetric(
                    ...
                    status="passed"
                    if accuracy >= self.thresholds["largest_char_accuracy"]
                    else "failed",
                    category="evaluation",
            ...
            metrics.append(
                QualityMetric(
                    ...
                    improvement_suggestions=["GPU最適化", "モデル軽量化"]
            ...
            # 4. 品質分布分析（追加指標）
            c_or_better = (
                quality_dist.get("A", 0) + quality_dist.get("B", 0) + quality_dist.get("C", 0)
            )
    ...
    def _check_mask_metrics(self, extraction_data: Dict, output_dir: Path) -> List[QualityMetric]:
        ...
        try:
                ...
                try:
                    ...
                    if not content_validation["valid"]:
                        if content_validation["reason"] == "empty_content":
                            ...
                            black_image_count += 1
                            logger.warning(
                                f"真っ黒画像検出: {img_file.name} (明度: {content_validation.get('brightness', 'N/A')})"
                            )
                        continue
            ...
            logger.info(
                f"内容検証結果: 有効={valid_content_count}, 真っ黒={black_image_count}, 空={empty_content_count}"
            )
            ...
            metrics.append(
                QualityMetric(
                    ...
                    improvement_suggestions=["真っ黒画像の原因調査", "SAM後処理改善"]
                    if true_success_rate < 0.7
                    else [],
                )
            )
            ...
            metrics.append(
                QualityMetric(
                    ...
                    improvement_suggestions=["YOLO検出精度向上", "SAM プロンプト最適化"]
                    if black_image_rate >= 0.1
                    else [],
                )
            )
            ...
            adaptive_coverage_threshold = self._calculate_adaptive_coverage_threshold(
                mask_qualities
            )
            ...
            metrics.append(
                QualityMetric(
                    ...
                    improvement_suggestions=["検出範囲拡張"]
                    if avg_coverage < adaptive_coverage_threshold
                    else [],
                )
            )
            ...
            metrics.append(
                QualityMetric(
                    ...
                    status="passed"
                    if avg_compactness >= self.thresholds["compactness"]
                    else "failed",
                    category="mask",
                    notes=f"輪郭の滑らかさ指標",
                    improvement_suggestions=["輪郭後処理", "ノイズ除去"]
                    if avg_compactness < self.thresholds["compactness"]
                    else [],
                )
            )
            ...
            metrics.append(
                QualityMetric(
                    ...
                    status="passed"
                    if avg_fill_ratio >= self.thresholds["fill_ratio"]
                    else "failed",
                    category="mask",
                    notes=f"マスク充填度",
                    improvement_suggestions=["境界線精度向上"]
                    if avg_fill_ratio < self.thresholds["fill_ratio"]
                    else [],
                )
            )
    ...
    def _calculate_adaptive_coverage_threshold(self, mask_qualities: List[Dict]) -> float:
        ...
        try:
            # 輪郭面積の統計分析
            contour_areas = [
                m.get("contour_area", 0) for m in mask_qualities if m.get("contour_area", 0) > 0
            ]
        ...
        try:
            ...
            if success_count > 0:
                ...
                metrics.append(
                    QualityMetric(
                        ...
                        improvement_suggestions=["YOLO検出精度向上", "SAMマスク品質改良"]
                        if sci_estimated < sci_threshold
                        else [],
                    )
                )
            else:
            ...
            metrics.append(
                QualityMetric(
                    ...
                    improvement_suggestions=["ground truth データ準備", "直接IoU計算"]
                    if pla_estimated < self.thresholds["pla_score"]
                    else [],
                )
            )
            ...
            try:
                ...
                if history_file.exists():
                    ...
                    if len(last_scores) >= 2:
                        recent_trend = (
                            last_scores[-1] - last_scores[-2] if len(last_scores) >= 2 else 0.0
                        )
                        ...
                        metrics.append(
                            QualityMetric(
                                ...
                                status="passed"
                                if ple_estimated >= self.thresholds["ple_score"]
                                else "failed",
                                category="objective",
                                notes=f"履歴トレンドから推定 (データ数: {len(last_scores)})",
                                improvement_suggestions=["長期履歴蓄積", "より精密なトレンド分析"]
                                if ple_estimated < self.thresholds["ple_score"]
                                else [],
                            )
                        )
    ...
    def _analyze_failure_patterns(
        self, extraction_data: Dict, dataset_name: str
    ) -> List[QualityMetric]:
        """失敗パターン分析の実行"""
        metrics = []
        ...
        try:
            ...
            analysis_results = analyzer.analyze_failure_patterns(
                failed_images_dir, success_images_dir if success_images_dir.exists() else None
            )
            ...
            pattern_identification_rate = min(
                1.0, n_clusters / max(1, total_failed / 5)
            )  # 5枚に1パターンを期待
            ...
            metrics.append(
                QualityMetric(
                    ...
                    improvement_suggestions=["より多くの失敗サンプル収集", "特徴量エンジニアリング改善"]
                    if pattern_identification_rate < 0.6
                    else [],
                )
            )
            ...
            metrics.append(
                QualityMetric(
                    ...
                    improvement_suggestions=["異常検出アルゴリズム調整", "前処理パイプライン見直し"]
            ...
            metrics.append(
                QualityMetric(
                    ...
                    improvement_suggestions=["分類アルゴリズム改善", "特徴量次元拡張"]
    ...
    def _create_unified_report(
        ...
        mask_metrics: List[QualityMetric],
        objective_metrics: List[QualityMetric],
        pattern_analysis_metrics: List[QualityMetric] = None,
        ...
        all_metrics = (
            evaluation_metrics + mask_metrics + objective_metrics + pattern_analysis_metrics
        )

        # 実装済み指標のみで計算
        implemented_metrics = [
            m for m in all_metrics if m.status not in ["not_implemented", "error"]
        ]
        ...
        if dataset_name in ["kana08", "kana07"]:
            technical_recommendations.extend(
                ["アニメキャラクター特化YOLO閾値最適化", "SAMマスク後処理パイプライン改良", "品質評価システムのA/B判定基準見直し"]
            )
    ...
    def print_report_summary(self, report: UnifiedQualityReport) -> None:
        ...
        for category, title in categories.items():
            # パターン分析メトリクスも含める
            all_report_metrics = (
                report.evaluation_metrics + report.mask_metrics + report.objective_metrics
            )
            ...
            for metric in metrics:
                ...
                if metric.threshold is not None:
                    print(
                        f"  {status_emoji} {metric.name}: {metric.value:.3f} (閾値: {metric.threshold:.3f})"
                    )
                else:
    ...
    def send_completion_notification(
        self, report: UnifiedQualityReport, results_file: str, include_images: bool = False
    ) -> bool:
        """
        ...
        try:
            ...
            # ステータス絵文字
            status_emoji = {"PASS": "✅", "FAIL": "❌", "PARTIAL": "⚠️"}.get(report.status, "📊")
            ...
            success_rate = (
                (success_count / report.total_images * 100) if report.total_images > 0 else 0
            )
            ...
            if include_images and success_count > 0:
                ...
                if success_images:
                    ...
                    grid_output_path = (
                        ...
                        / f"success_grid_{report.dataset_name}_{datetime.now().strftime('%Y%m%d_%H%M%S')}.jpg"
                    )
                    grid_image_path = self.create_success_images_grid(
                        success_images, str(grid_output_path)
                    )
            ...
            # 優先度設定（ステータスに基づく）
            priority = {"PASS": 0, "PARTIAL": 0, "FAIL": 1}.get(report.status, 0)  # 通常  # 通常  # 高
            ...
            if grid_image_path and Path(grid_image_path).exists():
                ...
                try:
                    ...
                    success = send_pushover_notification(
                        message=message, image_path=grid_image_path, title=title, priority=priority
                    )
                except ImportError:
                    success = False
    ...
    def create_success_images_grid(
        self, success_images: List[str], output_path: str
    ) -> Optional[str]:
        """
        成功画像のグリッド画像を作成
        ...
        try:
            ...
            for i, image_path in enumerate(success_images[:num_images]):
                ...
                try:
                    ...
                    # アスペクト比を保持してリサイズ
                    img.thumbnail(
                        (thumbnail_size - 10, thumbnail_size - 10), Image.Resampling.LANCZOS
                    )
                    ...
                    # 背景付きテキスト
                    draw.rectangle(
                        [text_x - 2, text_y - 2, text_x + text_width + 2, text_y + 12],
                        fill="white",
                        outline="gray",
                    )
            ...
            logger.info(
                f"グリッド画像作成完了: {output_path} ({output_path.stat().st_size / 1024 / 1024:.1f}MB)"
            )
...
def main():
    ...
    try:
        ...
        if args.output:
            ...
            output_path = (
                results_path.parent
                / f"unified_quality_report_{report.dataset_name}_{datetime.now().strftime('%Y%m%d_%H%M%S')}.json"
            )
