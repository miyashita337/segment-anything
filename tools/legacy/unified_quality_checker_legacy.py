        ...
        try:
            ...
            metrics.append(
                QualityMetric(
                    ...
                    status="passed"
                    if ab_rate >= self.thresholds["ab_evaluation_rate"]
                    else "failed",
                    category="evaluation",
                    notes=f"{ab_count}/{success_count} A/B評価",
                    improvement_suggestions=["品質判定基準見直し", "セグメンテーション精度向上"]
                    if ab_rate < self.thresholds["ab_evaluation_rate"]
                    else [],
                )
            )
    ...
    def _check_objective_metrics_legacy(self, extraction_data: Dict) -> List[QualityMetric]:
        ...
        try:
            ...
            if success_count > 0:
                ...
                weighted_sum = sum(
                    quality_dist.get(grade, 0) * weight for grade, weight in grade_weights.items()
                )
                ...
                metrics.append(
                    QualityMetric(
                        ...
                        status="passed"
                        if sci_estimated >= self.thresholds["sci_score"]
                        else "failed",
                        category="objective",
                        notes=f"品質分布から推定 ({success_count}枚ベース)",
                        improvement_suggestions=["直接画像分析", "MediaPipe姿勢推定強化"]
                        if sci_estimated < self.thresholds["sci_score"]
                        else [],
                    )
                )
...
def main():
    ...
    try:
        ...
        if args.output:
            ...
            output_path = (
                results_path.parent
                / f"unified_quality_report_legacy_{report.dataset_name}_{datetime.now().strftime('%Y%m%d_%H%M%S')}.json"
            )
        ...
        print(
            f"📊 A/B評価率: {[m.value for m in report.evaluation_metrics if 'A/B評価率' in m.name][0]:.1%}"
        )
