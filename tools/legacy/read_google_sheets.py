...
try:
    from progress_tracker.config import check_configuration, get_default_config
...
class SheetsReader:
    ...
    def __init__(self):
        ...
        try:
            ...
            print(
                f"📊 スプレッドシートURL: https://docs.google.com/spreadsheets/d/{self.config.spreadsheet_id}/edit"
            )
    ...
    def display_tasks_table(self, tasks: List[TaskRecord], limit: Optional[int] = None) -> None:
        ...
        for task in display_tasks:
            ...
            priority = (
                task.priority.value[:7] if len(task.priority.value) > 7 else task.priority.value
            )
            ...
            # 概要（60文字制限）
            description = (
                task.description[:57] + "..." if len(task.description) > 60 else task.description
            )
    ...
    def display_task_detail(self, task: TaskRecord) -> None:
        ...
        print(
            f"登録日     : {task.created_date.strftime('%Y-%m-%d %H:%M:%S') if task.created_date else '未設定'}"
        )
        print(
            f"更新日     : {task.updated_date.strftime('%Y-%m-%d %H:%M:%S') if task.updated_date else '未設定'}"
        )
        ...
        print(
            f"  品質評価           : {task.quality_evaluation.value if task.quality_evaluation else '未設定'}"
        )
        print(f"  統合実行スクリプト : {task.integrated_script.value if task.integrated_script else '未設定'}")
        print(
            f"  ダッシュボード生成 : {task.dashboard_generation.value if task.dashboard_generation else '未設定'}"
        )
        print(
            f"  抽出パイプライン   : {task.extraction_pipeline.value if task.extraction_pipeline else '未設定'}"
        )
        ...
        if task.metrics:
            print("\\n📈 メトリクス:")
            print(
                f"  LCA               : {task.metrics.lca if task.metrics.lca is not None else '未設定'}"
            )
            print(
                f"  A/B評価率         : {task.metrics.ab_evaluation_rate if task.metrics.ab_evaluation_rate is not None else '未設定'}"
            )
            print(
                f"  FPS               : {task.metrics.fps if task.metrics.fps is not None else '未設定'}"
            )
            print(
                f"  C以上評価率       : {task.metrics.c_plus_rate if task.metrics.c_plus_rate is not None else '未設定'}"
            )
            print(
                f"  平均カバレッジ率   : {task.metrics.avg_coverage_rate if task.metrics.avg_coverage_rate is not None else '未設定'}"
            )
            print(
                f"  平均コンパクトネス : {task.metrics.avg_compactness if task.metrics.avg_compactness is not None else '未設定'}"
            )
            print(
                f"  平均フィル率       : {task.metrics.avg_fill_rate if task.metrics.avg_fill_rate is not None else '未設定'}"
            )
            print(
                f"  SCI               : {task.metrics.sci if task.metrics.sci is not None else '未設定'}"
            )
            print(
                f"  PLA               : {task.metrics.pla if task.metrics.pla is not None else '未設定'}"
            )
            print(
                f"  PLE               : {task.metrics.ple if task.metrics.ple is not None else '未設定'}"
            )
        ...
        try:
            ...
            for task in tasks:
                task_dict = {
                    ...
                    "quality_evaluation": task.quality_evaluation.value
                    if task.quality_evaluation
                    else None,
                    "integrated_script": task.integrated_script.value
                    if task.integrated_script
                    else None,
                    "dashboard_generation": task.dashboard_generation.value
                    if task.dashboard_generation
                    else None,
                    "extraction_pipeline": task.extraction_pipeline.value
                    if task.extraction_pipeline
                    else None,
                }
