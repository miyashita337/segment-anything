...
try:
    from progress_tracker.data_models import PriorityLevel, TaskStatus
...
class SampleTaskRecord:
    ...
    def __init__(
        ...
        self.created_date = (
            datetime.strptime(created_date, "%Y-%m-%d") if created_date else datetime.now()
        )
...
class GoogleSheetsDemo:
    ...
    def _create_sample_data(self) -> List[SampleTaskRecord]:
        ...
        return [
            # Phase 1 残存タスク
            SampleTaskRecord(
                "P1-005", "優先度高", "着手前", "自動マスク修正機能: マスクエッジ自動スムージング・ノイズ除去", "2025-07-27"
            ),
            SampleTaskRecord(
                "P1-006", "優先度高", "着手前", "階層的品質評価: 画像全体→領域→ピクセルレベルの多段階評価", "2025-07-27"
            ),
            ...
            # Phase 1-A 緊急改善タスク
            SampleTaskRecord(
                "P1-A001", "優先度最高", "実装完了", "改善コード復旧: deprecatedから本番環境への復帰", "2025-07-27"
            ),
            ...
            SampleTaskRecord(
                "PH2-003", "優先度中", "着手前", "エラーハンドリング強化: 6種類カスタムエラークラス実装", "2025-07-27"
            ),
            # Phase 3 設定ベース管理
    ...
    def filter_tasks_by_priority(
        self, tasks: List[SampleTaskRecord], priority: str
    ) -> List[SampleTaskRecord]:
        """優先度でフィルタリング"""
    ...
    def filter_tasks_by_status(
        self, tasks: List[SampleTaskRecord], status: str
    ) -> List[SampleTaskRecord]:
        """ステータスでフィルタリング"""
