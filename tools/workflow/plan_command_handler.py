...
class PlanCommandHandler:
    ...
    def validate_inputs(
        self, tracker_id: str, summary: str, details: str
        ...
        if details_length > self.MAX_DETAILS_LENGTH:
            return (
                False,
                f"❌ 詳細が文字数制限を超えています: {details_length:,}文字 (制限: {self.MAX_DETAILS_LENGTH:,}文字)",
            )
    ...
    def create_google_sheets_tracker(
        self,
        tracker_id: str,
        summary: str,
        details: str,
        ...
        for attempt in range(max_retries):
            try:
                ...
                task = self.progress_manager.create_task(
                    tracker_id=tracker_id, description=description
                )
...
"""
                ...
                logger.info(
                    f"Google Sheetsトラッカー作成成功: {tracker_id} (試行 {attempt + 1}/{max_retries})"
                )
...
"""
    ...
    def _get_sheets_url(self) -> str:
        ...
        try:
            if hasattr(self.progress_manager, "config") and hasattr(
                self.progress_manager.config, "sheet_url"
            ):
                return self.progress_manager.config.sheet_url
            elif hasattr(self.progress_manager, "config") and hasattr(
                self.progress_manager.config, "spreadsheet_id"
            ):
    ...
    def execute_plan_command(
        self, tracker_id: str, summary: str, details: str, priority: str = "medium"
    ) -> Tuple[bool, str]:
        """
        planコマンドを実行
