...
class LongTaskQueue:
    ...
    def enqueue_task(self, command: str, task_type: str) -> str:
        ...
        self._update_status_file({"status": "task_enqueued", "task_id": task_id})
    ...
    def _execute_queue(self) -> None:
        ...
        while not self.stop_event.is_set():
            logger.debug(
                f"Queue loop: queue_length={len(self.task_queue)}, current_task={self.current_task is not None}"
            )
    ...
    def _execute_task(self, task: QueueTask) -> None:
        ...
        self._update_status_file(
            {"status": "task_running", "task_id": task.task_id, "command": task.command}
        )
        ...
        try:
            ...
            self._update_status_file(
                {"status": "task_error", "task_id": task.task_id, "error": str(e)}
            )
