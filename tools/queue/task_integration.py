...
class TaskIntegration:
    ...
    def execute_pytest(
        self, test_path: str = "tests/", options: Optional[List[str]] = None, coverage: bool = True
    ) -> str:
        """
        pytest実行タスク登録
        ...
        # 出力ファイル指定
        output_file = (
            self.workspace / "logs" / f"pytest_{datetime.now().strftime('%Y%m%d_%H%M%S')}.log"
        )
    ...
    def execute_custom_command(self, command: str, task_type: str = "custom") -> str:
        """
        カスタムコマンド実行タスク登録
    ...
    def start_monitoring(self, task_id: str) -> Dict[str, Any]:
        ...
        result = self.integration.monitor_long_task(
            task_id=task_id, task_command=f"Queue task {task_id}"
        )
    ...
    def parse_pytest_results(self, output_file: str) -> Dict[str, Any]:
        ...
        try:
            ...
            # 失敗テスト詳細抽出
            failure_matches = re.findall(r"FAILED ([\w/\.]+::\w+)(?:\[[\w-]+\])? - (.+)", content)

            for test_name, error_msg in failure_matches:
                results["failures"].append({"test": test_name, "error": error_msg})
...
class TaskOrchestrator:
    ...
    def run_pytest_with_monitoring(
        self, test_path: str = "tests/", **kwargs
    ...
    def run_extraction_with_monitoring(
        self, input_dir: str, **kwargs
    ) -> Tuple[str, Dict[str, Any]]:
        """
        キャラクター抽出実行と監視
    ...
    def run_quality_check_with_monitoring(
        self, extraction_dir: str, **kwargs
    ) -> Tuple[str, Dict[str, Any]]:
        """
        品質評価実行と監視
    ...
    def run_dashboard_generation(self, tracker_id: str, **kwargs) -> Tuple[str, Dict[str, Any]]:
        """
        ダッシュボード生成実行
    ...
    def generate_final_report(
        self, tracker_id: str = None, phase_results: Dict[str, Any] = None
    ) -> str:
        """
        最終統合レポート生成
...
"""
    ...
    def register_async_task(
        self, input_dir: str, task_type: str = "extraction", **kwargs
    ) -> Dict[str, Any]:
        """
        非同期タスク登録（即座に制御を返す）
    ...
    def collect_task_results(self, task_id: Optional[str] = None) -> Dict[str, Any]:
        ...
        try:
            ...
            final_report = self.generate_final_report(
                tracker_id=self.integration.tracker_id, phase_results=phase_results
            )
            ...
            if extraction_dir.exists():
                result_files = list(extraction_dir.glob("*.jpg")) + list(
...
def main():
    ...
    if len(sys.argv) < 2:
        ...
        if command == "pytest":
            ...
            orchestrator = TaskOrchestrator("QUAL-044")
            input_dir = (
                sys.argv[2] if len(sys.argv) > 2 else "/mnt/c/AItools/lora/train/yado/org/kana05/"
            )
