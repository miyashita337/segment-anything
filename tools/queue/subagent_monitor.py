...
class SubAgentMonitor:
    ...
    def __init__(
        self, workspace_path: str = "/mnt/c/AItools/lora/train/yado/tracker-workspace/QUAL-044"
    ):
        """
        初期化
        ...
        try:
            while self.is_monitoring:
                ...
                if self.last_status != status:
                    ...
                    # タスク完了検出
                    if (
                        status.get("status") == "task_completed"
            ...
            monitoring_result["duration"] = (
                monitoring_result["monitoring_ended"] - monitoring_result["monitoring_started"]
            )
    ...
    def detect_memory_leaks(self) -> Optional[str]:
        ...
        try:
            ...
            memory_increase_percent = (
                (memory_increase / self.memory_baseline) * 100 if self.memory_baseline > 0 else 0
            )
    ...
    def monitor_process_health(self, process_id: Optional[int] = None) -> Optional[str]:
        ...
        try:
            ...
            if process.status() in [psutil.STATUS_ZOMBIE, psutil.STATUS_DEAD]:
                ...
                self._record_anomaly(
                    "process_status_abnormal",
                    {"status": process.status(), "process_id": process.pid},
                )
                return anomaly_msg
    ...
    def _record_anomaly(self, anomaly_type: str, details: Dict[str, Any]) -> None:
        """異常履歴記録"""
        anomaly_record = {"timestamp": time.time(), "type": anomaly_type, "details": details}
    ...
    def get_anomaly_report(self) -> Dict[str, Any]:
        ...
        return {
            ...
            - (self.anomaly_history[0]["timestamp"] if self.anomaly_history else time.time()),
        }
...
class SubAgentIntegration:
    ...
    def monitor_long_task(self, task_id: str, task_command: str) -> Dict[str, Any]:
        ...
        # コールバック登録
        self.monitor.register_callbacks(on_complete=on_complete, on_failed=on_failed)
...
def demonstrate_subagent_monitoring():
    ...
    demo_result = {
        ...
        "next_action_result": {
            ...
            "context": {"tracker_id": "QUAL-044", "session_id": "current_claude_session"},
        },
        "session_context": {
            "tracker_id": "QUAL-044",
            "current_task": "long_task_queue_implementation",
