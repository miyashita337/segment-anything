...
class PushoverNotifier:
    ...
    def send_notification(
        self, title: str, message: str, priority: int = 0, url: Optional[str] = None
    ) -> bool:
        """
        通知送信
...
class TaskFailureEscalator:
    ...
    def create_escalation(
        self, task_id: str, task_type: str, error: str, retry_count: int, command: str
    ) -> Dict[str, Any]:
        """
        タスク失敗エスカレーション作成
...
class NotificationBridge:
    ...
    def handle_task_completion(self, task_id: str, task_type: str, results: Dict[str, Any]) -> None:
        """
        タスク完了ハンドリング
    ...
    def handle_task_failure(
        ...
        self.pushover.send_task_failed(
            task_id=task_id, task_type=task_type, error=error, retry_count=retry_count
        )
    ...
    def _should_suppress_notification(self, notification_hash: str, priority_level: str) -> bool:
        ...
        if priority_level in self.last_notification_times:
            ...
            if time_since_last < effective_interval:
                logger.info(
                    f"Notification suppressed: too frequent for {priority_level} priority ({time_since_last:.0f}s < {effective_interval:.0f}s)"
                )
                return True
    ...
    def _calculate_adaptive_interval(
        self, priority_level: str, priority_config: Dict[str, Any]
    ) -> float:
        """
        アダプティブ間隔計算
    ...
    def _is_burst_allowed(
        self, priority_level: str, priority_config: Dict[str, Any], current_time: float
    ) -> bool:
        """
        バースト許可判定
        ...
        if (
            priority_level not in self.burst_windows
            or current_time - self.burst_windows[priority_level] > burst_window
        ):
            # 新しいバーストウィンドウを開始
    ...
    def send_enhanced_notification(
        ...
        if success:
            ...
            logger.info(
                f"Enhanced notification sent: {priority_level} priority, hash={notification_hash[:8]}"
            )
    ...
    def send_anomaly_notification(self, anomaly_data: Dict[str, Any]) -> bool:
        ...
        for anomaly in anomalies:
            ...
            anomaly_msg = (
                anomaly["message"][:50] + "..."
                if len(anomaly["message"]) > 50
                else anomaly["message"]
            )
    ...
    def send_process_status_notification(
        self, process_status: Dict[str, Any], priority_level: str = "normal"
    ) -> bool:
        """
        プロセス状態通知
...
def demonstrate_notification_bridge():
    ...
    # デモ: タスク完了
    demo_results = {"total_images": 39, "successful": 34, "success_rate": 87.2}
    bridge.handle_task_completion(
        task_id="extract_20250830_164000", task_type="extract_character", results=demo_results
    )
    ...
    if len(sys.argv) < 2:
        ...
        if command == "test-pushover":
            ...
            success = bridge.pushover.send_notification(
                title="🧪 Test Notification", message="QUAL-044 Notification Bridge Test", priority=0
            )
