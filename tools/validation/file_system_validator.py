...
class FileSystemValidator:
    ...
    def validate_branch_verification(self, tracker_id: str) -> ValidationResult:
        ...
        try:
            ...
            if current_branch == expected_pattern or current_branch.startswith(f"feature/{tracker_id}"):
                return ValidationResult(
                    True, 
                    [], 
                    f"correct_branch={current_branch}"
                )
            else:
                return ValidationResult(
                    False,
            ...
            if result.returncode == 0:
                ...
                if 'sam-env' in python_path:
                    return ValidationResult(
                        True, 
                        [], 
                        f"venv={virtual_env},python={python_path}"
                    )
                else:
                    return ValidationResult(
                        False,
            ...
            if result.returncode == 0:
                ...
                if tracker_id in output and ("着手中" in output or "進行中" in output or "実装中" in output):
                    return ValidationResult(
                        True, 
                        [], 
                        f"sheets_sync_verified,status_contains={tracker_id}"
                    )
                else:
                    return ValidationResult(
                        False,
    ...
    def validate_sow_creation(self, tracker_id: str) -> ValidationResult:
        ...
        try:
            ...
            return ValidationResult(
                True, 
                [], 
                f"sow_file={sow_file},content_length={len(content)},sections_complete"
            )
    ...
    def validate_implementation(self, tracker_id: str) -> ValidationResult:
        ...
        try:
            ...
            if not commits or not any(tracker_id in commit for commit in commits):
                return ValidationResult(
                    False,
                    [f"No commits found for {tracker_id}"],
                    f"recent_commits={len(commits)}"
                )
            ...
            if result.returncode == 0:
                ...
                if not modified_files:
                    return ValidationResult(
                        False,
                        ["No files modified compared to main branch"],
                        "no_file_changes"
                    )
                ...
                return ValidationResult(
                    True,
                    [],
                    f"commits={len(commits)},modified_files={len(modified_files)}"
                )
            else:
    ...
    def validate_testing(self, tracker_id: str) -> ValidationResult:
        ...
        if not test_evidence:
            ...
            return ValidationResult(
                ...
                f"checked_paths={test_evidence_paths}"
            )
                ...
                if evidence_file.endswith('.json'):
                    ...
                    if 'passed' in test_data or 'success' in test_data:
                        return ValidationResult(
                            True,
                            [],
                            f"test_results={evidence_file},data_available"
                        )
            
            return ValidationResult(
                True,
                [],
                f"test_evidence_found={len(test_evidence)}"
            )
    ...
    def validate_extraction_completion(self, tracker_id: str) -> ValidationResult:
        ...
        if not os.path.exists(extraction_dir):
            return ValidationResult(
                ...
                f"expected_path={extraction_dir}"
            )
        ...
        if not os.path.exists(result_file):
            return ValidationResult(
                False,
                ["extraction_result.json not found"],
                f"expected_path={result_file}"
            )
    ...
    def validate_quality_workflow_completion(self, tracker_id: str) -> ValidationResult:
        ...
        This checks for dashboard generation and quality analysis.
        """
        workspace_path = os.path.join(self.workspace_base, tracker_id)
        
        # Check for dashboard file
        dashboard_paths = [
            os.path.join(workspace_path, "dashboard", "dashboard.html"),
            os.path.join(workspace_path, "dashboard.html"),
            os.path.join(workspace_path, "quality", "dashboard.html")
        ]
        ...
        if not dashboard_file:
            return ValidationResult(
                ...
                f"checked_paths={dashboard_paths}"
            )
        ...
        try:
            ...
            missing_sections = []
            for section in required_sections:
                if section not in dashboard_content:
                    missing_sections.append(section)
            
            if missing_sections:
                return ValidationResult(
                    False,
                    [f"Dashboard missing required sections: {missing_sections}"],
            ...
            if not quality_report:
                return ValidationResult(
                    False,
                    ["Quality report JSON not found"],
                    f"dashboard_file={dashboard_file},checked_quality_paths={quality_report_paths}"
                )
            ...
            return ValidationResult(
                True,
                [],
                f"dashboard_file={dashboard_file},quality_report={quality_report},sections_complete"
            )
    ...
    def validate_dashboard_generation(self, tracker_id: str) -> ValidationResult:
        ...
        try:
            ...
            try:
                ...
                request.add_header('Authorization', 'Basic YWRtaW46c2VjdXJlX3RyYWNrXzIwMjVfcTNfOGY5YQ==')
