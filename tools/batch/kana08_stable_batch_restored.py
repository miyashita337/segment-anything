...
class Kana08StableExtractor:
    ...
    def __init__(self, input_dir=None, output_dir=None):
        self.input_dir = (
            Path(input_dir) if input_dir else Path("/mnt/c/AItools/lora/train/yado/org/kana08")
        )
        self.output_dir = (
            Path(output_dir)
            if output_dir
            else Path("/mnt/c/AItools/lora/train/yado/clipped_boundingbox/workspace/P1-A001")
        )
    ...
    def extract_character(
        ...
        try:
            ...
            masks, scores, _ = self.sam_predictor.predict(box=box_prompt, multimask_output=True)
    ...
    def process_image(self, image_path: Path) -> Tuple[bool, Optional[str], Optional[Dict]]:
        ...
        try:
            ...
            # 透過背景を白背景に変換
            if extracted_img.shape[2] == 4:
                # Alpha blending for white background
                alpha = extracted_img[:, :, 3] / 255.0
                for c in range(3):
                    extracted_img[:, :, c] = extracted_img[:, :, c] * alpha + 255 * (1 - alpha)
                extracted_img = extracted_img[:, :, :3]
...
def main():
    ...
    extractor = Kana08StableExtractor(args.input_dir, args.output_dir)
