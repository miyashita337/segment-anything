...
def demo_single_evaluation():
    ...
    result = evaluate_hierarchical_quality(
        image, mask, item_id="demo_single", bbox=(35, 35, 80, 80)
    )
    ...
    for score in result.scores:
        print(
            f"  {score.level.value:>8} {score.metric.value:>15}: {score.score:.3f} (信頼度: {score.confidence:.2f})"
        )
    ...
    for level, summary in result.level_summaries.items():
        print(
            f"  {level:>8}: 平均スコア {summary['average_score']:.3f}, メトリクス数 {summary['metric_count']}"
        )
...
def demo_batch_evaluation():
    ...
    for i, result in enumerate(results):
        ...
        print(
            f"{result.item_id:>20} {result.overall_score:>8.3f} {result.overall_grade:>8} {expected:>10} {rec_count:>10}"
        )
...
def demo_level_detail_analysis(results):
    ...
    for level_name, stats in level_stats.items():
        print(
            f"{level_name:>10} {stats['count']:>6} {stats['average']:>8.3f} {stats['std']:>8.3f} {stats['min']:>8.3f} {stats['max']:>8.3f}"
        )
