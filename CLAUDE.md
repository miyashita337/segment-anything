...
**新規ドキュメント作成時の判断基準**:
1. **チェックリスト**: 実行時の確認項目 → `checklists/`
2. **テンプレート**: 作業用フォーマット → `templates/`
3. **ガイド・手順**: プロセス説明 → `workflows/`

⚠️ **重要**: 新規ドキュメント作成時は必ず上記フォルダ構造に従い、既存の類似ファイルがないか確認すること。重複作成を発見した場合は即座に統合・整理を実行する。
