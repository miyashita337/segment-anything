...
## 🚨 エラー時スキル参照必須ルール（KIRO-025制定）

**エラーメッセージに `/workflow-troubleshoot` または skill参照誘導が含まれる場合:**

1. **必ず該当スキルを参照すること**（Skillツールで実行）
2. スキル参照せずに独自対処は**禁止**
3. スキルに解決策がない場合のみユーザーに確認

**違反例（禁止）:**
- エラー発生 → 独自判断で再試行
- 「/workflow-troubleshoot」表示 → 無視して続行

**正しい対処:**
- エラー発生 → `/workflow-troubleshoot` 実行 → スキル内容確認 → 対処
...
### 🎯 抽出実行必須ルール（KIRO-025制定）

**subagent_extractionステップでのキャラクター抽出はデフォルト必須です**

- ❌ **禁止**: 「システム改良だから抽出不要」等の独断判断
- ❌ **禁止**: 抽出ステップの無断スキップ
- ✅ **必須**: 抽出をスキップする場合はユーザーの明示的な承認が必要

**理由**: 抽出不要の独断判断はワークフロー違反であり、品質保証プロセスの破綻を招く
