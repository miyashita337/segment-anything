...
#### 表示項目（7項目・表示順序）
1. **Current(平均品質スコア)**: 現在のトラッカーの品質スコア平均値
   - データソース: extraction_result.json の全画像品質スコア平均
   - 計算方法: np.mean(current_data)
   - 表示形式: 小数点第3位まで（例：0.735）
   - カラー: blue-600

2. **BaseLine**: 比較対象となるベースライントラッカーの品質スコア
   - データソース: ベースライントラッカーIDから算出された品質スコア
   - 表示形式: 小数点第3位まで（例：0.652）
   - カラー: gray-600

3. **p値**: 統計的有意性検定結果
   - 計算方法: ウェルチのt検定
   - 表示形式: 小数点第3位まで（例：0.042）
   - カラー: indigo-600

4. **効果サイズ、Cohen's d**: Cohen's d値による効果量評価
   - 計算方法: (current_mean - baseline_mean) / pooled_std
   - 表示形式: 小数点第2位まで（例：1.23）
   - カラー: purple-600

5. **改善率**: ベースラインからの改善パーセンテージ
   - 計算方法: ((current - baseline) / baseline) * 100
   - 表示形式: +XX.X%形式（例：+12.8%）
   - カラー: green-600

6. **統計的有意性**: 有意/非有意の判定結果
   - 判定基準: p < 0.05で「有意」、p >= 0.05で「非有意」
   - 表示形式: 日本語テキスト
   - カラー: red-600

7. **信頼区間**: 品質スコアの95%信頼区間
   - 計算方法: mean ± 1.96 * (std / √n)
   - 表示形式: (下限, 上限)形式（例：(0.68, 0.79)）
   - カラー: teal-600
...
**更新履歴**:
- 2025-08-27: ダッシュボード統計分析結果仕様追加（7項目順序定義）
