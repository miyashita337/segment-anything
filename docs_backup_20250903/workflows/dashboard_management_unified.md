...
**Base64画像埋め込み禁止**: CLAUDE.mdセキュリティ原則により、ダッシュボード内でのBase64画像埋め込みは厳禁です。

### 準拠要件
- ✅ **相対パス参照**: 画像は相対パス（`/TRACKER_ID/extraction/image.jpg`）で参照
- ❌ **Base64埋め込み**: `data:image/jpeg;base64,...` 形式での画像埋め込み禁止
- ✅ **軽量HTML**: Base64排除により994KB→8KB（99.2%削減）の軽量化達成

### 違反時の対処
Base64埋め込みが発見された場合は即座に相対パス参照方式に修正してください。
