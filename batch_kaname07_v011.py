#!/usr/bin/env python3
"""
kaname07 10枚バッチ処理スクリプト（v0.1.1版）
適応学習システムによる最高品質抽出
"""
...
def main():
    ...
    print(f"🚀 kaname07 10枚バッチ処理開始（v0.1.1）")
    ...
    try:
        ...
        print(f"✅ v0.1.1 extract_character_from_path import成功")
        
        # 適応学習システム確認
        from utils.learned_quality_assessment import assess_image_quality
        print(f"✅ v0.1.1 適応学習システム利用可能")
        
    except ImportError as e:
        print(f"❌ エラー: v0.1.1システムの必要モジュールが見つかりません: {e}")
        sys.exit(1)
    
    # モデル初期化
    print(f"🔄 v0.1.1システム初期化中...")
    from hooks.start import start
    start()
    print(f"✅ v0.1.1システム初期化完了")
    ...
    for i, image_name in enumerate(target_images, 1):
        ...
        output_file_path = Path(output_path) / f"v011_batch_{i:02d}_{Path(image_name).stem}.jpg"
        
        try:
            start_time = time.time()
            
            # v0.1.1最高品質設定（適応学習システム利用）
            result = extract_character_from_path(
                ...
                multi_character_criteria='size_priority',  # 推奨手法
    ...
    # 生成ファイル確認
    generated_files = list(Path(output_path).glob("v011_batch_*"))
    print(f"\\n💾 生成ファイル: {len(generated_files)}個")
    
    # v0.1.1 vs v0.1.0比較
    print(f"\\n🔍 v0.1.1 vs v0.1.0比較:")
    print(f"   v0.1.1成功率: {success_rate:.1f}%")
    print(f"   v0.1.0成功率: 80.0%（参考）")
    
    # 品質判定
    if success_rate >= 80:
        print(f"\\n🎉 v0.1.1バッチ処理成功!")
        print(f"   最高品質抽出完了（成功率 {success_rate:.1f}%）")
    elif success_rate >= 60:
        print(f"\\n✅ v0.1.1バッチ処理良好")
        print(f"   実用可能レベル（成功率 {success_rate:.1f}%）")
    else:
        print(f"\\n🚨 v0.1.1バッチ処理で品質問題発生")
        print(f"   成功率 {success_rate:.1f}% は期待値を下回ります")
