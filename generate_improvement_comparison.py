...
class ImprovementComparisonSystem:
    ...
    def calculate_improvement(self, baseline_value: float, current_value: float) -> Dict[str, Any]:
        ...
        if baseline_value == 0:
            if current_value == 0:
                return {"absolute_change": 0, "percent_change": 0, "improvement_type": "no_change"}
            else:
                return {
    ...
    def create_comparison_chart(self, comparison_report: Dict, output_path: str):
        ...
        bars1 = ax1.bar(
            x - width / 2, baseline_values, width, label="ベースライン", alpha=0.8, color="skyblue"
        )
        bars2 = ax1.bar(
            x + width / 2, current_values, width, label="現在", alpha=0.8, color="lightcoral"
        )
        ...
        ax1.set_xticks(x)
        ax1.set_xticklabels(
            [name.replace(" ", "\n") for name in metric_names], rotation=45, ha="right", fontsize=8
        )
        ax1.legend()
        ...
        # 2. 改善率チャート
        colors = [
            "green" if rate > 0 else "red" if rate < 0 else "gray" for rate in percent_changes
        ]
        bars = ax2.bar(range(len(metric_names)), percent_changes, color=colors, alpha=0.7)
...
"""
    ...
    def _get_status_emoji(self, status: str) -> str:
        """ステータス絵文字取得"""
        emoji_map = {"improvement": "🚀", "degradation": "⚠️", "mixed": "📊"}
        return emoji_map.get(status, "❓")

    def _get_improvement_emoji(self, improvement_type: str) -> str:
        """改善タイプ絵文字取得"""
        emoji_map = {"improvement": "✅", "degradation": "❌", "no_change": "➖"}
        return emoji_map.get(improvement_type, "❓")


def main():
    parser = argparse.ArgumentParser(description="改善効果測定システム")
    parser.add_argument("--baseline", required=True, help="ベースライン結果ディレクトリ")
    parser.add_argument("--current", required=True, help="現在の結果ディレクトリ")
    parser.add_argument("--output", required=True, help="改善レポート出力ファイル (JSON)")
    parser.add_argument("--chart", help="比較チャート出力パス (PNG)")
    parser.add_argument("--summary", help="サマリーテキスト出力パス")
