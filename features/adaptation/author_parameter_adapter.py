...
class AuthorParameterAdapter:
    ...
    def _get_default_config(self) -> Dict[str, Any]:
        ...
        return {
            ...
            "detection": {"known_authors": ["yado", "kiri", "zundamon"]},
        }
    ...
    _LEGACY_AUTHOR_PROFILES = {
        ...
        "kiri": AuthorProfile(
            ...
            score_threshold=0.05,
            description="kiri作者: 細密描写特化・高品質重視（元aichi）",
            processing_notes=["細かい線画・詳細な描写", "高品質抽出を優先", "処理時間よりも品質重視", "背景との分離に注意"],
        ),
        "zundamon": AuthorProfile(
            ...
            score_threshold=0.08,
            description="zundamon作者: シンプルスタイル・効率重視",
            processing_notes=["シンプルで明確な線画", "高速処理可能", "効率的なバッチ処理", "安定した基本品質"],
        ),
    }
    ...
    DEFAULT_PROFILE = AuthorProfile(
        ...
        processing_notes=["汎用設定", "未知の作者・データセットに対応", "標準的な品質・速度バランス"],
    )
    ...
    def get_optimized_parameters(cls, author_id: Optional[str]) -> Dict[str, Any]:
        ...
        logger.info(
            f"⚙️ Optimized parameters for {profile.author_id}: "
    ...
    def apply_author_optimization(cls, image_path: str) -> Dict[str, Any]:
        ...
        logger.info(
            f"🎯 Author optimization applied for {image_path}: " f"Author={author_id or 'default'}"
        )
    ...
    def get_author_statistics(cls) -> Dict[str, Any]:
        ...
        stats = {
            ...
            "confidence_range": {
                ...
                "avg": sum(p.yolo_confidence for p in cls._LEGACY_AUTHOR_PROFILES.values())
                / len(cls._LEGACY_AUTHOR_PROFILES),
            },
        }
        ...
        for profile in cls._LEGACY_AUTHOR_PROFILES.values():
            ...
            stats["characteristics_distribution"][char] = (
                stats["characteristics_distribution"].get(char, 0) + 1
            )
...
if __name__ == "__main__":
    ...
    print(
        f"Confidence Range: {stats['confidence_range']['min']:.3f} - {stats['confidence_range']['max']:.3f}"
    )
