...
class PixelLevelEvaluator:
    ...
    def evaluate(self, image: np.ndarray, mask: np.ndarray) -> List[QualityScore]:
        ...
        scores.append(QualityScore(
            QualityMetric.COMPLETENESS, QualityLevel.PIXEL,
        ...
        scores.append(QualityScore(
            QualityMetric.ACCURACY, QualityLevel.PIXEL,
            edge_accuracy['score'], edge_accuracy['confidence'],
            edge_accuracy
        ))
        ...
        scores.append(QualityScore(
            QualityMetric.CLARITY, QualityLevel.PIXEL,
            clarity['score'], clarity['confidence'],
            clarity
        ))
...
class ObjectLevelEvaluator:
    ...
    def evaluate(self, image: np.ndarray, mask: np.ndarray, 
                 bbox: Optional[Tuple] = None) -> List[QualityScore]:
        """オブジェクトレベル評価実行"""
        scores = []
        ...
        scores.append(QualityScore(
            QualityMetric.COMPLETENESS, QualityLevel.OBJECT,
            completeness['score'], completeness['confidence'],
            completeness
        ))
        ...
        scores.append(QualityScore(
            QualityMetric.RELEVANCE, QualityLevel.OBJECT,
            relevance['score'], relevance['confidence'],
            relevance
        ))
        
        return scores
    
    def _evaluate_object_completeness(self, image: np.ndarray, mask: np.ndarray,
                                      bbox: Optional[Tuple]) -> Dict[str, float]:
        """オブジェクト完全性評価"""
        if mask.size == 0:
            return {'score': 0.0, 'confidence': 1.0}
...
class HierarchicalQualityEvaluator:
    ...
    def evaluate(self, item_id: str, image: np.ndarray, mask: np.ndarray,
                 bbox: Optional[Tuple] = None,
                 metadata: Optional[Dict] = None) -> HierarchicalQualityResult:
        """階層的品質評価実行"""
        ...
        try:
            ...
            logger.info(f"✅ 階層的品質評価完了: {item_id} (スコア: {result.overall_score:.3f}, グレード: {result.overall_grade})")
...
# 使いやすいインターフェース関数
def evaluate_hierarchical_quality(image: np.ndarray, mask: np.ndarray,
                                  item_id: str = "unknown",
                                  bbox: Optional[Tuple] = None) -> HierarchicalQualityResult:
