...
class VisualExplainer:
    ...
    def create_quality_breakdown_chart(self, item_id: str, factors: List[QualityFactor]) -> str:
        ...
        for i, (bar, value) in enumerate(zip(bars, values)):
            plt.text(bar.get_width() + 0.01, bar.get_y() + bar.get_height()/2,
                    f'{value:.3f}', va='center', fontproperties=self.font_prop)
        
        # 凡例
        from matplotlib.lines import Line2D
        legend_elements = [
            Line2D([0], [0], marker='s', color='w', markerfacecolor='red', markersize=10, alpha=0.7, label='低品質 (0.0-0.3)'),
            Line2D([0], [0], marker='s', color='w', markerfacecolor='yellow', markersize=10, alpha=0.7, label='中品質 (0.3-0.7)'),
            Line2D([0], [0], marker='s', color='w', markerfacecolor='green', markersize=10, alpha=0.7, label='高品質 (0.7-1.0)')
        ]
    ...
    def create_image_annotation(self, item_id: str, image: np.ndarray, mask: np.ndarray,
                               factors: List[QualityFactor]) -> str:
        """画像アノテーション生成"""
        ...
        fig.text(0.98, 0.02, textstr, fontsize=10, verticalalignment='bottom',
                horizontalalignment='right', bbox=props, fontproperties=self.font_prop)
    ...
    def create_comparison_chart(self, results: List[ExplanationResult]) -> str:
        ...
        # 軸設定
        plt.xticks(range(len(all_factor_names)), all_factor_names, 
...
class ExplainableQualityEvaluator:
    ...
    def evaluate_with_explanation(self, item_id: str, image: np.ndarray, mask: np.ndarray,
                                  quality_scores: Dict[str, float],
                                  bbox: Optional[Tuple] = None) -> ExplanationResult:
        """説明付き品質評価"""
        ...
        try:
            ...
            logger.info(f"✅ 説明可能品質評価完了: {item_id} (スコア: {result.overall_score:.3f}, 信頼性: {result.confidence:.3f})")
    ...
    def _analyze_quality_factors(self, image: np.ndarray, mask: np.ndarray,
                                quality_scores: Dict[str, float],
                                bbox: Optional[Tuple]) -> List[QualityFactor]:
        """品質要因分析"""
        factors = []
    ...
    def _generate_visual_explanations(self, result: ExplanationResult,
                                      image: np.ndarray, mask: np.ndarray) -> Dict[str, str]:
        """ビジュアル説明生成"""
        visual_paths = {}
    ...
    def save_explanations(self, output_path: Path):
        ...
        summary = {
            ...
            'average_score': np.mean([r.overall_score for r in self.evaluation_history]) if self.evaluation_history else 0.0,
            'average_confidence': np.mean([r.confidence for r in self.evaluation_history]) if self.evaluation_history else 0.0,
...
# 使いやすいインターフェース関数
def explain_quality_evaluation(image: np.ndarray, mask: np.ndarray,
                               quality_scores: Dict[str, float],
                               item_id: str = "unknown",
                               output_dir: Optional[Path] = None) -> ExplanationResult:
