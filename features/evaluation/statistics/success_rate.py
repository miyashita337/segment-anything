...
class UnifiedSuccessRateCalculator:
    ...
    def calculate_unified_stats(
        self, input_directories: List[str], extraction_dir: str
    ) -> ExtractionStats:
        """
        統一成功率統計計算
        数学的に正確な成功率と信頼区間を算出
        ...
        if successful_extractions > total_input:
            self.logger.error(f"❌ 数学的矛盾検出: 成功数({successful_extractions}) > 入力数({total_input})")
            # 入力数で成功数を制限（安全策）
    ...
    def generate_quality_report(
        self, stats: ExtractionStats, output_path: Optional[str] = None
    ) -> Dict[str, any]:
        """
        品質レポート生成（統一フォーマット）
        ダッシュボード生成用の標準データ
        ...
        report = {
            ...
            "compliance": {
                "mathematical_consistency": stats.successful_extractions
                <= stats.total_input_images,
...
def calculate_qcc021_corrected_stats(
    tracker_id: str = "QCC-FIX-001", input_dirs: List[str] = None, extraction_dir: str = None
) -> ExtractionStats:
    """
    ...
    if extraction_dir is None:
        extraction_dir = (
            f"/mnt/c/AItools/lora/train/yado/tracker-workspace/{tracker_id}/extraction/"
        )
