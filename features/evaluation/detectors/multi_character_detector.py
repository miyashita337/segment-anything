...
class MultiCharacterDetector:
    ...
    def _process_yolo_results(
        self, results, image_shape: tuple, image: np.ndarray
    ...
    def _fallback_region_detection(
        self, image_shape: tuple, image: np.ndarray = None
    ) -> List[CharacterRegion]:
        """YOLO失敗時のフォールバック領域検出"""
        h, w = image_shape
        regions = []
    ...
    def _analyze_overlaps(self, regions: List[CharacterRegion]) -> dict:
        """キャラクター領域の重複分析"""
        if len(regions) < 2:
            return {"has_overlaps": False, "overlap_ratio": 0.0, "overlap_pairs": []}
    ...
    def _build_detection_result(
        self, regions: List[CharacterRegion], overlap_info: dict
    ) -> MultiCharacterDetectionResult:
        """最終的な検出結果を構築"""
    ...
    def visualize_detection(
        self, image: np.ndarray, result: MultiCharacterDetectionResult
    ) -> np.ndarray:
        """検出結果を可視化"""
