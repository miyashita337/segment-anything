...
class PartialExtractionDetector:
    ...
    def _analyze_completeness(self, image: np.ndarray, body_parts: Dict) -> Dict:
        ...
        completeness_score = sum(
            part_weights[part] for part in ['head', 'torso', 'arms', 'legs'] 
            if body_parts[part]
        )
        ...
        elif body_parts['head'] and body_parts['torso'] and body_parts['arms'] and not body_parts['legs']:
    ...
    def _analyze_fragmentation(self, image: np.ndarray) -> Dict:
        ...
        is_fragmented = (significant_fragments > 3) or (fragmentation_level > self.fragmentation_threshold)
    ...
    def _analyze_aspect_ratio(self, image: np.ndarray) -> Dict:
        ...
        aspect_ratio_warning = (
            aspect_ratio < 1.0 or  # 横長すぎ
            aspect_ratio > 4.0 or  # 縦長すぎ
            ratio_deviation > self.ideal_aspect_ratio * 0.5
        )
    ...
    def _integrate_analysis_results(self, basic_analysis, body_parts, completeness_analysis,
                                   fragmentation_analysis, aspect_ratio_analysis) -> PartialExtractionResult:
        """解析結果の統合"""
        # 部分抽出の判定
        is_partial = (
            completeness_analysis['completeness_score'] < self.completeness_threshold or
            fragmentation_analysis['is_fragmented'] or
            aspect_ratio_analysis['aspect_ratio_warning']
        )
    ...
    def _generate_recommendations(self, completeness_analysis, fragmentation_analysis,
                                 aspect_ratio_analysis) -> List[str]:
        """改善推奨事項の生成"""
        recommendations = []
        ...
        if completeness_analysis['completeness_score'] < 0.7:
            ...
            if 'legs' in completeness_analysis['missing_parts']:
                recommendations.append("Increase detection area to capture full body including legs")
