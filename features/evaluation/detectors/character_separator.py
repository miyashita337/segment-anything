...
class CharacterSeparator:
    ...
    def __init__(self, 
                 min_separation_distance: int = 20,
                 overlap_resolution_method: str = 'watershed'):
        """
    ...
    def _create_individual_masks(self, image: np.ndarray, 
                                regions: List[CharacterRegion]) -> List[np.ndarray]:
        """個別キャラクターのマスクを作成"""
        h, w = image.shape[:2]
        individual_masks = []
    ...
    def _resolve_overlaps(self, image: np.ndarray, 
    ...
    def _watershed_separation(self, image: np.ndarray, 
                             masks: List[np.ndarray]) -> List[np.ndarray]:
        """ウォーターシェッド法による分離"""
        # 全体マスクの作成
    ...
    def _extract_character_images(self, image: np.ndarray, 
                                 masks: List[np.ndarray]) -> List[np.ndarray]:
    ...
    def _evaluate_separation_quality(self, original_image: np.ndarray,
                                    masks: List[np.ndarray],
                                    character_images: List[np.ndarray]) -> List[float]:
        """分離品質の評価"""
        quality_scores = []
    ...
    def create_individual_masks(self, image: np.ndarray, 
                               detection_result) -> List[np.ndarray]:
