...
class BrightnessAnalyzer:
    ...
    def calculate_brightness_percentiles(self, image: np.ndarray, percentiles: List[float]) -> Dict[str, float]:
        """
        指定したパーセンタイルの明度値を計算
    ...
    def analyze_dark_regions(self, image: np.ndarray, dark_threshold: float = 30.0) -> Dict[str, float]:
        """
        暗い領域の分析
        ...
        dark_region_stats = {
            ...
            'dark_region_mean_brightness': float(np.mean(gray[dark_mask])) if dark_pixels > 0 else 0.0,
