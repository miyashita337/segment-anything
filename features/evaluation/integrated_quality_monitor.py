...
class IntegratedQualityMonitor:
    ...
    def _evaluate_comprehensive_quality(
        self,
        mask: np.ndarray,
        original_image: Optional[np.ndarray] = None,
    ...
    def _calculate_objective_metrics(
        ...
        try:
            ...
            sci_result = sci_calculator.calculate(
                original_image if original_image is not None else mask
            )
    ...
    def _determine_quality_status(self, quality_metrics: Dict[str, float]) -> str:
        ...
        try:
            ...
            if overall_score >= self.thresholds.overall_quality:
                ...
                overall_score >= self.thresholds.overall_quality * self.thresholds.warning_threshold
            ):
                return "warning"
            else:
                return "critical"
    ...
    def _generate_improvement_suggestions(
        self, quality_metrics: Dict[str, float], status: str
    ) -> List[str]:
        """改善提案生成"""
        suggestions = []
    ...
    def _check_quality_alerts(self, snapshot: QualitySnapshot) -> None:
        ...
        try:
            if snapshot.status == "critical":
                logger.warning(
                    f"🚨 品質危険レベル: {snapshot.image_path} - スコア {snapshot.overall_score:.3f}"
                )

                # 連続危険レベルチェック
                recent_critical = sum(
                    1 for s in self.quality_history[-5:] if s.status == "critical"
                )
    ...
    def _get_processing_recommendations(
        self, status: str, quality_metrics: Dict[str, float]
    ) -> Dict[str, Any]:
        """処理推奨設定取得"""
        try:
            recommendations = {
...
if __name__ == "__main__":
    ...
    thresholds = QualityThresholds(
        overall_quality=0.50, compactness=0.50, largest_char_accuracy=0.80
    )
