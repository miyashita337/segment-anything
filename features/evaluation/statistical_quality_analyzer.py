...
class StatisticalQualityAnalyzer:
    ...
    def load_extraction_results(self, tracker_id: str) -> QualityMetrics:
        ...
        if not quality_scores:
            raise ValueError(
                f"品質スコアデータが見つかりません: {tracker_id}. JSONファイルに 'extraction_results' または 'results' キーが必要です"
            )
    ...
    def compare_trackers(
        self, tracker_a: str, tracker_b: str, metric: str = "quality_score"
    ) -> TTestResult:
        """
        2つのトラッカーの品質を統計的に比較
    ...
    def analyze_improvement(self, baseline_tracker: str, improved_tracker: str) -> Dict:
        """
        改善効果を分析
        ...
        """
        # 品質スコアの比較
        quality_result = self.compare_trackers(baseline_tracker, improved_tracker, "quality_score")

        # 処理時間の比較（あれば）
        try:
            time_result = self.compare_trackers(
                baseline_tracker, improved_tracker, "extraction_time"
            )
        except:
            time_result = None
        ...
        quality_improvement = (
            ...
            / baseline_metrics.mean_quality_score
            * 100
            if baseline_metrics.mean_quality_score > 0
            else 0
        )

        success_rate_improvement = improved_metrics.success_rate - baseline_metrics.success_rate
        ...
        if time_result:
            ...
            analysis["time_comparison"] = {
                ...
                "speedup": baseline_mean_time / improved_mean_time
                if improved_mean_time > 0
                else 1.0,
    ...
    def batch_compare(self, tracker_ids: List[str], baseline: Optional[str] = None) -> Dict:
        """
        複数トラッカーのバッチ比較
        ...
        comparison_results = self.validator.compare_multiple_groups(
            quality_scores, baseline=baseline
        )
