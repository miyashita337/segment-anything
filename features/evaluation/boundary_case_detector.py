...
class BoundaryCaseDetector:
    ...
    def calculate_boundary_confidence(
        self, quality_metrics: Dict[str, float], case_type: BoundaryType
    ) -> float:
        """
        境界ケース信頼度計算
        ...
        if case_type == BoundaryType.POSE_BOUNDARY:
            ...
            # アスペクト比の極端さ
            aspect_extremity = max(abs(aspect_ratio - 1.0) / 2.0, 0.0)  # 1.0からの乖離
            ...
            if total_pixels < self.QUALITY_THRESHOLDS["size_small"]:
                return min(
                    (self.QUALITY_THRESHOLDS["size_small"] - total_pixels)
                    / self.QUALITY_THRESHOLDS["size_small"],
                    1.0,
                )
            elif total_pixels > self.QUALITY_THRESHOLDS["size_large"]:
                return min(
                    (total_pixels - self.QUALITY_THRESHOLDS["size_large"])
                    / self.QUALITY_THRESHOLDS["size_large"],
                    1.0,
                )
            return 0.0
        ...
        try:
            ...
            detectors = [
                ...
                (
                    ...
                    lambda: self.detect_quality_boundary(quality_metrics),
                ),
                (
                    BoundaryType.MULTIPLE_CHARACTER_BOUNDARY,
                    lambda: self.detect_multiple_character_boundary(image_path),
                ),
            ]
                ...
                try:
                    ...
                    if is_boundary:
                        ...
                        if case_type == BoundaryType.MULTIPLE_CHARACTER_BOUNDARY:
                            ...
                            confidence = self.calculate_boundary_confidence(
                                quality_metrics, case_type
                            )
    ...
    def process_directory(self, input_dir: Path) -> BoundaryAnalysisResult:
        ...
        for image_path in image_files:
            ...
            if processed_count % 10 == 0:
                logger.info(
                    f"📊 進捗: {processed_count}/{len(image_files)} "
        ...
        logger.info(
            f"✅ 境界ケース検出完了: {len(boundary_cases)}/{processed_count}枚 "
    ...
    def save_results(
        self, result: BoundaryAnalysisResult, output_dir: Optional[Path] = None
    ) -> Path:
        """
        結果保存
        ...
        result_data = {
            ...
            "boundary_cases": [
                {**asdict(case), "case_type": case.case_type.value}  # Enumを文字列に変換
    ...
    def _generate_summary_report(self, result: BoundaryAnalysisResult, output_file: Path):
        ...
        lines.append(
            f"- **境界ケース率**: {len(result.boundary_cases)/max(result.total_images, 1)*100:.1f}%"
        )
        lines.append("")
        ...
        if result.improvement_priorities:
            ...
            for i, priority in enumerate(result.improvement_priorities[:5], 1):
                lines.append(
                    f"### {i}. {priority['case_type']} (スコア: {priority['priority_score']:.2f})"
                )
        ...
        if result.boundary_cases:
            ...
            # 高信頼度順にソート
            sorted_cases = sorted(
                result.boundary_cases, key=lambda x: x.confidence_score, reverse=True
            )
