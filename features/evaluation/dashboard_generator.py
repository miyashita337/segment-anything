...
def main():
    """メイン実行"""
    parser = argparse.ArgumentParser(description="評価用ダッシュボード生成 - 統一ダッシュボードシステム経由")
    parser.add_argument("--tracker-id", default="UNKNOWN", help="トラッカーID（デフォルト: UNKNOWN）")
    parser.add_argument("--workspace-dir", help="ワークスペースディレクトリ（省略時は自動推定）")
    parser.add_argument("--extraction-result", help="抽出結果JSONファイルパス（省略時は自動推定）")
    parser.add_argument(
        "--mode",
    ...
    try:
        ...
        # 統一ダッシュボード生成器初期化
        generator = DashboardGenerator(workspace_dir=workspace_dir, tracker_id=args.tracker_id)
