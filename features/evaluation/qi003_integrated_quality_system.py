...
class QI003IntegratedQualitySystem:
            ...
            if quality_score < self.quality_threshold:
                if black_result.is_black_screen:
                    recommendations.append(
                        f"Image {i}: Black screen detected, consider brightness enhancement"
                    )
                else:
                    recommendations.append(
                        f"Image {i}: Low quality detected, review extraction parameters"
                    )
    ...
    def compare_quality_improvements(
        self, qi002_stats: Dict, qi003_stats: Dict
    ) -> Dict[str, float]:
        """
        QI-002とQI-003の品質比較
    ...
    def execute_full_workflow(
        self, input_images: List[np.ndarray], output_dir: Optional[Path]
    ) -> Dict[str, Any]:
        """
        QI-003完全ワークフロー実行
