...
class ImageDuplicateDetector:
    ...
    def _load_hash_cache(self):
        ...
        try:
            with open(self.hash_cache_file, 'r', encoding='utf-8') as f:
                ...
                self.image_hashes = {
                    path: DuplicateInfo(**info) 
                    for path, info in cache_data.items()
                }
    ...
    def _save_hash_cache(self):
        ...
        try:
            cache_data = {
                path: asdict(info) 
                for path, info in self.image_hashes.items()
            }
    ...
    def _get_image_info(self, image_path: Path) -> DuplicateInfo:
        ...
        visual_hash = self._calculate_visual_hash(image_path) if self.enable_visual_detection else None
    ...
    def detect_duplicates(self, image_dir: Path, 
                         image_extensions: Set[str] = {'.jpg', '.jpeg', '.png', '.bmp', '.tiff'}) -> DuplicateDetectionResult:
        """
        重複画像検出メイン処理
    ...
    def _detect_visual_duplicates(self, image_infos: List[DuplicateInfo], 
                                 hash_groups: Dict[str, List[DuplicateInfo]]) -> List[DuplicateGroup]:
        """視覚的類似重複検出（完全一致以外）"""
        visual_groups = []
        
        # 完全一致でない画像のみ対象
        unique_images = [
            info for hash_val, group in hash_groups.items() 
            if len(group) == 1 
            for info in group
        ]
        ...
        for i, img1 in enumerate(unique_images):
            for j, img2 in enumerate(unique_images[i+1:], i+1):
                ...
                if similarity >= self.visual_threshold:
                    ...
                    logger.debug(f"視覚的重複検出: {similarity:.3f} - {Path(img1.file_path).name} vs {Path(img2.file_path).name}")
...
def create_duplicate_detector(visual_threshold: float = 0.85,
                            enable_visual_detection: bool = True,
                            cache_dir: Optional[Path] = None) -> ImageDuplicateDetector:
    """重複検出器ファクトリー関数"""
