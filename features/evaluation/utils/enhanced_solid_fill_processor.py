...
class ColorUniformityAnalyzer:
    ...
    def analyze_color_uniformity(self, image: np.ndarray) -> Dict[str, Any]:
        ...
        overall_uniformity = {
            ...
            "lab": uniformity_lab,
            "combined_score": (
                uniformity_rgb["score"] + uniformity_hsv["score"] + uniformity_lab["score"]
            )
            / 3.0,
        }
...
class SolidFillDetector:
    ...
    def _fallback_color_clustering(self, pixels: np.ndarray) -> np.ndarray:
        ...
        for i, pixel in enumerate(sampled_pixels):
            ...
            # ベクトル化された距離計算
            distances = np.linalg.norm(sampled_pixels.astype(float) - pixel.astype(float), axis=1)
        ...
        if len(pixels) > max_samples:
            ...
            for i, pixel in enumerate(pixels):
                ...
                distances = [
                    np.linalg.norm(pixel.astype(float) - center.astype(float))
    ...
    def _classify_region_type(
        self, mask: np.ndarray, image: np.ndarray, color: Tuple[int, int, int]
    ) -> str:
        """領域タイプの分類"""
        # 位置情報による分類
        h, w = mask.shape
        ...
        edge_threshold = 0.1
        if (
            centroid_y < h * edge_threshold
            or centroid_y > h * (1 - edge_threshold)
            or centroid_x < w * edge_threshold
            or centroid_x > w * (1 - edge_threshold)
        ):
            return "background"
    ...
    def _merge_similar_regions(self, regions: List[SolidFillRegion]) -> List[SolidFillRegion]:
        ...
        for i, region1 in enumerate(regions):
            ...
            for j, region2 in enumerate(regions[i + 1 :], i + 1):
                ...
                # 色の類似性チェック
                color_diff = np.linalg.norm(np.array(region1.color) - np.array(region2.color))
    ...
    def _are_regions_adjacent(
        self, mask1: np.ndarray, mask2: np.ndarray, distance_threshold: int = 5
    ) -> bool:
        """2つの領域が隣接しているかチェック"""
        # 膨張処理で距離を評価
...
class AdaptiveSolidFillProcessor:
    ...
    def process_solid_fill_regions(
        self, image: np.ndarray, regions: List[SolidFillRegion]
    ) -> Dict[str, Any]:
        """ベタ塗り領域の適応的処理"""
        ...
        for i, region in enumerate(regions):
            ...
            if region.region_type == "character":
                processed_region = self._process_character_solid(image, region.mask, region.color)
            elif region.region_type == "background":
                processed_region = self._process_background_solid(image, region.mask, region.color)
            else:
                processed_region = self._process_effect_solid(image, region.mask, region.color)
    ...
    def _process_character_solid(
        self, image: np.ndarray, mask: np.ndarray, color: Tuple[int, int, int]
    ) -> np.ndarray:
        """キャラクター内ベタ塗りの処理"""
        # エッジを保持しながら滑らかに
...
class EnhancedSolidFillProcessor:
    ...
    def _generate_recommendations(self, regions: List[SolidFillRegion], ratio: float) -> List[str]:
        """処理推奨事項の生成"""
        recommendations = []
...
def evaluate_solid_fill_processing(image_path: str, save_results: bool = True) -> Dict[str, Any]:
    """ベタ塗り処理の評価関数"""
    import cv2
    ...
    if save_results and results["analysis"]:
        ...
        for i, region in enumerate(results["analysis"].regions):
            cv2.imwrite(str(output_dir / f"{base_name}_region_{i}_mask.jpg"), region.mask)
        ...
        if results["analysis"].regions:
            ...
            cv2.imwrite(str(output_dir / f"{base_name}_all_solid_fills.jpg"), combined_mask)
