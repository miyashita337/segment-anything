...
class TrueSuccessAnalyzer:
    ...
    def analyze_true_success_rate(self) -> Dict:
        ...
        coordinate_match_rate = (
            (coordinate_matches / total_analyzed * 100) if total_analyzed > 0 else 0
        )
    ...
    def _verify_single_result(
        ...
        confidence_level = self._determine_confidence(
            ai_result, coordinate_match, visual_content_match
        )
    ...
    def _classify_issue(
        self, coordinate_match: bool, visual_content_match: bool, ai_result: Dict
    ) -> Optional[str]:
        """問題分類"""
    ...
    def _determine_confidence(
        self, ai_result: Dict, coordinate_match: bool, visual_content_match: bool
    ) -> str:
        """確信度判定"""
        confidence = ai_result.get("confidence", 0.0)
        quality_score = ai_result.get("quality_score", 0.0)
    ...
    def _analyze_confidence(self, results: List[TrueVerificationResult]) -> Dict:
        ...
        confidence_counts = {}
        for result in results:
            confidence_counts[result.confidence_level] = (
                confidence_counts.get(result.confidence_level, 0) + 1
            )
