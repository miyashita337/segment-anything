...
class CompletenessValidator:
    ...
    def validate_completeness(self, image: np.ndarray) -> CompletenessValidationResult:
        ...
        try:
            ...
            # 6. 完全性判定
            is_complete = (
                completeness_percentage >= self.completeness_threshold * 100 and
                not fragmentation_result['is_fragmented'] and
                not aspect_ratio_result['has_warning']
            )
    ...
    def _analyze_fragmentation(self, gray: np.ndarray, issues: List[str], 
    ...
    def _detect_body_parts(self, gray: np.ndarray, issues: List[str], 
                          suggestions: List[str]) -> Dict:
        """身体部位の検出"""
        h, w = gray.shape
        ...
        # 輪郭品質の評価
        contours, _ = cv2.findContours(non_zero_mask.astype(np.uint8), 
    ...
    def _validate_aspect_ratio(self, gray: np.ndarray, issues: List[str], 
                              suggestions: List[str]) -> Dict:
        """アスペクト比の検証"""
        h, w = gray.shape
        aspect_ratio = h / w
    ...
    def _calculate_completeness_percentage(self, body_parts_result: Dict, 
                                          fragmentation_result: Dict, 
                                          aspect_ratio_result: Dict) -> float:
        """完全性パーセンテージの計算"""
        # 身体部位の完全性（40%の重み）
    ...
    def _calculate_validation_confidence(self, body_parts_result: Dict, 
                                        fragmentation_result: Dict, 
                                        aspect_ratio_result: Dict) -> float:
        """検証信頼度の計算"""
        confidence_factors = []
    ...
    def get_validation_summary(self, result: CompletenessValidationResult) -> Dict[str, any]:
        ...
        return {
            ...
            'confidence_level': 'high' if result.validation_confidence > 0.8 else 
                               'medium' if result.validation_confidence > 0.6 else 'low',
