...
class MaskQualityValidator:
    ...
    def validate_body_completeness(
        ...
        try:
            ...
            contours, _ = cv2.findContours(
                (mask_roi > 0).astype(np.uint8), cv2.RETR_EXTERNAL, cv2.CHAIN_APPROX_SIMPLE
            )
    ...
    def expand_mask_for_completeness(
        self,
        image: np.ndarray,
        mask: np.ndarray,
    ...
    def comprehensive_mask_validation(
        self, image: np.ndarray, mask: np.ndarray, bbox: Tuple[int, int, int, int]
    ) -> Dict[str, Any]:
        """
        ...
        if "expand_for_face_completeness" in validation["recommended_actions"]:
            ...
            if face_val["needs_expansion"]:
                improved_mask, improved_bbox = validator.expand_mask_for_completeness(
                    ...
                    improved_mask,
                    improved_bbox,
                    face_val["expansion_directions"],
                    max_expansion=25,
                )
                logger.info(
                    f"   🔄 Expanded mask for face completeness: {face_val['expansion_directions']}"
                )
