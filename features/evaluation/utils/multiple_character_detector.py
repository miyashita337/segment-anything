...
class MultipleCharacterDetector:
    ...
    def analyze_yolo_detections(self, 
                              yolo_detections: List[Dict[str, Any]], 
                              image_shape: Tuple[int, int]) -> MultipleCharacterResult:
        """
        ...
        logger.info(f"✅ 複数キャラクター分析完了: タイプ={detection_type.value}, "
                   f"ペナルティ={penalty_score:.2f}")
    ...
    def _classify_multiple_character_type(self, 
                                        characters: List[Dict[str, Any]], 
                                        primary_index: int,
                                        image_shape: Tuple[int, int]) -> Tuple[MultipleCharacterType, float]:
        """
        複数キャラクタータイプ分類
        ...
        for other in other_characters:
            ...
            if (relative_distance < self.DETECTION_THRESHOLDS['distance_threshold'] and 
                size_ratio > self.DETECTION_THRESHOLDS['size_ratio_threshold']):
                grouped_count += 1
    ...
    def _generate_improvement_suggestions(self, 
        ...
        if detection_type == MultipleCharacterType.OVERLAPPING_CHARACTERS:
            suggestions.extend([
                "⚠️ 抽出後重複残存: SAMマスク精度改善が必要",
                "アダプティブクロッピング処理の見直し推奨",
                "より厳密なマスク重複判定の導入検討"
            ])
        elif detection_type == MultipleCharacterType.BACKGROUND_CHARACTERS:
            suggestions.extend([
                "⚠️ 背景キャラ残存: 抽出アルゴリズム改善必要",
                "メインキャラクター選択ロジックの見直し",
                "背景ノイズフィルタリングの強化"
            ])
        elif detection_type == MultipleCharacterType.GROUPED_CHARACTERS:
            suggestions.extend([
                "⚠️ グループキャラ残存: 単一キャラ抽出失敗",
                "YOLO+SAM連携処理の改良が必要",
                "クロッピング範囲の最適化検討"
            ])
...
def detect_multiple_characters_from_image(image_path: Path, 
                                        yolo_wrapper,
                                        save_visualization: bool = False) -> MultipleCharacterResult:
    """
    画像から複数キャラクター検出（便利関数）
