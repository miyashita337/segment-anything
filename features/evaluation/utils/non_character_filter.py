...
class NonCharacterFilter:
    ...
    def is_speech_bubble(
        ...
        Returns:
            Detection results
        """
        result = {"is_speech_bubble": False, "confidence": 0.0, "reasons": []}
            ...
            if len(masked_pixels) > 0:
                ...
                if len(masked_pixels.shape) == 2 and masked_pixels.shape[1] == 3:
                    gray_pixels = cv2.cvtColor(
                        masked_pixels.reshape(-1, 1, 3), cv2.COLOR_BGR2GRAY
                    ).flatten()
                else:
    ...
    def is_mask_object(
        ...
        Returns:
            Detection results
        """
        result = {"is_mask_object": False, "confidence": 0.0, "reasons": []}
        ...
        try:
            ...
            lines = cv2.HoughLinesP(
                edges, 1, np.pi / 180, threshold=15, minLineLength=w // 5, maxLineGap=8
            )
            ...
            contours, _ = cv2.findContours(
                mask_roi_uint8, cv2.RETR_EXTERNAL, cv2.CHAIN_APPROX_SIMPLE
            )
            if contours:
                largest_contour = max(contours, key=cv2.contourArea)
    ...
    def is_text_or_effects(
        ...
        Returns:
            Detection results
        """
        result = {"is_text_effects": False, "confidence": 0.0, "reasons": []}
    ...
    def filter_non_character_elements(
        self, masks: List[Dict[str, Any]], image: np.ndarray
    ) -> List[Dict[str, Any]]:
        """
        ...
        for i, mask_data in enumerate(masks):
            try:
                ...
                is_non_character = (
                    speech_result["is_speech_bubble"]
                    or mask_result["is_mask_object"]
                    or text_result["is_text_effects"]
                )

                logger.info(
                    f"   マスク{i+1}: 吹き出し={speech_result['is_speech_bubble']}, "
...
def apply_non_character_filter(
