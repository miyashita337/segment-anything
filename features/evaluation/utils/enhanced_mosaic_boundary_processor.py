...
class MultiScaleMosaicDetector:
    ...
    def _detect_mosaic_single_scale(self, image: np.ndarray, scale: float) -> MosaicDetectionResult:
        ...
        # 最も信頼度の高い結果を選択
        candidates = [("grid", grid_result), ("pixelated", pixel_result), ("blur", blur_result)]
        ...
        if scale != 1.0:
            original_h, original_w = (
                image.shape[:2] if len(image.shape) == 3 else (image.shape[0], image.shape[1])
            )
...
class RotationInvariantDetector:
    ...
    def detect_rotated_patterns(self, image: np.ndarray) -> MosaicDetectionResult:
        ...
        if best_result and best_result["angle"] != 0:
            best_result["mask"] = self._rotate_image(best_result["mask"], -best_result["angle"])
...
class AdaptiveBoundaryProcessor:
    ...
    def process_mosaic_boundaries(
        self, image: np.ndarray, mosaic_result: MosaicDetectionResult
    ...
    def _process_grid_boundaries(
        ...
        if expanded_mask.shape[:2] != image.shape[:2]:
            expanded_mask = cv2.resize(
                expanded_mask, (image.shape[1], image.shape[0]), interpolation=cv2.INTER_NEAREST
            )
    ...
    def _process_pixelated_boundaries(
        self, image: np.ndarray, mosaic_result: MosaicDetectionResult
    ) -> BoundaryProcessingResult:
        """ピクセル化パターンの境界処理"""
        # ピクセル境界をスムージング
    ...
    def _gradual_blending(
        self, original: np.ndarray, processed: np.ndarray, mask: np.ndarray
    ) -> np.ndarray:
        """段階的ブレンディング"""
        # マスクサイズを画像に合わせる
        ...
        # ブレンディング
        result = (
            original.astype(np.float32) * (1 - smooth_mask)
    ...
    def _apply_with_mask(
        ...
        result = (
            original.astype(np.float32) * (1 - mask_norm) + processed.astype(np.float32) * mask_norm
        )
...
class EnhancedMosaicBoundaryProcessor:
    ...
    def process_image(self, image: np.ndarray) -> Dict[str, Any]:
        ...
        results = {
            ...
            "processing_info": {"methods_used": [], "confidence": 0.0, "mosaic_type": "none"},
        }
...
def evaluate_mosaic_boundary_enhancement(
