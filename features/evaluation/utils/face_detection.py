...
class FaceDetector:
    ...
    def validate_character_mask(
        ...
        try:
            ...
            result["validation_method"] = (
                "cascade" if (self.face_cascade and not self.face_cascade.empty()) else "fallback"
            )

            if faces:
                # Calculate face coverage in mask
                mask_roi = (
                    mask[y : y + h, x : x + w] if mask.ndim == 2 else mask[y : y + h, x : x + w]
                )
                total_face_area = 0
                covered_face_area = 0
    ...
    def _calculate_confidence(
        self, validation_result: Dict[str, Any], bbox: Tuple[int, int, int, int], mask: np.ndarray
    ) -> float:
        """
...
def filter_non_character_masks(
    ...
    for i, mask_data in enumerate(masks):
        try:
            ...
            logger.info(
                f"   マスク{i+1}: 顔検出={validation['has_face']}, "
            ...
            if (validation["is_character"] and validation["confidence"] >= min_confidence) or (
                basic_size_ok and basic_ratio_ok
            ):
