...
class ScreentoneFeatureExtractor:
    ...
    def extract_fft_features(self, image: np.ndarray) -> Dict[str, float]:
        ...
        # 各周波数帯域のパワー
        low_freq_power = self._calculate_freq_power(
            magnitude_spectrum, center_h, center_w, 0, h // 8
        )
        mid_freq_power = self._calculate_freq_power(
            magnitude_spectrum, center_h, center_w, h // 8, h // 4
        )
        high_freq_power = self._calculate_freq_power(
            magnitude_spectrum, center_h, center_w, h // 4, h // 2
        )
    ...
    def _find_dominant_frequency(
        self, magnitude_spectrum: np.ndarray, center_h: int, center_w: int
    ) -> float:
        """主要周波数を検出"""
        h, w = magnitude_spectrum.shape
        ...
        for radius in range(1, max_radius):
            power = self._calculate_freq_power(
                magnitude_spectrum, center_h, center_w, radius - 1, radius
            )
    ...
    def _calculate_spectral_spread(self, magnitude_spectrum: np.ndarray) -> float:
        ...
        spread = (
            np.sum(((distances / max(center_h, center_w)) - centroid) ** 2 * magnitude_spectrum)
            / total_magnitude
        )
...
class EnhancedScreentoneDetector:
    ...
    def detect_screentone(
        self, image: np.ndarray, roi_mask: Optional[np.ndarray] = None
        ...
        # パターン分類
        screentone_type, type_confidence = self.pattern_classifier.classify_pattern(
            combined_features
        )

        # スクリーントーン検出の総合判定
        has_screentone = bool(
            screentone_type != ScreentoneType.NONE
            and type_confidence > self.detection_params["min_confidence"]
        )
        ...
        quality_score = self._calculate_quality_score(
            combined_features, type_confidence, coverage_ratio
        )

        # 推論の生成
        reasoning = self._generate_reasoning(
            screentone_type, type_confidence, combined_features, coverage_ratio
        )
    ...
    def _generate_screentone_mask(
        self, image: np.ndarray, screentone_type: ScreentoneType, features: Dict[str, float]
    ) -> np.ndarray:
        """
        スクリーントーンマスクの生成
    ...
    def _calculate_quality_score(
        self, features: Dict[str, float], type_confidence: float, coverage_ratio: float
    ) -> float:
        """品質スコアの計算"""
        # 基本スコア（パターン分類の信頼度）
...
# 便利関数
def detect_screentone_enhanced(
    image: np.ndarray, roi_mask: Optional[np.ndarray] = None
) -> ScreentoneDetectionResult:
    """
    改良版スクリーントーン検出の便利関数
