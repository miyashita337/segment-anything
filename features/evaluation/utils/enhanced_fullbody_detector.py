...
class EnhancedFullBodyDetector:
    ...
    def _calculate_body_structure_score(
        self, image: np.ndarray, mask_data: Dict[str, Any]
    ) -> float:
        """人体構造スコア計算"""
        try:
    ...
    def _analyze_body_structure(self, image: np.ndarray, mask: np.ndarray) -> BodyStructureAnalysis:
        ...
        torso_density = (
            np.sum(torso_area > 0) / max(torso_area.size, 1) if torso_area.size > 0 else 0
        )
    ...
    def _detect_face_regions_in_mask(
    ...
    def _calculate_edge_distribution_score(
        self, mask_data: Dict[str, Any], h: int, w: int
    ) -> float:
        """エッジ分布スコア計算"""
        try:
            # マスクからエッジ分布を分析
            ...
            vertical_balance = 1.0 - abs(edge_densities["top"] - edge_densities["bottom"]) / max(
                edge_densities["top"] + edge_densities["bottom"], 0.01
            )

            # 左右のバランス
            horizontal_balance = 1.0 - abs(edge_densities["left"] - edge_densities["right"]) / max(
                edge_densities["left"] + edge_densities["right"], 0.01
            )
    ...
    def _calculate_semantic_region_score(
        self, image: np.ndarray, mask_data: Dict[str, Any]
    ) -> float:
        """セマンティック領域スコア計算"""
        try:
    ...
    def _detect_skin_regions(self, pixels: np.ndarray) -> float:
        ...
        skin_mask = (
            (
                (hsv_pixels[:, 0] >= 0) & (hsv_pixels[:, 0] <= 25)
                | (hsv_pixels[:, 0] >= 160) & (hsv_pixels[:, 0] <= 180)  # 赤系  # 赤系
            )
