...
class CharacterQualityAssessor:
    ...
    def assess_individual_character_quality(
        self, character_image: np.ndarray
    ) -> IndividualCharacterQuality:
        """
        個別キャラクターの品質評価
        ...
        try:
            ...
            # 3. サイズ適正性スコア
            size_adequacy_score = self._assess_size_adequacy(
                character_image, issues, recommendations
            )

            # 4. 形状品質スコア
            shape_quality_score = self._assess_shape_quality(
                character_image, issues, recommendations
            )

            # 5. 詳細保存スコア
            detail_preservation_score = self._assess_detail_preservation(
                character_image, issues, recommendations
            )
    ...
    def assess_multi_character_quality(self, scene_image: np.ndarray) -> MultiCharacterQuality:
        ...
        try:
            ...
            # シーン全体の評価
            scene_balance_score = self._assess_scene_balance(
            ...
            (
                ...
                interaction_score,
                has_interaction,
            ) = self._analyze_character_interactions(
                detection_result, scene_issues, scene_recommendations
            )

            # 総合シーン品質
            if individual_scores:
                avg_individual_quality = np.mean(
                    [s.overall_quality_score for s in individual_scores]
                )
            else:
    ...
    def _assess_completeness(
        self, image: np.ndarray, issues: List[str], recommendations: List[str]
    ) -> float:
        """完全性の評価（輪郭の完全性）"""
        ...
        contours, _ = cv2.findContours(
            (gray > 10).astype(np.uint8), cv2.RETR_EXTERNAL, cv2.CHAIN_APPROX_SIMPLE
        )
    ...
    def _assess_detail_preservation(
        self, image: np.ndarray, issues: List[str], recommendations: List[str]
    ) -> float:
        """詳細保存性の評価"""
        if len(image.shape) == 3:
    ...
    def _assess_scene_balance(
        self, detection_result, issues: List[str], recommendations: List[str]
    ) -> float:
        """シーンバランスの評価"""
