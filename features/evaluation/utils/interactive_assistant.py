    ...
    def extract_character_interactive(self, output_path: str = None) -> Dict[str, Any]:
        ...
        try:
            ...
            character_image = extract_character_from_image(
                self.current_image, mask, background_color=(0, 0, 0)
            )

            # クロップ
            cropped_character, cropped_mask, crop_bbox = crop_to_content(
                character_image, mask, padding=10
            )
            ...
            save_success = save_character_result(
                cropped_character, cropped_mask, output_path, save_mask=True, save_transparent=True
            )
...
class InteractiveGUI:
    ...
    def setup_ui(self):
        ...
        ttk.Button(seed_frame, text="最後のポイント削除", command=self.remove_last_point).pack(
            fill=tk.X, pady=2
        )
        ...
        ttk.Button(preview_frame, text="マスクプレビュー", command=self.preview_mask).pack(
            fill=tk.X, pady=2
        )
    ...
    def open_image(self):
        ...
        file_path = filedialog.askopenfilename(
            title="画像ファイルを選択",
            filetypes=[("画像ファイル", "*.jpg *.jpeg *.png *.bmp *.tiff"), ("すべてのファイル", "*.*")],
        )
        ...
        try:
            ...
            if mask is not None:
                ...
                overlay[mask_resized > 0] = cv2.addWeighted(
                    ...
                    0.7,
                    np.full_like(overlay[mask_resized > 0], [0, 255, 0]),
                    0.3,
                    0,
                )
    ...
    def save_result(self):
        ...
        output_path = filedialog.asksaveasfilename(
            ...
            filetypes=[("JPEG画像", "*.jpg"), ("PNG画像", "*.png"), ("すべてのファイル", "*.*")],
        )
