...
class PartialExtractionDetector:
    ...
    def _is_face_only_extraction(
        self, image: np.ndarray, mask: np.ndarray, has_face: bool, has_torso: bool, has_limbs: bool
    ) -> bool:
        """顔のみ抽出の判定"""
        if not has_face:
            return False
    ...
    def _calculate_completeness_score(
        self, has_face: bool, has_torso: bool, has_limbs: bool, issues: List[PartialExtractionIssue]
    ) -> float:
        """完全性スコアの計算"""
        base_score = 0.0
    ...
    def suggest_improvements(self, analysis: ExtractionAnalysis) -> List[str]:
        ...
        if analysis.quality_assessment == "poor":
            suggestions.extend(["手動ポイント指定による再抽出を検討", "前処理（コントラスト調整）の適用", "異なるYOLOモデルの使用"])
        elif analysis.quality_assessment == "partial":
            suggestions.extend(["マスク後処理の強化", "SAMパラメータの微調整"])
