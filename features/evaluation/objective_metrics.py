...
class PLACalculator:
    ...
    def calculate(self, predicted_mask: np.ndarray, ground_truth_mask: np.ndarray) -> ObjectiveMetricResult:
        """
        PLA計算（IoUベース）
        ...
        try:
            ...
            details = {
                ...
                "precision": float(intersection) / float(pred_binary.sum()) if pred_binary.sum() > 0 else 0.0,
                "recall": float(intersection) / float(gt_binary.sum()) if gt_binary.sum() > 0 else 0.0,
                "dice_coefficient": 2.0 * float(intersection) / float(pred_binary.sum() + gt_binary.sum()) if (pred_binary.sum() + gt_binary.sum()) > 0 else 0.0
            }
    ...
    def _calculate_confidence(self, pred_binary: np.ndarray, gt_binary: np.ndarray, iou: float) -> float:
        """信頼度計算"""
        # IoUベースの基本信頼度
...
class SCICalculator:
    ...
    def _calculate_contour_density(self, contour: np.ndarray, image_shape: Tuple[int, int]) -> float:
        """輪郭密度計算"""
        try:
...
class PLETracker:
    ...
    def _calculate_ple_score(self, baseline: List[float], recent: List[float]) -> Tuple[float, Dict[str, Any]]:
        """PLEスコア計算"""
...
class ObjectiveMetricsSystem:
    ...
    def evaluate_extraction_batch(self, extracted_images: List[np.ndarray],
                                 ground_truth_masks: Optional[List[np.ndarray]] = None) -> Dict[str, ObjectiveMetricResult]:
        """
        バッチ抽出結果の客観的評価
...
def main():
    ...
    parser.add_argument("--test", choices=["pla", "sci", "ple", "all"], default="all", help="テストする指標")
    ...
    if args.test in ["pla", "all"]:
        ...
        print(f"詳細: IoU={pla_result.details['iou']:.3f}, Dice={pla_result.details['dice_coefficient']:.3f}")
    ...
    if args.test in ["sci", "all"]:
        ...
        print(f"詳細: Face={sci_result.details['face_detection_score']:.3f}, "
    ...
    if args.test in ["ple", "all"]:
        ...
        if "improvement_rate" in ple_result.details:
            print(f"詳細: 改善率={ple_result.details['improvement_rate']:.3f}, "
