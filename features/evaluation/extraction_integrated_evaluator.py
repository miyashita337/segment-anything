...
class FileCorrespondenceMatcher:
    ...
    def match_directory_files(self, directory: Path) -> List[FileCorrespondence]:
        ...
        self.logger.info(
            f"ファイル分類完了: 元画像{len(source_images)}枚, "
    ...
    def _match_single_image(
        self, source_image: Path, gt_masks: List[Path], extracted_images: List[Path]
    ) -> FileCorrespondence:
        """単一画像の対応付け"""
        ...
        for gt_mask in gt_masks:
            if any(
                gt_mask.stem.startswith(base_name + pattern.replace(".png", ""))
        ...
        for ext_img in extracted_images:
            if any(
                ext_img.stem.startswith(base_name + pattern.replace(".png", ""))
...
class MetadataManager:
    ...
    def save_batch_metadata(self, directory: Path, metadata: Dict[str, ExtractionMetadata]):
        """バッチメタデータ保存"""
        metadata_file = self.metadata_dir / f"{directory.name}_metadata.json"
...
class ExtractionIntegratedEvaluator:
    ...
    def evaluate_extraction_batch(self, extraction_dir: str) -> List[IntegratedEvaluationResult]:
        ...
        for correspondence in correspondences:
            try:
                result = self._evaluate_single_correspondence(correspondence, batch_metadata)
    ...
    def _evaluate_single_correspondence(
        self, correspondence: FileCorrespondence, batch_metadata: Dict[str, ExtractionMetadata]
        ...
        if correspondence.ground_truth_mask and correspondence.extracted_image:
            try:
                ...
                ground_truth_mask = cv2.imread(
                    str(correspondence.ground_truth_mask), cv2.IMREAD_GRAYSCALE
                )
    ...
    def _analyze_metadata_statistics(self, results: List[IntegratedEvaluationResult]) -> Dict:
        ...
        return {
            ...
            "yolo_confidence": {"mean": np.mean(yolo_confidences), "std": np.std(yolo_confidences)},
            "sam_iou": {"mean": np.mean(sam_ious), "std": np.std(sam_ious)},
            "balanced_score": {"mean": np.mean(balanced_scores), "std": np.std(balanced_scores)},
...
def main():
    ...
    try:
        ...
        print(
            f"PLA統計: 平均{report['pla_statistics']['mean']:.3f} "
