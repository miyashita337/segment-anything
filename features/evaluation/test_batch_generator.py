...
class TestBatchGenerator:
    ...
    def generate_test_batch(
        self, benchmark_results: List[Dict[str, Any]], phase: str = "phase0"
    ) -> TestBatchSummary:
        """
        テストバッチ生成
        ...
        try:
            ...
            for i, result in enumerate(benchmark_results):
                ...
                try:
                    ...
                    scored_result = {**result, "claude_score": claude_score, "issues": issues}
    ...
    def create_test_batch_item(
        self, result: Dict[str, Any], rank: int, category: str
    ) -> TestBatchItem:
        """TestBatchItem作成"""
        ...
        return TestBatchItem(
            ...
            predicted_bbox=tuple(result["prediction_bbox"])
            if result.get("prediction_bbox")
            else None,
            iou_score=result.get("iou_score", 0.0),
            confidence=result.get("confidence_score", 0.0),
            quality_grade=result.get("quality_grade", "F"),
    ...
    def extract_key_insights(
        ...
        try:
            ...
            insights.append(
                f"高スコア(≥0.7): {high_score_count}/{len(scores)}件 ({high_score_count/len(scores):.1%})"
            )
            ...
            insights.append(
                f"A/B評価率: {ab_count}/{len(all_results)}件 ({ab_count/len(all_results):.1%})"
            )
    ...
    def create_annotated_image(self, item: TestBatchItem, output_dir: Path):
        ...
        try:
            ...
            # タイトル・情報追加
            title = (
                f"Rank {item.rank} ({item.category.upper()}) - {item.image_id}\n"
            ...
            # 保存
            output_file = (
                output_dir / f"{item.rank:02d}_{item.image_id}_score{item.claude_score:.3f}.png"
            )
    ...
    def create_summary_image(self, summary: TestBatchSummary, output_dir: Path):
        ...
        try:
            ...
            fig.suptitle(
                f"Test Batch Summary - {summary.phase.upper()} ({summary.timestamp})",
                fontsize=16,
                fontweight="bold",
            )
            ...
            bars1 = ax1.bar(
                x_pos - width / 2, best_scores, width, label="Best 5", color="green", alpha=0.7
            )
            bars2 = ax1.bar(
                x_pos + width / 2, worst_scores, width, label="Worst 5", color="red", alpha=0.7
            )
