...
class MetricsCollector:
    ...
    ) -> Tuple[List[SystemMetrics], List[ProcessingMetrics]]:
        """指定時間範囲のメトリクス取得"""
        system_metrics = [
            m for m in self.system_metrics_history if start_time <= m.timestamp <= end_time
        ]

        processing_metrics = [
            m for m in self.processing_metrics_history if start_time <= m.timestamp <= end_time
        ]
    ...
    def get_average_metrics(self, duration_minutes: int = 5) -> Dict[str, float]:
        ...
        return {
            ...
            "avg_memory_percent": sum(m.memory_percent for m in system_metrics)
            / len(system_metrics),
            "avg_gpu_memory_used_mb": sum(m.gpu_memory_used_mb for m in system_metrics)
            / len(system_metrics),
            "avg_gpu_utilization": sum(m.gpu_utilization for m in system_metrics)
            / len(system_metrics),
            "total_network_sent_mb": sum(m.network_bytes_sent for m in system_metrics)
            / (1024**2),
            "total_network_recv_mb": sum(m.network_bytes_recv for m in system_metrics)
            / (1024**2),
        }
...
class PH2006MonitoringSystem:
    ...
    def stop_monitoring(self):
        ...
        if self.monitoring_stats["start_time"]:
            self.monitoring_stats["uptime_seconds"] = (
                time.time() - self.monitoring_stats["start_time"]
            )
    ...
    def _monitoring_loop(self):
        ...
        while not self.stop_monitoring_event.wait(5.0):  # 5秒間隔でアラートチェック
            try:
                ...
                # 統計更新
                self.monitoring_stats["metrics_collected"] = len(
    ...
    def get_monitoring_status(self) -> Dict[str, Any]:
        ...
        status = {
            ...
            "uptime_seconds": time.time() - self.monitoring_stats["start_time"]
            if self.monitoring_stats["start_time"]
            else 0,
    ...
    def generate_report(self, duration_hours: int = 1) -> Dict[str, Any]:
        ...
        system_metrics, processing_metrics = self.metrics_collector.get_metrics_in_range(
            start_time, end_time
        )
        ...
        if processing_metrics:
            ...
            processing_stats = {
                ...
                "success_rate": len(successful_tasks) / len(processing_metrics)
                if processing_metrics
                else 0,
                "avg_duration": sum(m.duration for m in processing_metrics)
                / len(processing_metrics),
                "avg_throughput": sum(m.throughput for m in processing_metrics)
                / len(processing_metrics),
            }
        ...
        report = {
            ...
            "monitoring_health": {
                "monitoring_active": self.monitoring_active,
                "metrics_collection_rate": len(system_metrics) / duration_hours
                if duration_hours > 0
                else 0,
                "system_performance": "healthy"
                if system_stats.get("cpu_percent", {}).get("avg", 0) < 70
                else "stressed",
            },
        }
    ...
    def export_metrics(self, format: str = "json") -> str:
        ...
        if format.lower() == "json":
            export_data = {
                ...
                "processing_metrics": [
                    asdict(m) for m in self.metrics_collector.processing_metrics_history
                ],
