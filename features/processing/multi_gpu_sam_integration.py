...
class MultiGPUSAMIntegration:
    ...
    def process_with_existing_pipeline(self, 
                                     input_files: List[str], 
                                     output_dir: str,
                                     processing_params: Dict[str, Any] = None) -> bool:
        """既存パイプラインでマルチGPU処理"""
        ...
        with concurrent.futures.ThreadPoolExecutor(max_workers=gpu_count) as executor:
            ...
            for gpu_idx, (gpu_info, file_chunk) in enumerate(zip(self.processor.gpu_manager.available_gpus, file_chunks)):
    ...
    def _process_chunk_on_gpu(self, 
                            ...
                            output_dir: str, 
                            gpu_id: int,
                            processing_params: Optional[Dict[str, Any]]) -> int:
        """GPU上でファイルチャンク処理"""
        ...
        try:
            ...
            with tempfile.TemporaryDirectory() as temp_input_dir:
                ...
                cmd = [
                    ...
                    "--device", f"cuda:{gpu_id}",  # GPU指定
                    "--score_threshold", str((processing_params or {}).get('score_threshold', 0.07))
                ]
    ...
    def _single_gpu_processing(self, 
                             input_files: List[str], 
                             output_dir: str, 
                             processing_params: Optional[Dict[str, Any]]) -> bool:
        """単一GPU処理"""
        logger.info("単一GPU最適化処理を実行")
        
        return self._fallback_to_standard_processing(input_files, output_dir, processing_params)
    
    def _fallback_to_standard_processing(self, 
                                       input_files: List[str], 
                                       output_dir: str, 
                                       processing_params: Optional[Dict[str, Any]]) -> bool:
        """標準処理フォールバック"""
