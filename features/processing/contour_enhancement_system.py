...
class ContourEnhancementSystem:
    ...
    def enhance_contour_quality(self, 
                              mask: np.ndarray,
        ...
        try:
            ...
            anime_optimized_mask = self._anime_character_contour_optimization(precision_enhanced_mask)
    ...
    def _advanced_edge_detection(self, 
                               mask: np.ndarray,
        ...
        try:
            ...
            if original_image is not None:
                ...
                combined_edges = cv2.bitwise_or(
                    cv2.bitwise_or(multi_scale_edges, adaptive_edges),
                    image_guided_edges
                )
            else:
                # 2つのエッジ情報を統合
            ...
            return cv2.Canny(mask, self.config.canny_low_threshold, self.config.canny_high_threshold)
    ...
    def _image_guided_edge_detection(self, mask: np.ndarray, original_image: np.ndarray) -> np.ndarray:
        """画像ガイド付きエッジ検出"""
        try:
            # 元画像のエッジ検出
    ...
    def _gaussian_contour_smoothing(self, contour: np.ndarray) -> np.ndarray:
        ...
        try:
            ...
            x_smooth = gaussian_filter1d(x_coords, sigma=self.config.gaussian_smoothing_sigma, mode='wrap')
            y_smooth = gaussian_filter1d(y_coords, sigma=self.config.gaussian_smoothing_sigma, mode='wrap')
    ...
    def _subpixel_contour_detection(self, mask: np.ndarray) -> np.ndarray:
        ...
        try:
            ...
            # 輪郭近似で精度向上
            epsilon = self.config.contour_approximation_accuracy * cv2.arcLength(largest_contour, True)
    ...
    def _analyze_anime_character_features(self, mask: np.ndarray) -> Dict[str, Any]:
        ...
        try:
            ...
            complexity = perimeter / cv2.contourArea(largest_contour) if cv2.contourArea(largest_contour) > 0 else 0
    ...
    def _analyze_contour_improvement(self, 
                                  initial_quality: Dict[str, float], 
                                  final_quality: Dict[str, float]) -> Dict[str, Any]:
        """輪郭改善効果分析"""
        try:
            compactness_improvement = final_quality['compactness'] - initial_quality['compactness']
            overall_improvement = final_quality.get('overall_contour_quality', 0.0) - initial_quality.get('overall_contour_quality', 0.0)
            ...
            return {
                ...
                'improvement_percentage': (compactness_improvement / max(initial_quality['compactness'], 0.001)) * 100,
