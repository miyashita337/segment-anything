...
class AdaptiveCropper:
    ...
    def optimize_bbox_with_faces(self, yolo_bbox: DetectionBox, 
                                faces: List[DetectionBox], 
                                image_shape: Tuple[int, int]) -> DetectionBox:
        """顔検出結果でYOLO境界ボックス最適化"""
        if not faces:
            return yolo_bbox
        ...
        logger.info(f"境界ボックス最適化: {yolo_bbox.area} → {optimized_bbox.area} "
                   f"(顔検出信頼度: {primary_face.confidence:.3f})")
        
        return optimized_bbox
    
    def generate_multiscale_candidates(self, base_bbox: DetectionBox, 
    ...
    def evaluate_cropping_quality(self, bbox: DetectionBox, 
                                faces: List[DetectionBox], 
                                image: np.ndarray) -> Tuple[float, int, float]:
        """クロッピング品質評価"""
        # 1. 顔数カウント（境界ボックス内）
        faces_in_crop = 0
        ...
        # 総合品質スコア
        quality_score = (aspect_score * 0.3 + 
                        size_score * 0.3 + 
                        integrity_score * 0.4)
        ...
        try:
            ...
            for candidate in candidates:
                ...
                cropping_candidate = CroppingCandidate(
                    bbox=candidate,
                    scale_factor=candidate.source.split('_')[1] if '_' in candidate.source else '1.0',
            ...
            for i, result in enumerate(evaluation_results):
                logger.info(f"  候補{i+1}: 複合スコア={result.composite_score:.3f} "
...
def apply_adaptive_cropping(image: np.ndarray, 
                          yolo_bbox: List[int], 
                          confidence: float = 0.5) -> Optional[List[int]]:
    """適応的クロッピング適用（統合用関数）"""
    cropper = create_adaptive_cropper()
    
    # YOLO bbox を DetectionBox に変換
    detection_box = DetectionBox(
        x=yolo_bbox[0], y=yolo_bbox[1], 
        w=yolo_bbox[2], h=yolo_bbox[3],
        confidence=confidence,
        source='yolo'
    )
