...
class DifferentialProcessor:
        ...
        try:
            ...
            if self.dependency_file.exists():
                with open(self.dependency_file, 'r', encoding='utf-8') as f:
                    ...
                    self.dependency_map = {
                        path: set(deps) for path, deps in dep_data.items()
                    }
    ...
    def _save_caches(self):
        ...
        try:
            # ファイルハッシュキャッシュ保存
            hash_data = {
                path: asdict(info) for path, info in self.file_hashes.items()
            }
            ...
            # 処理キャッシュ保存
            cache_data = {
                path: asdict(info) for path, info in self.processing_cache.items()
            }
            ...
            # 依存関係マップ保存
            dep_data = {
                path: list(deps) for path, deps in self.dependency_map.items()
            }
    ...
    def process_changes(self, changes: List[FileChangeInfo]) -> DifferentialReport:
        ...
        logger.info(f"📊 処理済み: {self.stats['processed_files']}, "
                   f"スキップ: {self.stats['skipped_files']}, "
                   f"失敗: {self.stats['failed_files']}")
        logger.info(f"💾 キャッシュヒット率: {self.stats['cache_hits']}/{self.stats['cache_hits'] + self.stats['cache_misses']} "
    ...
    def _generate_markdown_report(self, report: DifferentialReport, output_file: Path):
        ...
        if report.change_details:
            ...
            for i, change in enumerate(report.change_details[:10], 1):
                ...
                emoji = {'added': '➕', 'modified': '📝', 'deleted': '🗑️'}.get(change.change_type, '🔧')
                lines.append(f"{i}. {emoji} **{file_name}** ({change.change_type}, {change.file_size:,} bytes)")
                
        lines.append("")
        lines.append("---")
        lines.append(f"*Generated by P1-013 Differential Processor at {datetime.now().strftime('%Y-%m-%d %H:%M:%S')}*")
...
def main():
    ...
    print(f"📊 処理結果: {report.processed_files}処理, {report.skipped_files}スキップ, {report.failed_files}失敗")
