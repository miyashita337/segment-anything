...
class AutoMaskCorrector:
    ...
    def correct_mask_automatically(
        self,
        mask: np.ndarray,
    ...
    def _apply_adaptive_correction(
        self, mask: np.ndarray, quality_score: Optional[float], original_image: Optional[np.ndarray]
    ...
    def _remove_noise(self, mask: np.ndarray) -> Tuple[np.ndarray, List[str]]:
        ...
        cleaned = cv2.morphologyEx(
            mask, cv2.MORPH_OPEN, kernel, iterations=self.params.morphology_iterations
        )
        log.append(
            f"モルフォロジカルOpening実行 - カーネル: {self.params.morphology_kernel_size}x{self.params.morphology_kernel_size}"
        )
    ...
    def _smooth_edges(
        self, mask: np.ndarray, original_image: Optional[np.ndarray]
    ) -> Tuple[np.ndarray, List[str]]:
        """エッジスムージング処理"""
        log = []
        ...
        if original_image is not None and len(original_image.shape) == 3:
            ...
            gaussian_smoothed = cv2.GaussianBlur(
                mask, (kernel_size, kernel_size), self.params.gaussian_sigma
            )
    ...
    def _calculate_quality_metrics(
        self, original_mask: np.ndarray, corrected_mask: np.ndarray
    ) -> Dict[str, float]:
        """品質メトリクス計算"""
        # 面積比較
        ...
        complexity_improvement = max(
            0, (original_complexity - corrected_complexity) / max(original_complexity, 0.001)
        )
