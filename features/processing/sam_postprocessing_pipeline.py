...
class SAMPostprocessingPipeline:
    ...
    def enhance_mask_quality(self, 
        ...
        try:
            ...
            try:
                ...
                contour_result = contour_enhancer.enhance_contour_quality(smoothed_mask, original_image)
    ...
    def _precision_edge_refinement(self, 
                                 mask: np.ndarray,
        ...
        try:
            ...
            if original_image is not None:
                edge_guided_refined = self._edge_guided_refinement(bilateral_refined, original_image)
            else:
    ...
    def _analyze_improvement_effect(self, 
                                  ...
                                  final_quality: Dict[str, Any]) -> Dict[str, float]:
        """改善効果分析"""
        try:
            initial_score = initial_quality.get('overall_score', 0.0)
            final_score = final_quality.get('overall_score', 0.0)
            
            improvement_ratio = ((final_score - initial_score) / max(initial_score, 0.001)) if initial_score > 0 else 0.0
