        ...
        for gpu_id in range(gpu_count):
            try:
                ...
                if is_usable:
                    ...
                    logger.info(
                        f"GPU {gpu_id}: {gpu_info.device_name} "
...
class MultiGPUSAMProcessor:
    ...
    def process_batch_parallel(self, input_files: List[str], output_dir: str) -> MultiGPUReport:
        ...
        with concurrent.futures.ThreadPoolExecutor(
            max_workers=len(self.gpu_manager.available_gpus)
        ) as executor:
            futures = {}
        ...
        logger.info(
            f"✅ マルチGPU並列処理完了: " f"{len(completed_tasks)}/{len(tasks)}成功 ({total_time:.1f}秒)"
        )
    ...
    def _fallback_cpu_processing(self, tasks: List[ProcessingTask]) -> MultiGPUReport:
        ...
        return MultiGPUReport(
            ...
            total_processing_time=total_time,
            gpu_usage_stats={},
            performance_metrics={"total_processing_time": total_time, "cpu_fallback": True},
        )
...
def main():
    ...
    report = processor.process_batch_parallel(
        input_files=[str(f) for f in input_files], output_dir=str(output_dir)
    )
