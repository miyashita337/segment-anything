...
class AdvancedBoundaryDetector:
    ...
    def __init__(
        ...
        logger.info(
            f"AdvancedBoundaryDetector初期化: panel={enable_panel_detection}, "
            f"multi_edge={enable_multi_stage_edge}, completion={enable_boundary_completion}"
        )

    def enhance_boundaries_advanced(
        ...
        if self.enable_boundary_completion:
            if mask is not None:
                completion_info, completion_enhanced = self._boundary_completion(
                    enhanced_image, mask
                )
    ...
    def _identify_panel_regions(
        self, boundaries: np.ndarray, image_shape: Tuple
    ) -> List[Dict[str, Any]]:
        """コマ領域の特定"""
    ...
    def _detect_character_overflow(
        self, image: np.ndarray, boundaries: np.ndarray
    ) -> List[Dict[str, Any]]:
        """コマ外はみ出しキャラクターの検出"""
    ...
    def _evaluate_enhancement_quality(
        self, original: np.ndarray, enhanced: np.ndarray, analysis_result: Dict[str, Any]
    ) -> float:
        """強化品質の総合評価"""
        # 各要素の品質スコア
        ...
        if analysis_result["boundary_completion"]:
            completion_score = analysis_result["boundary_completion"].get(
                "boundary_smoothness", 1.0
            )

        # 重み付き平均
        overall_quality = panel_score * 0.3 + min(edge_score, 1.0) * 0.4 + completion_score * 0.3
...
def test_advanced_boundary_detector():
    ...
    detector = AdvancedBoundaryDetector(
        enable_panel_detection=True, enable_multi_stage_edge=True, enable_boundary_completion=True
    )
