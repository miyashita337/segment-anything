...
class AdaptiveParameterOptimizer:
    ...
    def optimize_parameters_for_image(
        self, image: np.ndarray, initial_params: Optional[OptimizationParameters] = None
        ...
        try:
            ...
            logger.info(
                f"📊 画像特性: 明度={characteristics.brightness:.1f}, "
            ...
            # Step 6: アニメ特性ベース調整
            anime_adjusted = self._adjust_for_anime_characteristics(
                complexity_adjusted, characteristics
            )
    ...
    def _adjust_for_brightness(
        self, params: OptimizationParameters, characteristics: ImageCharacteristics
    ) -> OptimizationParameters:
        """明度ベース調整"""
        try:
    ...
    def _adjust_for_complexity(
        ...
        try:
            ...
            if complexity < 0.05:
                ...
                adjusted.sam_points_per_side = min(
                    64, adjusted.sam_points_per_side * 2
                )  # より多くのポイント
            ...
            # 高複雑度 (0.15-1.0) - 複雑な画像
                ...
                adjusted.noise_removal_threshold = int(
                    adjusted.noise_removal_threshold * 1.5
                )  # ノイズ除去強化
    ...
    def _adjust_for_anime_characteristics(
        self, params: OptimizationParameters, characteristics: ImageCharacteristics
    ) -> OptimizationParameters:
        """アニメ特性ベース調整"""
        try:
    ...
    def _generate_pattern_key(self, characteristics: ImageCharacteristics) -> str:
        ...
        try:
            ...
            brightness_level = (
                ...
                else ("bright" if characteristics.brightness > 180 else "normal")
            )
            contrast_level = (
                "low"
                if characteristics.contrast < 30
                else ("high" if characteristics.contrast > 80 else "normal")
            )
            complexity_level = (
                "simple"
                if characteristics.complexity < 0.05
                else ("complex" if characteristics.complexity > 0.15 else "normal")
            )
    ...
    def _update_optimization_history(
        self, characteristics: ImageCharacteristics, optimized_params: OptimizationParameters
    ) -> None:
        """最適化履歴更新"""
        try:
            history_entry = {
    ...
    def learn_from_quality_feedback(
        self,
        characteristics: ImageCharacteristics,
        quality_score: float,
        used_params: OptimizationParameters,
    ) -> None:
        """品質フィードバックからの学習"""
        try:
            pattern_key = self._generate_pattern_key(characteristics)
        ...
        try:
            ...
            if recent_entries:
                avg_brightness = np.mean(
                    [e["characteristics"]["brightness"] for e in recent_entries]
                )
                avg_contrast = np.mean([e["characteristics"]["contrast"] for e in recent_entries])
                avg_complexity = np.mean(
                    [e["characteristics"]["complexity"] for e in recent_entries]
                )
            else:
        ...
        try:
            ...
            for entry in self.optimization_history:
                ...
                save_data["optimization_history_summary"]["pattern_distribution"][pattern] = (
                    save_data["optimization_history_summary"]["pattern_distribution"].get(
                        pattern, 0
                    )
                    + 1
                )
