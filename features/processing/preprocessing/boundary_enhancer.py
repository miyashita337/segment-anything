...
class BoundaryEnhancer:
    ...
    def __init__(
        ...
        logger.info(
            f"BoundaryEnhancer初期化: skin_factor={skin_enhancement_factor}, "
    ...
    def _enhance_clothing_boundaries(self, image: np.ndarray) -> np.ndarray:
        ...
        # 黒領域の処理
        black_cleaned = cv2.morphologyEx(
            black_mask.astype(np.uint8), cv2.MORPH_CLOSE, kernel_smooth
        )
        black_dilated = cv2.dilate(black_cleaned, kernel_dilate, iterations=1)

        # 白領域の処理 - より慎重に
        white_cleaned = cv2.morphologyEx(
            white_mask.astype(np.uint8), cv2.MORPH_CLOSE, kernel_smooth
        )
    ...
    def _enhance_edges(self, image: np.ndarray) -> np.ndarray:
        ...
        edges_laplacian = np.clip(
            np.abs(edges_laplacian) / np.abs(edges_laplacian).max() * 255, 0, 255
        ).astype(np.uint8)
