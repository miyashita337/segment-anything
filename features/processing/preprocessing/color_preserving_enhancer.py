...
class ColorPreservingEnhancer:
    ...
    def __init__(
        ...
        logger.info(
            f"ColorPreservingEnhancer初期化: luminance={preserve_luminance}, "
    ...
    def enhance_image_boundaries(self, image: np.ndarray) -> Tuple[np.ndarray, Dict[str, float]]:
        ...
        if self.preserve_luminance or self.preserve_saturation:
            enhanced_image = self._preserve_color_properties(image, enhanced_image, original_stats)
    ...
    def _is_grayscale_image(self, image: np.ndarray) -> bool:
        ...
        logger.debug(f"グレースケール判定: max_diff={max_channel_diff:.3f}, " f"is_grayscale={is_gray}")
        return is_gray
    ...
    def _preserve_color_properties(
        self, original: np.ndarray, enhanced: np.ndarray, original_stats: Dict[str, float]
    ) -> np.ndarray:
        """色調特性を保持"""
        ...
        if self.preserve_saturation:
            ...
            if current_mean_s > 0:
                ...
                s_enh = np.clip(s_enh.astype(np.float32) * saturation_factor, 0, 255).astype(
                    np.uint8
                )
    ...
    def _gentle_edge_enhancement(self, image: np.ndarray) -> np.ndarray:
        ...
        for channel in range(3):
            ...
            channel_data[edge_pixels] = np.clip(channel_data[edge_pixels] * 1.1, 0, 255)  # 1.1倍のみ
    ...
    def _calculate_enhancement_metrics(
        ...
        return {
            ...
            "overall_quality": (
                enh_contrast / max(orig_contrast, 1.0) + max(0, 1.0 - color_change / 100.0)
            )
            / 2.0,
        }
