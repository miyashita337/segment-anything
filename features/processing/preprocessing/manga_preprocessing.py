...
class EffectLineRemover:
    ...
    def detect_radial_lines(
        self, image: np.ndarray, center: Optional[Tuple[int, int]] = None
    ) -> List[Tuple]:
        """
        放射状エフェクト線を検出
    ...
    def _is_radial_line(
        self, center: Tuple[int, int], line: Tuple[int, int, int, int], tolerance: int = 30
    ) -> bool:
        """直線が中心から放射状かチェック"""
        cx, cy = center
        x1, y1, x2, y2 = line
        ...
        # 直線から中心点までの距離を計算
        dist = abs((y2 - y1) * cx - (x2 - x1) * cy + x2 * y1 - y2 * x1) / math.sqrt(
            (y2 - y1) ** 2 + (x2 - x1) ** 2
        )
...
class MultiPanelSplitter:
    ...
    def _grid_based_split(
        self, image: np.ndarray, borders: List[Tuple[int, int, int, int]]
    ) -> List[Tuple[np.ndarray, Tuple[int, int, int, int]]]:
        """
        グリッドベースでのコマ分割
        """
    ...
    def find_best_panel_for_character(
        self, panels: List[Tuple[np.ndarray, Tuple[int, int, int, int]]], character_detection_func
...
class ScreentoneBoundaryProcessor:
    ...
    def detect_screentone_regions(self, image: np.ndarray) -> Tuple[np.ndarray, float]:
        ...
        high_freq_mask[
            center_h - mask_size : center_h + mask_size, center_w - mask_size : center_w + mask_size
        ] = 1
    ...
    def create_boundary_enhancement_mask(self, image: np.ndarray) -> Dict[str, Any]:
        ...
        return {
            ...
            "issue_types": {"screentone": screentone_conf > 0.3, "mosaic": mosaic_conf > 0.3},
        }
...
class SolidFillDetector:
    ...
    def detect_solid_regions(self, image: np.ndarray) -> Dict[str, Any]:
        ...
        adaptive_thresh = cv2.adaptiveThreshold(
            ...
            cv2.THRESH_BINARY,
            self.adaptive_threshold_window,
            5,
        )
    ...
    def separate_background_foreground(
        self, image: np.ndarray, solid_regions: List[Dict[str, Any]]
    ) -> Dict[str, Any]:
        """
        ソリッドフィル領域を背景と前景に分離
    ...
    def _is_background_region(self, region: Dict[str, Any], image: np.ndarray) -> bool:
        ...
        edge_threshold = 0.1
        is_near_edge = (
            centroid_x < w * edge_threshold
            or centroid_x > w * (1 - edge_threshold)
            or centroid_y < h * edge_threshold
...
class MangaPreprocessor:
    ...
    def preprocess_manga_image(
        self,
        image: np.ndarray,
        enable_effect_removal: bool = True,
        enable_panel_split: bool = True,
        enable_boundary_processing: bool = True,
        ...
        if enable_effect_removal:
            ...
            if density > 0.01:  # 閾値以上でエフェクト線処理
                ...
                result["processed_image"] = self.effect_remover.remove_effect_lines(
        ...
        if enable_solid_fill_detection:
            solid_fill_info = self.solid_fill_detector.detect_solid_regions(
                result["processed_image"]
            )
    ...
    def _apply_solid_fill_enhancement(
        self, image: np.ndarray, mask: np.ndarray, is_background: bool = True
    ) -> np.ndarray:
        """
        ソリッドフィル領域に対する画像強化処理
    ...
    def get_preprocessing_recommendations(self, image: np.ndarray) -> Dict[str, Any]:
        ...
        recommendations = {
            "boundary_processing": {
                ...
                "confidence": max(
                    boundary_info["screentone_confidence"], boundary_info["mosaic_confidence"]
                ),
            },
            "effect_removal": {
