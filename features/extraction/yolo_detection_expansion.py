...
class YOLODetectionExpander:
    ...
    def expand_detection_capabilities(
    ...
    def _calculate_adaptive_threshold(
        ...
        try:
            ...
            adaptive_threshold = max(
                self.config.adaptive_min, min(self.config.adaptive_max, adaptive_threshold)
            )
    ...
    def _expand_bounding_boxes(
        ...
        for mask in masks:
            try:
                ...
                if (
                    self.config.fullbody_aspect_ratio[0]
                    <= aspect_ratio
                    <= self.config.fullbody_aspect_ratio[1]
                ):
    ...
    def _optimize_fullbody_detection(
        self, masks: List[Dict], image_shape: Tuple[int, int, int]
    ) -> List[Dict]:
        """全身検出最適化"""
        optimized_masks = []
        ...
        for mask in masks:
            try:
                ...
                if fullbody_score > 0.7:
                    ...
                    logger.debug(
                        f"🏃 全身キャラクター検出: スコア {current_score:.3f} → {optimized_mask['yolo_score']:.3f}"
                    )
    ...
    def _calculate_fullbody_score(self, mask: Dict, image_shape: Tuple[int, int, int]) -> float:
        ...
        try:
            ...
            if (
                ...
                ideal_ratio = (
                    self.config.fullbody_aspect_ratio[0] + self.config.fullbody_aspect_ratio[1]
                ) / 2
            ...
            # 重み付き合計
            fullbody_score = (
                aspect_score * 0.4 + size_score * 0.3 + center_score * 0.2 + coverage_score * 0.1
            )
    ...
    def _calculate_anime_character_score(self, mask: Dict, image: np.ndarray) -> float:
        ...
        try:
            ...
            if is_grayscale:
                ...
                anime_score = (
                    color_score * 0.3 + edge_score * 0.4 + texture_score * 0.2 + shape_score * 0.1
                )
            else:
                # カラー画像: 従来の重み
                anime_score = (
                    color_score * 0.4 + edge_score * 0.3 + texture_score * 0.2 + shape_score * 0.1
                )

            # デバッグログ
            if logger.isEnabledFor(logging.DEBUG):
                logger.debug(
                    f"🎨 アニメスコア分析: グレースケール={is_grayscale}, "
    ...
    def _rescore_with_expanded_boxes(
        self, yolo_model: Any, masks: List[Dict], image: np.ndarray, threshold: float
    ) -> List[Dict]:
        """拡張境界ボックスでYOLOスコア再計算"""
        rescored_masks = []
            ...
            try:
                ...
                if expanded_bbox:
                    ...
                    if new_score >= threshold:
                        ...
                        logger.debug(
                            f"❌ 拡張スコア未達: {original_score:.3f} → {new_score:.3f} < {threshold:.3f}"
                        )
                else:
                    # 拡張なしの場合は元のスコアで判定
                    original_score = mask.get("yolo_score", 0.0)
    ...
    def _calculate_improvement_stats(
        self, original_masks: List[Dict], improved_masks: List[Dict]
    ) -> Dict[str, float]:
        """改善統計計算"""
        try:
            ...
            improved_scores = [
                m.get("rescored_yolo_score", m.get("yolo_score", 0.0)) for m in improved_masks
            ]
...
if __name__ == "__main__":
    ...
    config = DetectionConfig(
        base_threshold=0.07, adaptive_min=0.005, adaptive_max=0.15, expansion_ratio=0.3
    )
