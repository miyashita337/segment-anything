...
class ExtractionNotifier:
    ...
    def send_extraction_completion_notification(
        self, extraction_results: Dict[str, Any], include_images: bool = True
    ) -> bool:
        """
        抽出完了通知を送信
...
"""
                ...
                try:
                    ...
                    if success_image_paths:
                        ...
                        grid_path = self._create_success_images_grid(
                            success_image_paths, extraction_results.get("output_dir", "/tmp")
                        )
            ...
            success = self.pushover.send_notification(
                message=message, title="🎯 キャラクター抽出完了", priority=1 if success_rate >= 80 else 0
            )
    ...
    def _create_success_images_grid(self, image_paths: List[str], output_dir: str) -> Optional[str]:
        ...
        try:
            ...
            final_image = Image.new(
                "RGB", (grid_width, grid_height + header_height), (255, 255, 255)
            )
            ...
            output_path = (
                Path(output_dir)
                / f"extraction_success_grid_{Path().cwd().name}_{Path(image_paths[0]).stem}.jpg"
            )
...
def create_extraction_results_dict(
    total_images: int,
    ...
    return {
        ...
        "improvement_suggestions": ["YOLO閾値調整による検出精度向上", "SAM後処理パイプライン最適化", "困難姿勢への対応強化"],
    }
