...
@click.option(
    "--max-files", type=int, help="OPTET-016: Maximum number of files to process in batch mode"
)
@click.option("--resume", is_flag=True, help="OPT-017: Resume from checkpoint if available")
@click.option(
    "--sam-optimization-profile",
...
def extract_character(
    ...
    if batch:
        ...
        stable_processor = StableBatchProcessor(
            checkpoint_dir=str(checkpoint_dir), micro_batch_size=3  # OPT-017: マイクロバッチサイズ
        )
        ...
        if GOOGLE_SHEETS_HOOK_AVAILABLE:
            try:
                ...
                update_extraction_status(
                    "PHS-006", "start", dataset_name=dataset_name, total_images=total_images
                )
                if verbose:
        ...
        if verbose:
            ...
            if stats:
                click.echo(
                    f"📊 統計: 成功 {stats.get('success_count', 0)}件, "
        ...
        if verbose:
            try:
                ...
                click.echo(
                    f"    最適化実行: GC {opt_stats['gc_calls']}回, "
        ...
        if mask is not None:
                    ...
                    try:
                        ...
                        if pipeline_result.success and pipeline_result.final_mask is not None:
                            ...
                            if verbose:
                                ...
                                click.echo(
                                    f"🚀 3段階改善完了: {pipeline_result.initial_quality:.3f} → {pipeline_result.final_quality:.3f} "
                                    f"({improvement_pct:+.1f}%, リトライ {pipeline_result.retry_count}回)"
                                )
                        else:
                            if verbose:
                                click.echo(
                                    f"⚠️ 3段階改善未達（元マスク使用）: {pipeline_result.final_quality:.3f}"
                                )
            ...
            try:
                ...
                if verbose:
                    ...
                    click.echo(
                        f"✅ 抽出完了: {input_path.name} ({pipeline_status}版, 品質: {quality_score:.3f})"
                    )
                return final_mask
            except Exception as e:
                if verbose:
    ...
    try:
                ...
                if adaptive_cropping and len(persons) > 1:
                    try:
                        ...
                        if detection_boxes:
                            ...
                            optimized_bbox = adaptive_cropper.adaptive_crop(
                                image, primary_detection
                            )
        ...
        if best_mask:
            ...
            if "face_validation" in best_mask:
                ...
                print(
                    f"✅ Selected mask validation: faces={face_val['face_count']}, "
...
def _select_best_mask_with_fallback(
    masks: list, image_shape: tuple, primary_method: str = "balanced"
) -> Optional[dict]:
...
def extract_character_from_image(
    image: np.ndarray, mask: np.ndarray, background_color: Tuple[int, int, int] = (0, 0, 0)
) -> np.ndarray:
    """
...
def save_character_result(image: np.ndarray, mask: np.ndarray, output_path: str) -> bool:
    """
...
def save_extracted_character(image: ImageType, mask: MaskType, output_path: Path) -> None:
    ...
    try:
        ...
        print(
            f"✅ QC成功版保存完了: {output_path.name} (サイズ: {cropped_image.shape[1]}x{cropped_image.shape[0]})"
        )
...
def generate_character_mask_qc_style(
    ...
    try:
        ...
        if len(persons) == 0:
            ...
            if verbose:
                print(
                    f"🎯 QC成功版最大面積選択: {max_person['area']}px, confidence={max_person['confidence']:.3f}"
                )
