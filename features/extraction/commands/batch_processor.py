...
class BatchProcessor(BaseExtractionCommand):
        ...
        if self.config.max_files and self.config.max_files > 0:
            ...
            if self.config.max_files < len(image_files):
                ...
                if self.config.verbose:
                    self.logger.info(
                        f"Batch size limited: {original_count} → {self.config.max_files}"
                    )

        # Initialize batch processor
        checkpoint_dir = output_dir / ".checkpoint"
        stable_processor = StableBatchProcessor(
            checkpoint_dir=str(checkpoint_dir), micro_batch_size=3
        )
        ...
        if SHEETS_AVAILABLE:
            try:
                ...
                update_extraction_status(
                    "P1-021", "start", dataset_name=dataset_name, total_images=len(image_files)
                )
                if self.config.verbose:
    ...
    def _process_batch(
        self, image_files: List[Path], output_dir: Path, stable_processor: StableBatchProcessor
    ) -> Dict[str, Any]:
        ...
        success_rate = (
            len(self.successful_extractions) / total_images * 100 if total_images > 0 else 0
        )
