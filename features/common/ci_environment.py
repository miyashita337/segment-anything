...
class CIEnvironmentDetector:
    ...
    def detect_ci_environment() -> CIConfiguration:
        ...
        # CI環境検出
        is_ci = (
            os.getenv('CI_ENVIRONMENT', 'false').lower() == 'true' or
        ...
        cpu_only = (
            is_ci or 
            os.getenv('CPU_ONLY', 'false').lower() == 'true'
        )
        
        memory_limit_disabled = (
            is_ci or
            os.getenv('MEMORY_LIMIT_DISABLED', 'false').lower() == 'true'
        )
    ...
    def log_environment_info():
        ...
        # 環境変数の詳細
        env_vars = [
            'CI', 'CI_ENVIRONMENT', 'GITHUB_ACTIONS',
            'CPU_ONLY', 'MEMORY_LIMIT_DISABLED',
            'YOLO_MODEL', 'SAM_MODEL', 'MAX_PROCESSING_TIME'
        ]
