...
class StrictPathValidator:
    ...
    def validate_input_path(self, 
                          ...
                          context: str = "入力パス") -> StrictValidationResult:
        """
        入力パスの厳密検証
        ...
        return StrictValidationResult(
            ...
            path=path,
            issues=issues,
            author_work_info=author_work_info
        )
    
    def validate_output_path(self, 
                           output_path: Optional[Union[str, Path]], 
                           context: str = "出力パス") -> StrictValidationResult:
        """
        出力パスの厳密検証
    ...
    def interactive_path_input(self, 
                             prompt: str, 
                             path_type: str = "input",
                             default_suggestion: Optional[str] = None) -> Optional[Path]:
        """
        対話的パス入力
    ...
    def validate_paths_comprehensive(self, 
                                   input_path: Optional[Union[str, Path]], 
                                   output_path: Optional[Union[str, Path]]) -> Tuple[StrictValidationResult, StrictValidationResult]:
        """
        入力・出力パスの包括的検証
        ...
        if self.interactive_mode:
            if not input_path:
                input_path = self.interactive_path_input(
                    "🔍 画像入力ディレクトリを指定してください:",
                    "input"
                )
            
            if not output_path:
                output_path = self.interactive_path_input(
                    "📁 抽出結果出力ディレクトリを指定してください:",
                    "output"
                )
    ...
    def log_validation_summary(self, 
        ...
        if input_result.has_warnings:
            all_warnings.extend([f"入力: {issue.message}" for issue in input_result.issues if issue.severity == ValidationSeverity.WARNING])
        if output_result.has_warnings:
            all_warnings.extend([f"出力: {issue.message}" for issue in output_result.issues if issue.severity == ValidationSeverity.WARNING])
...
# 便利な関数
def validate_strict_paths(input_path: Optional[Union[str, Path]], 
                         output_path: Optional[Union[str, Path]], 
                         strict_mode: bool = True,
                         interactive_mode: bool = False,
                         require_author_structure: bool = False) -> Tuple[Optional[Path], Optional[Path]]:
    """
    厳密パス検証の便利関数
