...
class BackupManager:
    ...
    def _create_archive(self, backup_path: Path, files: List[Path], backup_name: str) -> None:
        ...
        if file_size_mb > self.config.max_backup_size_mb:
            logger.warning(
                f"⚠️ バックアップサイズが上限超過: {file_size_mb:.1f}MB > {self.config.max_backup_size_mb}MB"
            )

        logger.info(f"📦 アーカイブ作成完了: {backup_path} ({file_size_mb:.1f}MB)")

    def _create_backup_metadata(
        self, backup_path: Path, files: List[Path], backup_name: str
    ) -> None:
        """バックアップメタデータ作成"""
        metadata = {
    ...
    def list_backups(self) -> List[Dict]:
        ...
        tar_gz_files = [
            f for f in all_files if f.name.endswith(".tar.gz") and not f.name.endswith(".meta.json")
        ]
        tar_files = [
            f
            for f in all_files
            if f.name.endswith(".tar")
            and not f.name.endswith(".tar.gz")
...
def create_backup_manager(
    workspace_path: Path, config: Optional[BackupConfig] = None
) -> BackupManager:
    """バックアップマネージャー作成"""
...
def backup_extraction_results(
    workspace_path: Path, backup_name: Optional[str] = None
) -> Optional[Path]:
    """
    抽出結果バックアップ（フック関数）
