...
class IOOptimizer:
    ...
    async def safe_file_write(
        self, file_path: Union[str, Path], content: Union[str, bytes], encoding: str = "utf-8"
    ) -> bool:
        """競合回避ファイル書き込み"""
    ...
    async def batch_file_operations(
        ...
        for i in range(0, total_ops, self.batch_size):
            ...
            for op in batch:
                if op["type"] == "write":
                    task = self.safe_file_write(
                        op["path"], op["content"], op.get("encoding", "utf-8")
                    )
                elif op["type"] == "read":
    ...
    async def google_sheets_safe_update(
        self, update_func: Callable[[], Any], post_update_delay: float = 1.0
    ) -> Any:
        """Google Sheets更新の安全実行（ファイル競合回避）"""
        async with self._io_operation_context("google_sheets_update"):
            try:
                # Google Sheets更新実行
                result = await asyncio.get_event_loop().run_in_executor(None, update_func)
    ...
    async def optimized_report_generation(
        self, reports: Dict[str, Any], output_dir: Union[str, Path]
    ) -> Dict[str, bool]:
        ...
        for report_name, content in reports.items():
            ...
            if isinstance(content, dict) and "markdown" in content:
                ...
                operations.append(
                    {"type": "write", "path": md_path, "content": content["markdown"]}
                )

            # HTML レポート（内容に応じて）
            if isinstance(content, dict) and "html" in content:
                html_path = output_dir / f"{report_name}.html"
                operations.append({"type": "write", "path": html_path, "content": content["html"]})
    ...
    async def workspace_sync_workflow(
        ...
        try:
            ...
            if reports_data:
                report_results = await self.optimized_report_generation(reports_data, workspace_dir)
...
if __name__ == "__main__":
    ...
    async def test_io_optimizer():
        ...
        try:
            ...
            for i in range(5):
                operations.append(
                    {"type": "write", "path": test_dir / f"batch_{i}.txt", "content": f"バッチテスト {i}"}
                )
