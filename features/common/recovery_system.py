...
class ProcessMonitor:
    ...
    def get_system_status(self) -> SystemStatus:
        ...
        try:
            ...
            # プロセス生存確認
            process_alive = any(
                proc.name() == self.process_name for proc in psutil.process_iter(["name"])
            )
...
class AutoRecoverySystem:
    ...
    def attempt_recovery(self, recovery_state: RecoveryState, error_context: str) -> bool:
        ...
        self.logger.info(
            f"復旧試行 {recovery_state.retry_count}/{recovery_state.max_retries} - {delay}秒待機"
        )
        ...
        if issues:
            ...
            # 重大な問題がある場合は復旧を延期
            if (
                status.memory_percent > self.config["memory_threshold"]
                or status.disk_free_gb < self.config["disk_threshold"]
            ):
                self.logger.error("重大なシステム問題により復旧を中断")
                return False

        self.logger.info(
            f"復旧試行準備完了: CPU {status.cpu_percent:.1f}%, RAM {status.memory_percent:.1f}%"
        )
        return True
