...
class InteractivePathInput:
    ...
    def __init__(self, use_colors: bool = True, max_attempts: int = 3, auto_suggest: bool = True):
        """
        初期化
        ...
        self.colors = (
            ...
            if use_colors
                ...
                key: ""
                for key in ["reset", "bold", "red", "green", "yellow", "blue", "purple", "cyan"]
            }
        )
                ...
                if Path(base_path).exists():
                    try:
                        for item in Path(base_path).iterdir():
                            if item.is_dir() and (
                                not partial_input or partial_input.lower() in item.name.lower()
                            ):
    ...
    def _show_suggestions(self, input_prompt: InputPrompt, partial_input: str = ""):
        ...
        if suggestions:
            ...
            for i, suggestion in enumerate(suggestions[:10], 1):
                # パスの存在チェック
                status = ""
                if input_prompt.input_type in [
                    InputType.INPUT_DIRECTORY,
                    InputType.OUTPUT_DIRECTORY,
                ]:
    ...
    def prompt_for_input(self, input_prompt: InputPrompt) -> InteractiveResult:
        ...
        while attempt < self.max_attempts:
            ...
            try:
                ...
                if user_input.lower() in ["cancel", "quit"]:
                    return InteractiveResult(success=False, value=None, cancelled=True)
                ...
                return InteractiveResult(success=True, value=user_input, path=result_path)
        ...
        return InteractiveResult(
            success=False, value=None, error_message=f"{self.max_attempts}回の試行に失敗しました"
        )
    ...
    def prompt_for_paths(
        ...
        output_context: str = "抽出結果出力ディレクトリ",
    ) -> Dict[str, Any]:
        """
        入力・出力パスの対話的プロンプト
...
def interactive_setup(title: str = "QUAL-033 厳密パス検証システム") -> Dict[str, Any]:
    ...
    results = {"success": False, "paths": {}, "tracker_info": {}, "cancelled": False, "errors": []}
