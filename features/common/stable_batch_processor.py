...
class CheckpointManager:
    ...
    def save_progress(
        ...
        try:
            ...
            self.logger.info(
                f"🔧 根本修正版チェックポイント保存: {len(processed_files)}/{len(original_total_files)} 完了"
            )
        except Exception as e:
        ...
        try:
            ...
            if "original_total_files" in checkpoint_data:
                ...
                checkpoint_data["remaining_files"] = [
                    f
                    for f in checkpoint_data["original_total_files"]
                    if f not in checkpoint_data["processed_files"]
                ]
                self.logger.info(
                    f"🔧 v2.0形式検出: 完全ファイルリスト {len(checkpoint_data['original_total_files'])} 件復元"
                )
...
class StableBatchProcessor:
    ...
    def process_with_checkpoint(
        ...
        if resume:
            ...
            if checkpoint_data:
                ...
                if "original_total_files" in checkpoint_data:
                    ...
                    self.logger.info(
                        f"🔧 v2.0形式: 完全リスト {len(checkpoint_data['original_total_files'])} から残り {len(remaining_files)} 件計算"
                    )
                else:
        ...
        for batch_idx, micro_batch in enumerate(micro_batches):
                    ...
                    try:
                        ...
                        if success:
                            ...
                            if retry_count < self.max_retries:
                                ...
                                self.logger.warning(
                                    f"🔄 リトライ {retry_count}/{self.max_retries}: {message}"
                                )
                                ...
                                self.stats["processed_files"] += 1
                                self.logger.error(
                                    f"❌ 失敗: {os.path.basename(file_path)} - {message}"
                                )
                        ...
                        if retry_count < self.max_retries:
                            ...
                            self.stats["retry_count"] += 1
                            self.logger.warning(
                                f"🔄 例外リトライ {retry_count}/{self.max_retries}: {str(e)}"
                            )
                            time.sleep(self.retry_delay_seconds)
                        else:
                            self.stats["error_count"] += 1
            ...
            self.logger.info(
                f"📊 進捗: {len(processed_files)}/{len(original_total_files)} ({progress_percent:.1f}%)"
            )

        # 処理完了
        self.checkpoint_manager.clear_checkpoint()
        self.logger.info(
            f"🎯 バッチ処理完了: 成功 {self.stats['success_count']}, エラー {self.stats['error_count']}"
        )

        return self._generate_result(True, "正常完了")

    def _save_current_progress(
        self, processed_files: List[str], original_total_files: List[str]
    ) -> None:
        """現在の進捗を保存"""
...
if __name__ == "__main__":
    ...
    processor = StableBatchProcessor(
        checkpoint_dir="/tmp/p1_019_checkpoint", micro_batch_size=3, max_retries=2
    )

    result = processor.process_with_checkpoint(
        files=test_files, process_function=sample_processing_function, output_dir="/tmp/test_output"
    )
