    ...
    def generate_learning_insights(self) -> Dict[str, Any]:
        ...
        insights = {
            "dataset_summary": {
                ...
                "success_rate": char_stats["files_with_character"]
                / max(char_stats["total_files"], 1),
            },
            "character_features": {
            ...
            "extraction_patterns": {
                ...
                "dominant_type": max(type_stats.items(), key=lambda x: x[1])[0]
                if type_stats
                else "unknown",
            },
        ...
        if char_stats["files_with_character"] > 80:
            insights["learning_recommendations"].append("豊富なラベルデータ（80+）が利用可能。深層学習アプローチが有効。")

        if char_stats.get("aspect_ratio_stats", {}).get("std", 0) > 0.5:
            insights["learning_recommendations"].append("アスペクト比の変動が大きい。多様な抽出範囲に対応する必要がある。")

        if type_stats.get("full_body", 0) > type_stats.get("face_close", 0):
            insights["learning_recommendations"].append("全身抽出が多い。身体検出機能の強化が重要。")
    ...
    def create_visualization_report(self, output_dir: Path):
        ...
        try:
            ...
            if char_stats["aspect_ratios"]:
                ...
                plt.hist(
                    char_stats["aspect_ratios"], bins=20, alpha=0.7, color="blue", edgecolor="black"
                )
                ...
                plt.savefig(
                    output_dir / "aspect_ratio_distribution.png", dpi=150, bbox_inches="tight"
                )
                plt.close()
