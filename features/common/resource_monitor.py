...
class ResourceMonitor:
    ...
    def _optimize_memory(self) -> List[str]:
        ...
        try:
            ...
            if processes:
                ...
                actions.append(
                    f"メモリ消費プロセス特定: {top_process['name']} " f"({top_process['memory_percent']:.1f}%)"
                )
        except Exception as e:
    ...
    def _log_status_summary(self, status: ResourceStatus):
        ...
        if status.gpu_count > 0:
            ...
            if gpu_memory_percents:
                ...
                avg_gpu_temp = (
                    sum(status.gpu_temperature) / len(status.gpu_temperature)
        ...
        logger.info(
            f"📊 リソース状況 - CPU: {status.cpu_percent:.1f}%, "
    ...
    def get_statistics(self) -> Dict:
        ...
        stats = {
            "monitoring_duration_minutes": len(self.resource_history)
    ...
    def export_report(self, output_path: Optional[Path] = None) -> Path:
        ...
        if output_path is None:
            ...
            output_path = (
                PROJECT_ROOT / "logs" / "resource_monitoring" / f"resource_report_{timestamp}.json"
            )
