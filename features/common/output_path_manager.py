...
class OutputPathManager:
    ...
    def __init__(self, tracker_id: str, base_path: Optional[Path] = None):
        ...
        # ワークスペース構造初期化
        self.workspace = WorkspaceStructure(base_path=self.base_path, tracker_id=tracker_id)
    ...
    def ensure_output_dir(
        self, category: OutputCategory, subcategory: Optional[str] = None
    ) -> Path:
        """
        出力ディレクトリ確保（作成）
    ...
    def validate_compliance(self) -> Dict[str, Any]:
        ...
        Returns:
            検証結果の詳細
        """
        results = {"compliant": True, "issues": [], "recommendations": []}
        ...
        for name, path in structure.items():
            if name != "root":  # ルートは自動作成される
                self.ensure_output_dir(
                    OutputCategory(name)
                    if name in [e.value for e in OutputCategory]
                    else OutputCategory.TEMP
                )
