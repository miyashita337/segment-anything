...
class IntegratedQualityMonitor:
    ...
    def run_quality_check(self, extraction_results_dir: Path) -> QualityResult:
        ...
        # 抽出画像の品質評価
        image_files = list(extraction_results_dir.glob("*.jpg")) + list(
            extraction_results_dir.glob("*.png")
        )
        ...
        success_rate = (
            (quality_grades["A"] + quality_grades["B"]) / total_processed
        ...
        if self.config.alert_on_degradation:
            result.degradation_detected, result.baseline_comparison = self._check_degradation(
                result
            )
    ...
    def _check_degradation(self, current: QualityResult) -> Tuple[bool, Optional[Dict[str, float]]]:
        ...
        try:
            ...
            degradation_detected = (
                success_rate_drop > self.config.degradation_threshold
                or avg_score_drop > self.config.degradation_threshold
            )
            ...
            if degradation_detected:
                logger.warning(
                    f"⚠️ 品質劣化検出: 成功率変化 {-success_rate_drop:+.1%}, スコア変化 {-avg_score_drop:+.3f}"
                )
    ...
    def _update_baseline_if_needed(self, result: QualityResult):
        ...
        try:
            ...
            if len(history) % self.config.baseline_update_frequency == 0:
                ...
                avg_success_rate = sum(r["success_rate"] for r in recent_results) / len(
                    recent_results
                )
                avg_quality_score = sum(r["avg_quality_score"] for r in recent_results) / len(
                    recent_results
                )
                ...
                logger.info(
                    f"📊 品質ベースライン更新: 成功率 {avg_success_rate:.1%}, 平均スコア {avg_quality_score:.3f}"
                )
...
def create_quality_monitor(
    workspace_path: Path, config: Optional[QualityMonitoringConfig] = None
...
def run_integrated_quality_check(
    workspace_path: Path, tracker_id: Optional[str] = None
