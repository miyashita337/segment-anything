...
def validate_input_file(input_path: Union[str, Path], description: str = "入力ファイル") -> Path:
    """
    入力ファイルの存在チェック（統一仕様）
...
def validate_output_directory(
    output_path: Union[str, Path], description: str = "出力ディレクトリ", create_if_missing: bool = True
) -> Path:
    """
    出力ディレクトリの検証・作成
...
def log_validation_summary(
    input_paths: List[Path], output_paths: List[Path], script_name: str = "処理スクリプト"
):
    """
    検証結果のサマリーログ出力
...
# 使いやすいラッパー関数
def check_input_dir_with_images(input_dir: Union[str, Path], min_images: int = 1) -> Path:
    """
    画像ディレクトリの簡易チェック
