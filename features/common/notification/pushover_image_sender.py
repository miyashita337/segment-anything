...
class PushoverImageSender:
    ...
    def send_extraction_complete_with_images(
        self, tracker_id: str, image_paths: List[str], extraction_stats: Dict[str, Any]
    ) -> Dict[str, Any]:
        """
        抽出完了通知を画像付きで送信
    ...
    def _send_image_batch(
        ...
        try:
            ...
            message = self._create_batch_message(
                tracker_id, batch_number, total_batches, len(batch_images), extraction_stats
            )
            ...
            response = requests.post(
                "https://api.pushover.net/1/messages.json", data=data, files=files, timeout=30
            )
            ...
            return PushoverSendResult(success=False, error=str(e))
