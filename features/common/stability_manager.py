...
class StabilityManager:
    ...
    def __init__(
        ...
        logger.info(
            f"StabilityManager初期化: メモリ={self.memory_enabled}, I/O={self.io_enabled}, 遅延={self.lazy_enabled}"
        )
    ...
    async def safe_character_extraction(
        ...
        **extraction_kwargs,
        ...
        result = {"success": False, "extraction_result": None, "stability_stats": {}}
        ...
        try:
            ...
            if self.memory_monitor:
                ...
                if pressure["ram_pressure"]:
                    ...
                    logger.info(
                        f"事前メモリクリーンアップ: RAM解放={cleanup_result.get('ram_freed_gb', 0):.2f}GB"
                    )
            ...
            extraction_result = await asyncio.get_event_loop().run_in_executor(
                None, extraction_func, str(input_path), str(output_path), **extraction_kwargs
            )
    ...
    async def safe_batch_processing(
        ...
        **batch_kwargs,
        ...
        try:
            ...
            for i in range(0, len(input_items), batch_size):
                ...
                try:
                    batch_result = await asyncio.get_event_loop().run_in_executor(
                        None, batch_func, batch, str(output_dir), **batch_kwargs
                    )

                    result["batch_results"].append(
                        {"batch_num": batch_num, "success": True, "result": batch_result}
                    )
                    ...
                    result["batch_results"].append(
                        {"batch_num": batch_num, "success": False, "error": str(batch_error)}
                    )
            ...
            logger.info(
                f"安定性最適化バッチ処理完了: 成功={result['processed_count']}, 失敗={result['failed_count']}"
            )
    ...
    async def integrated_tracker_workflow(
        ...
        result = {"tracker_id": tracker_id, "success": False, "phases": {}, "stability_stats": {}}
...
async def stable_extract_character(input_path: str, output_path: str, **kwargs):
    ...
    return await manager.safe_character_extraction(
        extraction_func=extract_func, input_path=input_path, output_path=output_path, **kwargs
    )


# デコレータ: 統合安定性管理
def with_stability_management(
    enable_memory: bool = True, enable_io: bool = True, enable_lazy: bool = True
):
    """
    関数デコレータ: 統合安定性管理適用
