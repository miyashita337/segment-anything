...
class APIConfig:
    ...
    def _get_default_config(self) -> Dict[str, Any]:
        ...
        return {
            ...
            "openai": {"api_key": "", "model": "gpt-4o", "timeout": 30, "max_retries": 3},
            "claude": {
                "api_key": "",
...
if __name__ == "__main__":
    ...
    print("=== API設定テスト ===")
    print(
        f"Gemini APIキー: {config.get_gemini_api_key()[:20] if config.get_gemini_api_key() else 'なし'}..."
    )
    print(
        f"OpenAI APIキー: {config.get_openai_api_key()[:20] if config.get_openai_api_key() else 'なし'}..."
    )
    print(
        f"Claude APIキー: {config.get_claude_api_key()[:20] if config.get_claude_api_key() else 'なし'}..."
    )
