...
class SampleSizeValidator:
    ...
    def __init__(self, default_power: float = 0.80, default_alpha: float = 0.05):
        """
        初期化
    ...
    def calculate_required_sample_size(
        self, test_type: TestType, effect_size: float, power: float = None, alpha: float = None
    ) -> int:
        """
        統計検定に必要な最小サンプル数を計算
    ...
    def _calculate_one_sample_t_size(self, effect_size: float, power: float, alpha: float) -> int:
        """
        1標本t検定の必要サンプル数計算
    ...
    def _calculate_two_sample_t_size(self, effect_size: float, power: float, alpha: float) -> int:
        """
        2標本t検定の必要サンプル数計算（各群）
    ...
    def _calculate_paired_t_size(self, effect_size: float, power: float, alpha: float) -> int:
        """
        対応のあるt検定の必要サンプル数計算
    ...
    def _calculate_proportion_size(self, effect_size: float, power: float, alpha: float) -> int:
        """
        比率検定の必要サンプル数計算
    ...
    def calculate_confidence_interval_width(
        self, sample_size: int, confidence_level: float = 0.95
    ) -> float:
        """
        信頼区間の幅を計算
    ...
    def validate_sample_adequacy(
        ...
        for scenario in test_scenarios:
            required_n = self.calculate_required_sample_size(
                scenario["test_type"], scenario["effect_size"]
            )

            confidence_width = self.calculate_confidence_interval_width(current_sample_size)
            ...
            if not is_adequate:
                ...
                suggestions.append(f"{scenario['description']}には最低{required_n}サンプル推奨")
        ...
        if requirements:
            ...
            current_power = self._calculate_current_power(
                current_sample_size, worst_case.effect_size, worst_case.test_type
            )
        else:
            current_power = 0.0
    ...
    def _calculate_current_power(
        self, current_n: int, effect_size: float, test_type: TestType
    ) -> float:
        """
        現在のサンプル数での検出力を計算
...
def main():
    ...
    for req in validation.sample_requirements:
        ...
        print(
            f"  {status} {req.test_type.value}: {req.current_n}/{req.required_n} "
