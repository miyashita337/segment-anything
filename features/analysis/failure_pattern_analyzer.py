...
class FailurePatternAnalyzer:
    ...
    def analyze_failure_patterns(
        self, failed_images_dir: Path, success_images_dir: Optional[Path] = None
    ) -> Dict[str, Any]:
        """
        失敗パターンを分析
    ...
    def _classify_patterns(
        self, features: np.ndarray, paths: List[Path], clustering_results: Dict
    ) -> Dict[str, Any]:
        """失敗パターンの分類と命名"""
        patterns = {}
    ...
    def _compare_with_success(
        self, failed_features: np.ndarray, success_features: np.ndarray
    ) -> Dict[str, Any]:
        """成功画像との比較分析"""
        # 統計的差異
        ...
        top_differences = {
            ...
            else f"feature_{idx}": float(difference[idx])
...
def main():
    ...
    results = analyzer.analyze_failure_patterns(
        Path(args.failed_dir), Path(args.success_dir) if args.success_dir else None
    )
