...
async def async_file_operation(filepath: str) -> Dict[str, Any]:
    ...
    return {"filepath": filepath, "size_mb": np.random.randint(1, 10), "loaded": True}
...
def example_parallel_preprocessing():
    ...
    # 並列プロセッサー作成
    parallel_processor = ParallelProcessor(max_workers=4, use_processes=True, chunk_size=10)

    # 並列処理実行
    start_time = time.time()
    results = parallel_processor.process_batch(simulate_image_preprocessing, image_paths)
...
async def example_async_file_loading():
    ...
    # 非同期処理実行
    start_time = time.time()
    results = await async_processor.process_async_batch(async_file_operation, filepaths)
    total_time = time.time() - start_time
...
def example_pipeline_processing():
    ...
    # パイプラインステージ定義
    stages = [simulate_image_preprocessing, simulate_yolo_detection, simulate_sam_segmentation]

    # パイプラインプロセッサー作成
    pipeline_processor = PipelineProcessor(stages=stages, buffer_size=20, max_workers_per_stage=2)
...
def example_gpu_batch_processing():
    ...
    # GPU バッチ処理実行
    start_time = time.time()
    results = gpu_processor.process_tensors_batch(gpu_model_inference, test_tensors)
    total_time = time.time() - start_time
...
def performance_comparison():
    ...
    start_time = time.time()
    thread_results = parallel_thread.process_batch(simulate_image_preprocessing, test_data)
    ...
    start_time = time.time()
    process_results = parallel_process.process_batch(simulate_image_preprocessing, test_data)
