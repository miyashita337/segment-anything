...
class IntegratedDashboardServer:
    ...
    def _log_access(self, request):
        ...
        try:
            ...
            client_ip = (
                ...
                or getattr(request, "transport", {}).get("peername", ["Unknown"])[0]
            ...
            logger.info(
                f"🌐 [{timestamp}] {client_ip} - {request.method} {request.path} - {user_agent}"
            )
        except Exception as e:
    ...
    def _setup_routes(self):
        ...
        self.app.router.add_get(
            "/tracker/{tracker_id}/{dashboard_name}", self.handle_tracker_dashboard
        )
    ...
    async def handle_tracker_dashboard(self, request):
        ...
        nav_html = self._generate_navigation_wrapper(
            f"{tracker_id} - {dashboard_name}", dashboard_key
        )
        return web.Response(text=nav_html, content_type="text/html")
        ...
        if file_path.exists() and file_path.is_file():
            ...
            if file_path.suffix.lower() == ".html":
                ...
                with open(file_path, "rb") as f:
                    content = f.read()
                return web.Response(
                    body=content, content_type=mime_type or "application/octet-stream"
                )
    ...
    async def handle_refresh(self, request):
        ...
        self._scan_dashboards()
        return web.json_response(
            {"status": "success", "message": f"{len(self.dashboards)} dashboards rescanned"}
        )
...
<body>
    ...
    def _is_extracted_image(self, path):
        ...
        if (
            any(pattern in path_lower for pattern in allowed_patterns)
    ...
    def _get_workspace_for_dashboard(self, dashboard_path):
        ...
        for path_part in dashboard_path.parts:
            if "tracker-workspace" in str(path_part):
                ...
                train_path_idx = next(
                    (i for i, p in enumerate(dashboard_path.parts) if p == "train"), None
                )
    ...
    def _generate_navigation_wrapper(self, title, current_key):
        ...
        others = sorted(
            [t for t in trackers.keys() if not t.startswith(("QUAL-", "OPTM-", "TEST-", "INTG-"))]
        )
        ...
        if qual_trackers:
            ...
            for tracker in qual_trackers:
                ...
                nav_items.append(
                    f'<a href="/tracker/{tracker}" class="nav-item {active}">{tracker}</a>'
                )
