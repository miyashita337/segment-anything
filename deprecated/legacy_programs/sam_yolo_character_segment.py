...
class CharacterQualityEvaluator:
    ...
    def evaluate_character_quality(
        ...
        for mask_idx in self.selected_masks:
            if mask_idx < len(self.masks):
                ...
                resized_mask = cv2.resize(
                    mask.astype(np.uint8), (self.display_shape[1], self.display_shape[0])
                )
...
class SAMYOLOCharacterSegmentor:
    ...
    def __init__(
        ...
        try:
            ...
            self.sam_optimizer = SAMOptimizationConfig()

            # P1-020最適化プロファイル適用（6分→2.5分目標）
            optimization_profile = getattr(args, "sam_optimization_profile", "p1_020_optimized")
            self.mask_generator = create_optimized_sam_generator(
                self.sam, self.sam_optimizer, optimization_profile
            )
            print(f"🚀 P1-020 SAM最適化適用: {optimization_profile}")

        except ImportError:
            # フォールバック: 従来の設定
            print("⚠️ P1-020最適化システム利用不可、従来設定を使用")
    ...
    def filter_masks_with_yolo(
        ...
        # 閾値以上のマスクのみ返す
        filtered_masks = [
            (mask, score) for mask, score in mask_scores if score >= self.score_threshold
        ]
        ...
        if mask_scores:
            ...
            print(
                f"YOLO検出スコア統計: 最高={max(all_scores):.3f}, 最低={min(all_scores):.3f}, 平均={np.mean(all_scores):.3f}"
            )
    ...
    def process_single_image(
        self, image_path: str, output_dir: str, interactive: bool = False, mask_choice: int = None
    ) -> str:
        """
        単一画像を処理
        ...
        try:
            ...
            if interactive:
                return self._process_interactive(
                    image, filtered_masks, image_path, output_dir, mask_choice
                )
            else:
    ...
    def process_choice_mode(self, input_dir: str, output_dir: str) -> None:
        ...
        if hasattr(args, "max_files") and args.max_files and args.max_files > 0:
            if args.max_files < len(image_files):
                image_files = image_files[: args.max_files]
                print(f"🔢 P1-018バッチサイズ制御: {original_count}枚 → {args.max_files}枚に制限")
        ...
        for i, image_file in enumerate(image_files):
            ...
            try:
                ...
                result = self._process_choice_mode(
                    image, filtered_masks, str(image_file), str(output_subdir)
                )
    ...
    def _save_result(
        self, image: np.ndarray, mask: np.ndarray, image_path: str, output_dir: str
    ) -> str:
        """
        結果を保存
        """
            ...
            try:
                ...
                if selection_result is not None:
                    ...
                    if (
                        ...
                        and selection_result.get("type") == "multi_select"
                    ):
                        ...
                        print(
                            f"保存完了: {output_path} (マスク{selected_masks}を結合、{len(selected_masks)}個)"
                        )
                        return "success"
                    ...
                    # シングルセレクションの場合の処理
                        ...
                        output_path = self._save_result(
                            image, selected_mask, image_path, output_dir
                        )
                        print(
                            f"保存完了: {output_path} (マスク{selected_idx}選択、スコア: {selected_score:.3f})"
                        )
                        return "success"
                else:
    ...
    def _display_overlayed_masks(
        self, image: np.ndarray, top_masks: List[Tuple[dict, float]], image_path: str
    ) -> Optional[int]:
        """
        ...
        for backend in backends_to_try:
            try:
                ...
                selected_idx = self._show_matplotlib_viewer_for_selection(
                    display_image, top_masks, image_path, (orig_h, orig_w), display_shape
                )
                return selected_idx
    ...
    def _show_matplotlib_viewer_for_selection(
        self,
        display_image: np.ndarray,
        top_masks: List[Tuple[dict, float]],
        image_path: str,
        ...
        try:
            ...
            if click_handler.is_multi_select and click_handler.selected_masks:
                ...
                return {
                    "type": "multi_select",
                    "masks": click_handler.selected_masks,
    ...
    def _determine_mask_from_click(
        self, x: int, y: int, top_masks: List[Tuple[dict, float]]
    ) -> Optional[int]:
        """
        クリック位置からマスクを判定
        """
    ...
    def _show_image_viewer_for_selection(
        self,
        display_image: np.ndarray,
        top_masks: List[Tuple[dict, float]],
        image_path: str,
        ...
        class OpenCVImageViewer:
            ...
            def mouse_callback(self, event, x, y, flags, param):
                ...
                if event == cv2.EVENT_LBUTTONDOWN:
                    ...
                    if abs(scale_x - scale_y) > 0.1:
                        print(
                            f"警告: 座標変換でズレが発生している可能性があります (scale_x: {scale_x:.3f}, scale_y: {scale_y:.3f})"
                        )
            ...
            def _determine_mask(self, x, y):
                ...
                print(
                    f"選択されたマスク: {selected_idx} (スコア: {self.mask_scores[selected_idx]:.3f}, ピクセル数: {max_pixels})"
                )
                if len(candidate_masks) > 1:
                    print(f"重複マスク数: {len(candidate_masks)}, 最大ピクセル数で選択")
    ...
    def _process_single_auto_extraction(self, image_path: str, output_dir: str) -> str:
        ...
        try:
            ...
            # 5. 品質評価による最適マスク選択
            best_mask, best_score = self._select_best_character_mask(
                image_rgb, filtered_masks, self.default_multi_character_criteria
            )
    ...
    def _select_best_character_mask(
        self,
        image: np.ndarray,
        filtered_masks: List[Tuple[dict, float]],
        ...
        if len(filtered_masks) > 1:
            ...
            if len(filtered_masks) > max_masks:
                ...
                # YOLOスコア順でトップ20を選択
                filtered_masks = sorted(filtered_masks, key=lambda x: x[1], reverse=True)[
                    :max_masks
                ]

            # 複合スコアによる選択（改良版）
            best_mask_info = self._select_best_character_with_criteria(
                filtered_masks, image.shape, criteria
            )
            if best_mask_info:
    ...
    def _try_merge_nearby_masks(
        self, filtered_masks: List[Tuple[dict, float]], image: np.ndarray
    ) -> Optional[dict]:
        """
        ...
        primary_center = (
            primary_bbox[0] + primary_bbox[2] / 2,
            primary_bbox[1] + primary_bbox[3] / 2,
        )
        ...
        for mask_data, yolo_score in filtered_masks[1:3]:  # 最多3個に制限
            ...
            # 距離計算
            distance = np.sqrt(
                (primary_center[0] - center[0]) ** 2 + (primary_center[1] - center[1]) ** 2
            )
    ...
    def _extract_and_save_character(
        self,
        image: np.ndarray,
        mask_data: dict,
        image_path: str,
    ...
    def _calculate_smart_merge_distance(
        self, primary_mask: dict, secondary_mask: dict, image: np.ndarray
    ) -> float:
        """
...
if __name__ == "__main__":
    ...
    parser.add_argument(
        "--mode",
        choices=["interactive", "batch", "choice", "reproduce-auto"],
        default="interactive",
        help="実行モード",
    )
    ...
    parser.add_argument(
        "--model_type", default="vit_h", choices=["vit_h", "vit_l", "vit_b"], help="SAMモデルタイプ"
    )
    ...
    parser.add_argument("--max_files", type=int, help="P1-018: 処理する最大ファイル数の制限")
    parser.add_argument(
        "--sam_optimization_profile",
        choices=["original", "p1_020_optimized", "p1_020_balanced", "p1_020_aggressive"],
        default="p1_020_optimized",
        help="P1-020: SAM最適化プロファイル (default: p1_020_optimized)",
    )
