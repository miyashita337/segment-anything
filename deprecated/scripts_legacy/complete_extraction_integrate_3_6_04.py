...
def get_missing_files():
    ...
    # 入力画像（cover画像除外）
    input_files = sorted([f.stem for f in input_dir.glob("kana08_*.jpg") 
                         if "cover" not in f.name])
...
def run_complete_extraction(logger):
    ...
    try:
        ...
        if missing_files:
            logger.info(f"  未処理リスト: {', '.join(missing_files[:5])}{'...' if len(missing_files) > 5 else ''}")
...
def send_completion_notification(stats, logger):
    ...
    try:
        ...
        notify_success(
            message,
            f"{status_emoji} 完全抽出{status_text} ({success_rate:.0f}%)",
            priority=1
        )
