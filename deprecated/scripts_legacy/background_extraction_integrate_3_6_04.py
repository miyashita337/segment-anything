...
def count_and_compare_results(logger):
    ...
    try:
        ...
        general_dir = Path(
            "/mnt/c/AItools/lora/train/yado/tracker-workspace/INTEGRATE-3-6-03/extraction"
        )
        ...
        logger.info(f"📊 比較結果:")
        logger.info(
            f"  アニメ特化版(3-6-04): {anime_count}/{total_input}枚 ({anime_count/total_input*100:.1f}%)"
        )
        logger.info(
            f"  汎用版(3-6-03): {general_count}/{total_input}枚 ({general_count/total_input*100:.1f}%)"
        )
...
def main():
    ...
    try:
        ...
        logger.info(
            f"最終結果: アニメ特化版 {anime_count}/{total_count}枚 vs 汎用版 {general_count}/{total_count}枚"
        )
        logger.info("=" * 60)
