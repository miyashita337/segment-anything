...
def analyze_extraction_results(extraction_dir: Path) -> dict:
    ...
    for img_path in image_files:
        ...
        total_size += size_kb
        file_data.append(
            {"name": img_path.name, "size_kb": size_kb, "base64": encode_image_to_base64(img_path)}
        )
...
def generate_dashboard_html(tracker_id: str, analysis_data: dict) -> str:
    ...
    success_rate = (
        (analysis_data["total_files"] / 24 * 100) if analysis_data["total_files"] > 0 else 0
    )
...
def generate_tracker_dashboard(tracker_id: str, logger: logging.Logger):
    ...
    logger.info(
        f"  - 分析完了: {analysis_data['total_files']}ファイル, {analysis_data['total_size_mb']:.2f}MB"
    )
