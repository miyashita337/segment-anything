...
class ImprovedExtractionSystem:
    ...
    def __init__(self):
        ...
        self.boundary_enhancer = BoundaryEnhancer(
            skin_enhancement_factor=1.3, edge_enhancement_factor=1.8, contrast_enhancement=1.2
        )
    ...
    def process_test_image(self, image_path: Path, output_dir: Path) -> dict:
        ...
        results["processing_steps"].append(
            {"step": "boundary_enhancement", "time": enhancement_time, "stats": enhancement_stats}
        )
        ...
        results["processing_steps"].append(
            {"step": "mask_generation", "time": mask_time, "mask_area": np.sum(test_mask > 0)}
        )
        ...
        results["processing_steps"].append(
            {"step": "limb_protection", "time": protection_time, "analysis": limb_analysis}
        )

        # 結果保存
        self._save_test_results(
            output_dir, image_path.stem, image_rgb, enhanced_image, test_mask, protected_mask
        )

        logger.info(
            f"処理完了: {image_path.name} (総時間: {sum(step['time'] for step in results['processing_steps']):.2f}秒)"
        )
    ...
    def _save_test_results(
        ...
        # テストマスク
        cv2.imwrite(str(output_dir / f"{base_name}_03_test_mask.jpg"), test_mask)

        # 保護後マスク
        cv2.imwrite(str(output_dir / f"{base_name}_04_protected_mask.jpg"), protected_mask)

        # 比較画像作成
        comparison = self._create_comparison_image(original, enhanced, test_mask, protected_mask)
        cv2.imwrite(str(output_dir / f"{base_name}_05_comparison.jpg"), comparison)
    ...
    for image_name in test_images:
        ...
        if image_path.exists():
            try:
                ...
                results.append({"image_name": image_name, "error": str(e)})
        else:
        ...
        if "error" not in result:
            ...
            for step in result["processing_steps"]:
                if step["step"] == "boundary_enhancement":
                    ...
                    logger.info(
                        f"  フィルタリング: {step['original_candidates']} → {step['filtered_candidates']} 候補"
                    )
                elif step["step"] == "limb_protection":
                    analysis = step["analysis"]
                    logger.info(
                        f"  手足保護: 適用={analysis['protection_applied']}, 品質={analysis['protection_quality']:.2f}"
                    )
        else:
