...
class Kana08ImprovedExtractor:
    ...
    def extract_character(
        ...
        try:
            # YOLO検出
            detections = self.yolo_wrapper.detect(
                image, confidence_threshold=self.confidence_threshold
            )
            if not detections:
                return None
            ...
            masks, scores, _ = self.sam_wrapper.predict(
                image, box=box_prompt, multimask_output=True
            )
            ...
            # SCI評価計算
            sci_scores = self.sci_processor.calculate_sci(image, mask, bbox=(x1, y1, x2, y2))
    ...
    def run_batch(self):
        ...
        # 個別結果追加
        for i, (image_path, sci_scores) in enumerate(
            zip(image_files[: len(sci_scores_list)], sci_scores_list)
        ):
            report["individual_results"].append(
                {"filename": image_path.name, "sci_scores": sci_scores}
            )
