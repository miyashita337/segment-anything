#!/usr/bin/env python3
"""
バックアップスクリプトロジック移植版バッチ処理 - 26枚の画像を正しく抽出
"""
import os
import shutil
import sys
...
def main():
    ...
    output_dir = Path("/mnt/c/AItools/lora/train/yado/clipped_boundingbox/kana08_claude_uni_13_9_backup_migrated")
                ...
                if result.stdout:
                    ...
                    for line in result.stdout.split('\n'):
                        if 'Character extracted:' in line and 'size:' in line:
                            print(f"   {line.strip().split('Character extracted:')[1]}")
                            break
            else:
                failed += 1
