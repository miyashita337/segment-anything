#!/usr/bin/env python3
"""
Phase 2 Fixed Background Batch Processor
LoRA学習用キャラクター判定修正版バッチ処理
"""
...
# ロギング設定（ファイル出力）
log_file = f"phase2_fixed_batch_{datetime.now().strftime('%Y%m%d_%H%M%S')}.log"
logging.basicConfig(
    level=logging.INFO,
...
class Phase2FixedBatchProcessor:
    """Phase 2修正版バッチ処理システム"""
    ...
    def __init__(self):
        ...
        try:
            ...
            logger.info("✅ Phase 2修正版バッチ処理システム初期化完了")
    ...
    def _save_current_progress(self):
        ...
        progress_file = "phase2_fixed_batch_progress.json"
        with open(progress_file, 'w', encoding='utf-8') as f:
            json.dump(progress_data, f, indent=2, ensure_ascii=False)
        
        logger.info(f"💾 進捗保存完了: {progress_file}")

    def process_single_image_fixed(self, 
                                  image_path: Path, 
        ...
        try:
            ...
            result.update({
                ...
                "method_used": extraction_result.get("best_method", "robust_extraction_fixed")
            })
    ...
    def run_fixed_batch(self):
        """修正版バッチ処理実行"""
        logger.info("🎬 Phase 2修正版バッチ処理開始")
        logger.info(f"📄 ログファイル: {log_file}")
        
        # 入力・出力ディレクトリ
        input_dir = Path("/mnt/c/AItools/lora/train/yado/org/kana08")
        output_dir = Path("/mnt/c/AItools/lora/train/yado/clipped_boundingbox/kana08_fixed_character_detection")
        ...
        for i, image_path in enumerate(image_files, 1):
            ...
            # 単一画像処理
            result = self.process_single_image_fixed(image_path, output_path)
    ...
    def _finalize_batch_processing(self, results: List[Dict[str, Any]], output_dir: Path):
        ...
        summary = {
            ...
            "phase2_improvements": {
                "character_detection_fix": "LoRA学習用に顔なしキャラクター対応",
        ...
        # 結果保存
        results_path = output_dir / "phase2_fixed_batch_results.json"
        with open(results_path, 'w', encoding='utf-8') as f:
            json.dump(summary, f, indent=2, ensure_ascii=False, default=str)
        
        # 最終ログ出力
        logger.info("🎉 Phase 2修正版バッチ処理完了")
        logger.info("=" * 60)
        ...
        # 完了通知メッセージ作成
        notification_message = f"""🎉 Phase 2修正版バッチ処理完了！
...
📄 ログ: {log_file}"""
        ...
        # 通知メッセージをファイルに保存
        with open("phase2_fixed_completion_message.txt", 'w', encoding='utf-8') as f:
...
def main():
    ...
    try:
        ...
        processor = Phase2FixedBatchProcessor()
        results = processor.run_fixed_batch()
...
if __name__ == "__main__":
    success_count = main()
    print(f"\n🎯 Phase 2修正版バッチ処理完了: {success_count}枚成功")