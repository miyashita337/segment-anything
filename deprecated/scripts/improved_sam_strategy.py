...
class ImprovedSAMStrategy:
    ...
    def strategy_single_center_point(
        self, image: np.ndarray, yolo_bbox: Tuple[int, int, int, int]
    ...
    def strategy_with_negatives(
        ...
        if not negative_points:
            # フォールバック: 四隅から適当な背景点
            negative_points = [[margin, margin], [img_w - margin, margin]]
    ...
    def strategy_bbox_prompt(
        ...
        expanded_bbox = np.array(
            [max(0, x - expand_w), max(0, y - expand_h), x + w + expand_w, y + h + expand_h]
        )
