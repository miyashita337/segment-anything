    ...
    <div class="container">
        ...
        <div class="content">
            <div class="external-access">
                <h4>🌐 外部アクセス情報</h4>
                <p><strong>内部URL:</strong> http://localhost:80</p>
                <p><strong>外部URL:</strong> http://100.123.241.106 (Basic認証: admin / integrate36)</p>
...
def start_external_server(port=80):
    ...
    try:
        ...
        logger.error("❌ ポート80の使用には管理者権限が必要です。別のポートで起動してください。")
        logger.info("💡 推奨: sudo python3 external_dashboard_server.py")
    except Exception as e:
