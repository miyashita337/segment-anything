...
def analyze_extraction_directory(extraction_dir):
    ...
    # dashboard, .checkpointディレクトリ内のファイルを除外
    image_files = [f for f in image_files if 'dashboard' not in str(f) and '.checkpoint' not in str(f)]
    
    if not image_files:
        raise FileNotFoundError(f"画像ファイルが見つかりません: {extraction_dir}")
    
    print(f"📊 P1-021品質解析開始: {len(image_files)}枚")
...
def main():
    ...
    extraction_dir = '/mnt/c/AItools/lora/train/yado/tracker-workspace/P1-021/extraction'
    output_file = '/mnt/c/AItools/lora/train/yado/tracker-workspace/P1-021/extraction_result.json'
    
    print("🔧 P1-021品質スコア生成システム起動")
