...
#!/usr/bin/env python3
"""
INTG-046 外部アクセス対応ダッシュボードサーバー（ポート8080版）
Basic認証付きで外部IPからアクセス可能
"""
...
<head>
    ...
    <style>
        ...
        .auth-box { background: white; padding: 30px; border-radius: 10px; box-shadow: 0 4px 6px rgba(0,0,0,0.1); max-width: 400px; margin: 0 auto; }
        h1 { color: #e74c3c; }
        .credentials { background: #f8f9fa; padding: 15px; border-radius: 5px; margin: 20px 0; border-left: 4px solid #3498db; }
    </style>
</head>
<body>
    <div class="auth-box">
        <h1>🔒 認証が必要です</h1>
        <p>INTG-046 ダッシュボードにアクセスするには認証が必要です。</p>
        <div class="credentials">
            <strong>認証情報:</strong><br>
            ユーザー名: <code>admin</code><br>
            パスワード: <code>integrate36</code>
        </div>
        <p>ブラウザの認証ダイアログに上記の情報を入力してください。</p>
    </div>
</body>
</html>"""
...
<head>
    ...
    <title>INTG-046 Series Dashboard Portal</title>
    <style>
        * {{ margin: 0; padding: 0; box-sizing: border-box; }}
        ...
        .auth-info {{ background: #e8f4fd; padding: 15px; border-radius: 8px; margin: 20px 0; border-left: 4px solid #3498db; }}
        ...
        .external-access {{ background: #d4edda; padding: 15px; border-radius: 8px; margin: 20px 0; border-left: 4px solid #28a745; }}
    </style>
</head>
<body>
    <div class="container">
        <div class="header">
            <h1>🔬 INTG-046 Series</h1>
            <div class="subtitle">キャラクター抽出結果ダッシュボード ポータル (外部アクセス版)</div>
            <div class="auth-info">
                <strong>🔒 認証済み:</strong> admin ユーザーとしてアクセス中
            </div>
        </div>
        <div class="content">
            <div class="external-access">
                <h4>🌐 外部アクセス情報</h4>
                <p><strong>内部URL:</strong> http://localhost:8080</p>
                <p><strong>外部URL:</strong> http://100.123.241.106:8080 (Basic認証: admin / integrate36)</p>
        ...
        <div class="footer">
            <p>🔬 INTG-046 Series Dashboard Portal | 外部アクセス版 | Basic認証対応 | Generated with Claude Code</p>
        </div>
    </div>
</body>
</html>"""
...
def start_external_server(port=8080):
    """外部アクセス対応サーバー起動"""
    ...
    try:
        with HTTPServer(server_address, BasicAuthHandler) as httpd:
            ...
            logger.info(f"🌐 外部アクセス: http://100.123.241.106:{port} (認証: admin/integrate36)")
            ...
            for tracker_id in ["INTG-046-01", "INTG-046-02", "INTG-046-03", "INTG-046-04"]:
                ...
                if dashboard_file.exists():
                    logger.info(f"  ✅ http://100.123.241.106:{port}/{tracker_id}")
                else:
        ...
        # PIDファイル削除
        if Path("external_dashboard_server.pid").exists():
            Path("external_dashboard_server.pid").unlink()
