...
def send_image_to_pushover(image_path: Path, config: dict, logger: logging.Logger, index: int, total: int):
    """個別画像をPushoverに送信"""
    try:
        ...
        with open(image_path, 'rb') as f:
            ...
            if response.status_code == 200:
                logger.info(f"✅ 送信成功 [{index}/{total}]: {image_path.name} ({file_size_kb:.1f}KB, 品質:{quality:.3f})")
                return True
            else:
