...
class BulkPathFixer:
    ...
    def add_import_if_needed(self, content: str) -> str:
        ...
        if any(func in content for func in ['get_path(', 'get_test_image_path(', 'setup_test_env(']):
            # インポート追加位置を特定
            lines = content.split('\n')
            import_insert_pos = 0
    ...
    def find_target_files(self) -> List[Path]:
        ...
        for root, dirs, files in os.walk(project_root):
            # 除外ディレクトリ
            dirs[:] = [d for d in dirs if d not in ['.git', '__pycache__', '.pytest_cache', 'sam-env']]
