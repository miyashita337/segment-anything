...
class PracticalSampleSizeValidator:
    ...
    def __init__(self):
        ...
        self.validator = SampleSizeValidator(
            default_power=0.70, default_alpha=0.10  # 70%検出力（統計学的80%より緩和）  # 10%有意水準（統計学的5%より緩和）
        )
    ...
    def validate_practical_adequacy(self, current_sample_size: int):
        ...
        validation = self.validator.validate_sample_adequacy(
            current_sample_size=current_sample_size, test_scenarios=practical_scenarios
        )
    ...
    def _evaluate_ml_adequacy(self, n: int) -> dict:
        ...
        if n >= 50:
            ...
            return {"level": "good", "description": "機械学習プロジェクトとして十分なサンプルサイズ", "confidence": "中信頼性"}
        elif n >= 20:
            return {
                "level": "acceptable",
            ...
            return {"level": "minimal", "description": "プロトタイプ・初期検証レベル", "confidence": "限定的"}
        else:
            return {
                "level": "insufficient",
