...
### **Git作業ブランチ検証・作成** ✅ 【KIRO-002準拠違反対策】
- [ ] **現在ブランチ状態確認**: `git branch --show-current` で現在ブランチ確認
- [ ] **🚨 CRITICAL**: feature/{TRACKER_ID}ブランチでない場合は以下必須実行
  ```bash
  # ブランチ検証関数（必須実行）
  function validate_branch() {
      current_branch=$(git branch --show-current)
      if [[ "$current_branch" != "feature/"* ]]; then
          echo "❌ FATAL: 作業ブランチではありません - 現在: $current_branch"
          echo "✅ 修正必要: git checkout -b feature/{TRACKER_ID} を実行してください"
          return 1
      fi
      echo "✅ ブランチ検証OK: $current_branch"
      return 0
  }
  ...
  ```
- [ ] **🚨 CRITICAL**: ブランチ作成・切り替え完了まで次フェーズ禁止
- [ ] **feature/{TRACKER_ID}ブランチ作成**: `git checkout -b feature/{TRACKER_ID}`
- [ ] **作成後確認**: `git branch --show-current` でfeature/{TRACKER_ID}表示確認
- [ ] **⚠️ 重要**: この検証完了まで Phase 1 以降の全作業実行禁止
...
### ステップ2.5: SOW（作業範囲確定書）作成 ✅ 【新規追加】
- [ ] **作業スコープ定義**: 実装対象・除外項目の明文化
- [ ] **成果物定義**: 作成する成果物・品質基準の明確化
- [ ] **責任範囲明示**: Claude Code担当範囲・ユーザー確認事項の分離
- [ ] **承認条件設定**: 完了判定条件・中間承認ポイント設定
- [ ] **リソース制約確認**: 時間・技術・依存関係の制約事項明文化
- [ ] **📋 SOW承認取得**: 作業開始前のSOW内容承認必須

### ステップ3: 詳細実装計画書作成（SOW承認後） ✅ 【改訂】
- [ ] **前提条件**: SOW承認完了確認必須
- [ ] **実装計画**: SOW範囲内での技術アーキテクチャ・実装手順・品質基準
- [ ] **品質計画**: テスト戦略・品質ワークフロー・成功基準
- [ ] **パフォーマンス測定計画**: ベースライン取得・測定項目・評価方法
- [ ] **リスク分析**: 技術的困難・計画変更への対応策
- [ ] **例外処理手順**: エラーパターン・エスカレーション・回復手順

### ステップ4: 📋 **承認1: SOW・詳細計画承認** ✅ 【改訂】
- [ ] **📋 SOW最終承認**: 作業範囲・成果物・責任範囲の最終確認
- [ ] **📋 計画承認**: SOW準拠の詳細実装計画承認取得
...
### ステップ6: 抽出実行・Git管理開始 ✅
- [ ] **📋 Phase 0.5完了確認**: feature/{TRACKER_ID}ブランチ作業中であることを確認
...
- [ ] **🤖 自動ダッシュボード品質保証チェック必須実行**（TEST-001バグ再発防止）:
  ```bash
  # プログラムレベル品質チェック（4セクション完全自動化）
  python3 tools/testing/dashboard_quality_validator.py {TRACKER_ID}
  
  # 【重要】終了コード0（全セクション通過）確認必須
  echo $?  # 0なら成功、1なら失敗
  ```
- [ ] **📋 手動補完チェック**（自動チェック通過後の最終確認）:
  ...
  - [ ] 🔴 **画像ギャラリー必須表示確認** (実際のファイル名・品質スコア・品質ラベル・Base64埋め込み禁止・相対パス参照表示)
  - [ ] 🔴 **curl仕様書通り動作確認**: `curl -u admin:secure_track_2025_q3_8f9a http://[SERVER]:8088/tracker/{TRACKER_ID}`
  - [ ] 🔴 **完了報告前curl必須実行**: すべての完了報告前に上記curlコマンドでアクセス確認を実行し、要件満たされていることを検証すること
...
## 🤖 **AI非冪等性への対策（KIRO-002事例から学習）**

### **問題の本質**
- **AIの確率的推論**: 同じチェックリストでも注意機構により項目スキップが発生
- **文脈依存の優先度**: 技術的タスクが手続き的タスクより優先される傾向
- **記憶劣化**: 長いセッションで初期要件の重要度が低下

### **対策の設計思想** 
```bash
# 従来のアプローチ（失敗）
チェックリスト項目記載 → AI判断 → 確率的実行 → スキップリスク

# 新アプローチ（成功）  
Phase 0.5独立実行 → 強制検証 → 次フェーズ禁止 → 確実実行
```

### **Phase 0.5の技術的意義**
- **独立性**: 他フェーズから分離・専門化
- **強制性**: CRITICAL レベルによる次ステップ禁止
- **検証性**: スクリプト化による自動確認
- **冪等性**: 外部システム設計による動作保証
...
### 🚫 ブランチ検証フェーズの省略（KIRO-002対策）
```
❌ Phase 0.5スキップ → mainブランチ作業 → ワークフロー準拠違反
✅ Phase 0.5必須実行 → feature/{TRACKER_ID}確認 → 安全作業開始
```

### 🚫 計画フェーズの省略
```
❌ 仕様書確認なし → 直接実装開始
✅ Phase 0.5 → ステップ1-4完了 → 計画承認後実装開始
```
