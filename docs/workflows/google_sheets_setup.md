...
| 列 | 項目 | 説明 |
|---|---|---|
| A | トラッカーID | PH1-001, PH2-001 等 |
| B | ステータス | 着手前、着手中、実装完了等 |
| C | 登録日付 | YYYY-MM-DD形式（デフォルト空文字） |
| D | 更新日付 | YYYY-MM-DD形式（デフォルト空文字） |
| E | 概要 | タスクの説明 |
| F | 動作確認 | 完了、失敗、未実行 |
| G | テストUNIT | 完了、失敗、未実行 |
| H | 品質評価 | 完了、失敗、未実行 |
| I | 統合実行スクリプト | 完了、失敗、未実行 |
| J | ダッシュボード生成 | 完了、失敗、未実行 |
| K | 抽出パイプライン | 完了、失敗、未実行 |
...
| N | FPS | 処理速度（フレーム/秒） |
| O | C以上評価率 | 標準以上品質の割合 |
| P | 平均カバレッジ率 | マスクカバレッジ平均 |
| Q | 平均コンパクトネス | 形状コンパクト性平均 |
| R | 平均フィル率 | マスク充填率平均 |
| S | SCI | Semantic Completeness Index |
| T | PLA | Pixel-Level Accuracy |
| U | PLE | Progressive Learning Efficiency |
