...
**参照**: [AI-人間協調ワークフロー](./README.md) - ①タスク定義フェーズ  
**最終更新**: 2025-07-27  
**重要変更**: GitHub Action仕様から現在のClaude Code仕様に更新

## 📋 概要

人間からの曖昧な要求やフィードバックを、Claude Code が分析・具体化してGoogle Sheetsで管理する具体的なタスクに変換するプロセスを定義します。

## 🎯 タスク定義の目標

1. **言語化・具体化**: 人間の曖昧な要求を明確なタスクに変換
2. **実装可能性評価**: 技術的制約・リスクの評価と解決策提示
3. **工数見積り**: メリット・デメリット・工数の定量化
4. **体系化**: 大規模タスクの適切な分割とフェーズ化
...
- **最終判断**: Claude Code提案に対するLGTM/NG判定
- **優先度決定**: タスクの重要度・緊急度の指定
- **品質基準設定**: 期待する品質レベルの明示

### 🤖Claude Code の役割

- **要求分析**: 人間の意図を技術的観点から詳細分析
- **実装計画策定**: 具体的な実装手順とテスト計画の作成
- **リスク評価**: 技術的制約・リスクの特定と対策案提示
- **Google Sheets管理**: タスク登録と進捗更新の自動化
- **代替案提示**: 複数のアプローチ案の比較検討
- **工数算出**: 開発・テスト・検証工数の見積り
